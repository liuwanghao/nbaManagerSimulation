Original prompt: 那就按照你的开发计划执行吧

Current request (2026-09-26): 阵容轮换页不再横排展示五个首发位置。

### 轮换编辑器改为逐人调整 — complete

- [x] 移除顶部五位置横排；每名球员的分钟行增加首发位置选择，保留五人唯一首发与 240 分钟校验。
- [x] 替补升为首发时自动接收被替换球员的分钟（仅当原本为 0 分钟）；首发改为非首发时自动补齐空缺位置。
- [x] 移除轮换编辑器下方重复的只读阵容名单，保留一键自动匹配与手动保存。
- [x] TypeScript、12 项轮换/阵容定向测试、生产构建及差异检查通过。
- [ ] 浏览器截图仍受现有本地文件访问策略和 Playwright 权限限制，本轮未完成视觉实测。

Current request (2026-09-26): 阵容轮换增加一键自动匹配最合理的首发与时间分配。

### 一键自动匹配阵容轮换 — complete

- [x] 沿用已验证的自动算法：联合优化五个首发位置，排除不可出战球员，按能力选择轮换并分配 240 分钟。
- [x] “一键自动匹配”按钮直接提交 `SET_ROTATION_PLAN` 并自动保存；保存成功后同步编辑器，失败保留原方案与错误状态。手动调整及保存入口保留。
- [x] 调整按钮与窄屏布局；TypeScript、12 项阵容/轮换定向测试、生产构建及差异检查通过。
- [ ] 本机浏览器访问与 Playwright Chromium 仍受前述沙箱和浏览器策略限制，本轮无画面截图。

Current request (2026-09-26): 整理常规赛底部“管理”菜单，核对并移除当前引擎不支持的内容。

### 常规赛管理菜单整理 — complete

- [x] 管理二级菜单收为概览、阵容轮换、合同薪资、选秀权；移除仅季前可设置的培养占位页和未实现的交易货架。
- [x] 阵容页移除错放的完整赛程，合同页移除重复的交易窗口状态，选秀权库存改为“持有/已承诺”并明确暂不能主动加入报价；移除与顶栏重复且已被样式隐藏的存读档页脚。
- [x] TypeScript、生产构建、16 项阵容/轮换/交易定向测试及差异检查通过。
- [ ] 浏览器视觉检查未完成：本地服务绑定端口被沙箱拒绝，Playwright Chromium 被 MachPort 权限阻止，浏览器安全策略禁止打开本地构建文件。

Current request (2026-09-26): 赛后数据中心的本场 MVP 只从获胜球队球员中选取。

### 本场 MVP 胜方限定 — complete

- [x] `getGameMvpStat` 只读取 `winnerTeamId` 对应的 Box Score，沿用原有数据权重和同分规则；胜方详细数据缺失时不误选败方。
- [x] 回归测试覆盖败方球员数据更高、主客场互换和胜方详细数据缺失；定向测试 5/5 通过。
- [x] 全量 232 项测试、TypeScript/生产构建、差异检查、ZIP 完整性与 H5/ZIP 脚本 SHA-256 对照通过。
- [ ] Playwright 浏览器检查仍被本机 Chromium/MachPort 权限拦截；本地 HTTP 服务也被沙箱禁止绑定端口，本轮未取得浏览器截图。逻辑变更有领域回归测试覆盖。

Current request (2026-09-26): 下一场对阵将分区排名移至战绩同行，客场/主场文案另放合适位置。

### 对阵卡信息层级调整 — complete

- [x] 两侧球队卡改为“队名 → 战绩 · 分区排名 → OVR”；中间 VS 下方以“客场 · 主场”明确左右顺序，卡片高度不增加。
- [x] 320px 开发版赛季 fixture 视觉检查：战绩/排名与中间标识未截断；球队卡打开阵容预览并关闭返回正常，页面错误日志为空。
- [x] 回归测试、TypeScript/生产构建、231 项全量测试与发布 ZIP 校验通过。
- [ ] 技能附带的 Playwright 客户端仍被本机 Chromium/MachPort 权限拦截；已用本地浏览器完成页面视觉和交互核对。

Current request (2026-09-26): 下一场对阵中展示两支球队各自的分区排名。

### 下一场对阵球队排名 — complete

- [x] 复用东西部分区排名结算结果，分别给客队和主队显示“西部/东部第 N”；与战绩、OVR 同时保留，不增加卡片行高。
- [x] 添加页面回归测试，核对两队当前排名、主客标识和战绩。
- [x] TypeScript/生产构建、231 项全量测试、差异检查、ZIP 完整性及 H5/ZIP SHA-256 对照通过。
- [x] 开发版赛季 fixture 在 320px 手机宽度下确认两侧排名与 OVR 同行、无溢出；页面错误日志为空。
- [ ] 技能附带的 Playwright 客户端仍被本机 Chromium/MachPort 权限拦截；已用本地浏览器完成等效的页面视觉与 DOM 核对。

Current request (2026-09-26): 修正首发对位中自动排布的错位、赛后比分头部移除“胜”字并拉开比分与中间状态的距离。

### 自动首发位置与赛后比分 — complete

- [x] 自动首发和缺阵补位改为五槽位整体优化；原生/副位置均零损失，避免逐格贪心造成不必要的错位。
- [x] 自动与手动首发方案分开保存；仅精确匹配旧自动方案的存档会升级，玩家手动错位仍保留并影响模拟。
- [x] 首发对位显示错位标记；赛后球队名不再加“胜”字，两侧比分略向队徽方向移动。
- [x] 230 项测试、TypeScript/生产构建和差异检查通过；H5 与发布 ZIP 已更新。
- [ ] 赛后/对位弹框视觉回归尚未通过浏览器实测；本机 file URL 被浏览器策略拦截。

Current request (2026-09-26): 赛后数据中心标题改主色青蓝、压缩比分头部、球队数据分两个 tab、每队五项最高值按队色高亮。

### 赛后数据中心布局与队内最高值 — complete

- [x] “比赛详情 · 日期”改主色青蓝；比分头部缩短，队徽在球队名上方，比分分列于靠中间侧。
- [x] 球员表格维持现有列/行样式，改为客队/主队两个可点击且可方向键切换的标签页；历史存档无 Box Score 时给出空态。
- [x] 得分、篮板、助攻、抢断、盖帽分别选当前球队最高值各一人，沿用表格顺序处理并列，使用与球队标签圆点一致的主色高亮。
- [x] 纯函数与页面级定向测试、TypeScript/生产构建通过。
- [x] 全量 229 项测试、TypeScript/生产构建、差异检查、ZIP 完整性及 H5/ZIP 脚本 SHA-256 对照通过。
- [ ] Playwright 已尝试，本机 Chromium/MachPort 权限仍阻止本轮视觉截图。

Current request (2026-09-26): 首页及首发对位统一左客场、右主场；弹框 OVR 移至姓名/位置的侧边。

### 主客场顺序与侧边 OVR — complete

- [x] 下一场对阵卡按赛程 awayTeamId/homeTeamId 固定左右顺序，双方仍可分别点击查看阵容，并标明客场/主场。
- [x] 首发对位卡改成镜像布局：客队姓名/位置在左、OVR 在右；主队 OVR 在左、姓名/位置在右，OVR 显示整数并沿用分级色。
- [x] 增加玩家主场及客场两种首页布局回归测试；定向测试与生产构建通过。
- [x] 全量 226 项测试、TypeScript/生产构建、差异检查、ZIP 完整性与 H5/ZIP 脚本 SHA-256 对照通过。
- [ ] Playwright 已尝试，本机 Chromium/MachPort 权限仍阻止本轮视觉截图。

Current request (2026-09-26): 阵容预览的“首发”筛选标记每名球员实际首发位置。

### 首发槽位标记 — complete

- [x] 在首发球员状态旁标出 PG/SG/SF/PF/C 实际槽位，保留原生/副位置标签以区分错位首发。
- [x] “全部/首发”中的五名首发按 PG→C 排列；位置映射读取当前有效首发安排。
- [x] 增加手动交换 PG/SG 的回归测试，确保显示的是首发槽位而非球员原生位置。
- [x] 全量 225 项测试、TypeScript/生产构建、差异检查、发布 ZIP 完整性及 H5/ZIP 脚本 SHA-256 对照通过。
- [ ] Playwright 已尝试，Chromium/MachPort 权限仍阻止本轮视觉截图。

Current request (2026-09-26): 在赛季首页主客场标识下增加首发位置对位按钮和弹框，比较两队球员姓名、位置与 OVR。

### 首发位置对位图 — complete

- [x] “下一场对阵”中间的主客场标识下新增“首发对位”入口；弹框按 PG、SG、SF、PF、C 对照客场与主场预计首发。
- [x] 沿用已保存的手动首发与现有伤病自动补位计算；展示中文球员名、实际位置、OVR 分级颜色。
- [x] 延续赛季页青蓝直角样式，支持点击遮罩、关闭按钮或 Esc 退出。
- [x] 定向 2 项和全量 225 项测试、TypeScript/生产构建、差异检查通过；发布 ZIP 完整性及 H5/ZIP 脚本 SHA-256 一致性通过。
- [ ] Playwright/本地浏览器工具受 macOS MachPort 与 file URL 安全策略限制，未取得真实截图。

Current request (2026-09-26): 阵容预览按“全部/首发/轮换/未激活”筛选，并稳定模块与球员卡高度。

### 阵容状态筛选与固定高度 — complete

- [x] 只将预览页筛选改为四种轮换状态；赛季管理与季前名单继续保留位置筛选。
- [x] 全部按首发→轮换→未激活分组排序，各分类显示人数；卡片状态文案同步统一为“首发/轮换/未激活”。
- [x] 球员卡固定高度，预览弹框固定为较低的视口高度并仅让名单区滚动，避免切换筛选时整体跳动。
- [x] 定向 9 项与全量 223 项测试、TypeScript/生产构建、差异检查、发布 ZIP 完整性及 H5 脚本 SHA-256 对照通过。
- [x] Playwright 截图客户端已尝试；本机 Chromium/MachPort 沙箱权限阻止启动，因此本轮无真实浏览器视觉截图。

Current request (2026-09-26): 阵容预览恢复“全部/PG/SG/SF/PF/C”位置筛选列表，展示全队并区分上场状态与伤病。

### 完整名单筛选版预览 — complete

- [x] 复用阵容管理中的完整名单横卡与位置筛选；新增预览状态：首发、轮换上场、未上场，受伤/缺阵独立标记。
- [x] 保留弹框 82% 视口高度上限和球员详情返回层级；移除预览专用五人卡片及替补卡片 CSS。
- [x] 先增加旧列表“全部位置和全员卡片”回归测试，再增加新预览全员/轮换/伤病测试；定向 9 项与构建通过。
- [x] 全量 223 项测试、生产构建、发布包完整性与 H5 脚本 SHA-256 对照通过；Playwright 仍因本机 Chromium/MachPort 沙箱权限失败，未取得本轮视觉截图。

Current request (2026-09-26): 阵容预览按球队实际轮换最多展示 10 人，同时交代全队人数。

### 阵容预览十人轮换 — complete

- [x] 首发五人继续按固定位置展示；替补仅取可出场、目标分钟大于 0 的球员，按目标分钟排序且总人数不超过 10；无手动计划时按能力值选默认替补。
- [x] 顶部显示轮换人数与全队人数，替补区明确标为“轮换替补”；本队完整名单仍在阵容管理。
- [x] 增加默认十人、手动七人、缺阵替补的领域测试；定向测试 7/7、TypeScript/构建和差异检查通过。
- [x] 全量 221 项测试、生产构建、ZIP 完整性与 H5 脚本 SHA-256 对照通过；Playwright 再次因本机 Chromium/MachPort 沙箱权限失败，未完成视觉截图。

Current request (2026-09-26): 阵容预览改为紧凑五人阵容条、易读替补名单，弹框高度随内容收起。

### 阵容预览布局优化 — complete

- [x] 保留 PG/SG/SF/PF/C 同行，改为共用边框的五个紧凑阵容位；预览用简称，详情与无障碍标签保留完整姓名。
- [x] 手机端替补改为单列横向名单，移除需要将姓名缩至 8.5px 的动态测量组件。
- [x] 弹框由手机端强制全屏改为内容自适应，最高限制在视口约 82%，超出时仅名单区滚动。
- [x] TypeScript/生产构建、`git diff --check`、全量 219 项测试、发布 ZIP 完整性与 H5 脚本 SHA-256 对照通过。
- [x] Playwright 截图客户端已尝试；Chromium 因当前 macOS 沙箱 MachPort 权限被拒，未取得本轮视觉截图，后续需在可运行浏览器环境人工核对 360px/390px 布局。

Current request (2026-09-26): 首页“开始新游戏”和“读取存档”文字大小不一致。

### 首页按钮字号统一 — complete

- [x] 移除流程主/次按钮全局规则对首页启动按钮的覆盖；“开始新游戏”“读取存档”“继续上次进度”统一使用原有 13px / 850 字重。
- [x] 保留扩军及赛前其他流程按钮现有的主次字号，不影响其布局。

Current request (2026-09-26): 按参考图收小阵容预览字体比例。

### 阵容预览字号比例 — complete

- [x] 手机端缩小球队标题、分组标题、位置标签、首发/替补 OVR 与底部按钮字号，并将首发卡高度从 108px 收至 96px（极窄屏 92px）。
- [x] 姓名字号改按容器宽度自适应：窄屏首发约 9px、替补约 10–11px，桌面仍可放大；长名字保留两行/单行截断与完整 title。
- [x] 保持五个首发一行、直角边框、方形头像与球员详情返回层级不变。
- [x] TypeScript/生产构建、相关 10 项测试、ZIP 完整性及构建脚本 SHA-256 对照通过；本机浏览器安全策略仍阻止本轮自动截图验证。

Current request (2026-09-26): 阵容预览恢复五名首发一行，压低卡片并优化紧凑布局。

### 五列紧凑首发预览 — complete

- [x] 手机端始终维持 PG/SG/SF/PF/C 五列，不再拆成三列或两列。
- [x] 首发卡改为较矮的矩形，位置徽标、方形头像、两行自适应姓名、底部能力值分层对齐；压缩弹窗内容区与分组间距，保留直角青蓝主风格。
- [x] 保留从阵容预览打开球员详情、关闭后返回原阵容预览的行为。
- [x] TypeScript/生产构建、`git diff --check` 与正式 ZIP 完整性和脚本 SHA-256 一致性通过；受本机浏览器安全限制，本轮无最新截图视觉判定。

Current request (2026-09-26): 阵容预览统一主 UI 的直角风格、方形头像与首发卡，长名字自适应，并修正球员详情关闭后的返回层级。

### 阵容预览样式与返回层级 — complete

- [x] 阵容预览边框、关闭按钮、位置标签、首发/替补卡片、头像和底部按钮统一为直角与青蓝主色；首发卡固定正方形，窄屏改三列、极窄屏两列。
- [x] 首发姓名按卡片可用宽度自动缩字号并最多显示两行；替补姓名按单行可用宽度缩字号，极长名字保留完整 title。
- [x] 打开球员详情时保留阵容预览底层，详情层级置于其上，关闭后回到原球队阵容。
- [x] 生产构建、TypeScript 和相关 10 项测试通过。浏览器截图检查仍受本机启动权限与本地文件 URL 策略限制，未声称完成视觉实测。

Current request (2026-09-26): 对阵双方保留球队 OVR 数值，仅移除“OVR”文案。

### 对阵球队能力数字 — complete

- [x] 两张球队卡片恢复按球队总评算法计算的彩色能力数字，不显示“OVR”标签；赛季概览的球队 OVR 保持原样。
- [x] TypeScript 与生产构建通过。自动截图验证受本机浏览器启动权限和本地文件 URL 安全策略限制，未进行本轮视觉截图。

Current request (2026-09-26): 精简赛季对阵卡、修正日历跟随，并重做球队阵容展示。

### 赛季对阵与球队阵容 — complete

- [x] 对阵卡移除重复的球队 OVR，模拟下一场按钮移除箭头并居中文案。
- [x] 赛季页初始日历定位到游戏当前日期；连续模拟时逐日横向跟随，跨月切换，且不会带动页面纵向滚动。
- [x] 点击对阵球队进入独立阵容页：PG/SG/SF/PF/C 五名首发、两列替补；本队提供阵容管理入口。
- [x] 球员详情去掉球队角色与“加入交易获取报价”操作。
- [x] 390px 浏览器核对阵容、球员详情与模拟 5 场跨月滚动，错误日志为空；生产构建与 ZIP 完整性/脚本 SHA-256 校验通过。全量测试 216/219 通过，另 3 项因默认超时失败，针对其所在两文件延长超时单独重跑 26/26 通过。

Current request (2026-09-25): 用最新 H5 覆盖发布 ZIP，并只保留一个正式包。

### 唯一正式发布包 — complete

- [x] 使用最新 `h5/` 覆盖 `release/篮球经理_联盟扩军时代.zip`，ZIP 中 `assets/game.js` 与构建产物 SHA-256 一致。
- [x] 删除重复的 `篮球经理_20260923球员名单更新.zip`；发布目录仅保留一个 ZIP，完整性校验通过。

Current request (2026-09-25): 交易报价与自由球员列表筛选统一为选秀池可用球员风格，并说明保障比例、合同选项和承诺角色对签约的影响。

### 球员列表统一筛选与报价因素说明 — complete

- [x] 抽取可复用的球员列表筛选组件，统一搜索、排序菜单和位置计数标签；排序菜单会按剩余视口空间自动向上或向下展开。
- [x] 接入扩军选秀池、交易报价列表和自由球员列表，保留交易薪资上限等页面特有条件。
- [x] 类型检查、33 项定向测试、全量 211 项测试和生产构建通过；390px/360px 实测搜索、排序、位置筛选及空状态，控制台无错误。
- [x] 已按真实引擎公式核对保障比例、球队/球员选项和承诺角色对签约 Utility 与后续合同生命周期的影响。

Current request (2026-09-25): 自由球员报价支持球队选项和球员选项。

### 自由球员报价末年选项 — complete

- [x] 报价弹窗支持无选项、球队选项、球员选项；选项仅能设置在至少 2 年合同的最后一年。
- [x] 报价、RFA 匹配与最终签约完整保存 `option_by_year`，并复用现有 Option Phase 决策。
- [x] 球队选项年不计入保障金额；球员选项按普通合同保障比例处理。
- [x] 完成领域测试、生产构建与 390px/360px 浏览器回归：全量 211 个测试通过，生产构建通过，浏览器控制台无报错。

Current request (2026-09-25): 增加自由市场前资质报价决策与 UFA/RFA 标识；报价改为首年薪资加涨幅，后续年度自动计算。

### RFA 资质报价与自动涨幅报价 — complete

- [x] 新增本队 RFA 资质报价指令：提交 QO 保留 RFA 匹配权，不提交则转为 UFA；未处理完不得进入自由市场。
- [x] AI 球队按公开球员价值确定性处理 QO；RFA Cap Hold 按冻结规则改为 previous salary 150% 与 QO 的较高值。
- [x] 自由球员卡片明确显示 UFA/RFA，并为 RFA 增加暖色识别。
- [x] 报价弹窗改为首年薪资 + 年限 + 涨幅；后续年度薪资自动生成，外队硬上限 5%，原球队权利续约硬上限 8%。
- [x] TypeScript、自由市场/合同生命周期 20 项定向测试和全量 210 项测试通过。
- [x] 生产构建与 H5 同步完成；390px/360px 实测 UFA/RFA 卡片、自动涨幅报价弹窗与底部操作布局无截断，控制台无错误。

Current request (2026-09-25): 统一自由市场状态字号，阻止同一市场窗口内被拒报价后重复报价，并放大底部操作按钮文字。

### 自由市场报价状态与按钮可读性 — complete

- [x] 所有卡片市场状态统一为 8.5px / 700 字重，不再让截止日小于“等待首份报价”。
- [x] 卡片识别当前市场窗口内已提交的待定、拒绝和到期报价；拒绝/到期后显示明确结果并禁用重复报价，领域层同步拦截绕过 UI 的重复提交。
- [x] 底部“结算今日”和“结束市场”主文案提高到 11px，360px 下结束市场按钮保持 10px。
- [x] TypeScript、18 项自由市场定向测试、生产构建和 `git diff --check` 通过；390px/360px 实测杜伦报价、逐日结算、统一状态字号和底部按钮布局，文案无截断且控制台无错误。

Current request (2026-09-25): 自由球员卡片将建议年薪与年限放在年龄后，并让市场状态独占下一行。

### 自由球员卡片合同与状态重排 — complete

- [x] 将球队、年龄、建议年薪/报价首年和年限合并为同一行，合同信息紧跟年龄。
- [x] 将“等待首份报价”及“等待决定 · 签约意愿 · 截止日”等市场状态移动到独立下一行。
- [x] TypeScript、17 项自由市场定向测试和生产构建通过；390px/360px 实测合同年限完整显示，长状态文案独占下一行且无截断，控制台无错误。

Current request (2026-09-24): 将自由市场“联盟动态”移动到名单规则下方、自由球员列表上方。

### 联盟动态位置调整 — complete

- [x] 将联盟公共事件流移到名单规则和自由球员列表之间，继续展示最近 5 条动态。
- [x] 现有动态覆盖市场开启、球员签约、RFA 匹配/放弃匹配与玩家报价因名单已满失效；TypeScript、14 项自由市场测试和 390px 浏览器检查通过，页面顺序与控制台均正常。

Current request (2026-09-24): 放大自由球员卡片中的“等待首份报价”状态文字。

### 等待首份报价字号 — complete

- [x] 仅将“等待首份报价”从 7px 提升至 8.5px，并适当增强字重和对比度；截止日、签约意愿与限制原因保持原字号。
- [x] TypeScript 通过；390px 浏览器实测薪资与状态仍在同一行，OVR 和报价按钮未受挤压，结算后截止日恢复 7px，控制台无错误。

Current request (2026-09-24): 自由市场底部并排放置“结算今日”和“结束市场”，自由球员默认按 OVR 排序并增加薪资/年龄排序，同时适当缩小列表 OVR。

### 自由市场底部操作与列表排序 — complete

- [x] 将“结算今日”移入底部操作栏，与“结束市场，进入季前调整”并排显示；RFA 待决定时保留匹配/放弃处理区并禁用底部推进。
- [x] 自由球员默认按 OVR 从高到低，增加建议年薪升/降序与年龄从小到大排序，排序价格统一使用建议首年年薪。
- [x] 列表 OVR 调整为 20px、窄屏 18px；TypeScript、29 项定向测试和生产构建通过，390px 浏览器实测四种排序、底部结算推进、报价弹窗均正常，控制台无错误。

Current request (2026-09-24): 自由球员卡片放大 OVR 并移到信息右侧，其余信息压缩为两行。

### 自由球员卡片信息重排 — complete

- [x] 将卡片重排为球员信息、大号 OVR、报价按钮三栏。
- [x] 球队与年龄归并为第一行，建议年薪/报价与市场状态归并为第二行。
- [x] TypeScript、26 项定向测试与生产构建通过；390px 窄屏实测确认两行信息、大号 OVR、详情入口和报价弹窗正常，控制台无错误。

Current request (2026-09-24): 报价弹窗金额超过一亿美元时改用“亿美元”，提交按钮统一为“提交报价”。

### 报价金额单位与按钮文案 — complete

- [x] 新增按金额量级自动切换“万美元 / 亿美元”的报价展示格式。
- [x] 报价弹窗的期望薪资、合同汇总使用自适应单位，输入框继续使用万美元。
- [x] 提交按钮统一为“提交报价”；17 项相关测试、TypeScript 与生产构建通过，浏览器实测显示合同总额 1.72 亿美元、保障金额 1.46 亿美元，控制台无错误。

Current request (2026-09-24): 球员个人评分文案统一为 OVR，并移除能力值颜色发光效果。

### 球员 OVR 文案与纯色显示 — complete

- [x] 将球员个人总评相关的“能力 / 综合”统一为“OVR”，保留训练专项与球队战力等不同语义文案。
- [x] 移除 OVR 数字、徽章和属性条的发光阴影，仅保留暖冷纯色。
- [x] TypeScript、26 项定向测试与生产构建通过；Playwright 及浏览器实测确认阵容、自由市场和球员详情均显示 OVR，纯色无光晕，控制台无错误。

Current request (2026-09-24): 球员卡片能力值按暖色高分、冷色低分统一优化。

### 球员能力值暖冷色阶 — complete

- [x] 新增统一能力值颜色映射及边界测试；60 分以下从冰蓝平滑过渡至深蓝。
- [x] 将映射接入球员详情属性、阵容、自由市场、扩军选秀与交易球员卡片。
- [x] TypeScript 与生产构建通过；全量 Vitest 39 个文件、202 项测试通过；Playwright 与浏览器实测确认高分暖色、低分冷色和属性条显示正常，控制台无错误。

Current request (2026-09-21): 选秀、扩军交易和扩军选秀页面未铺满；扩军选秀当前阵容需要位置筛选；新秀选秀在第34位灰熊处卡住。

### 选秀页面宽度、阵容筛选与第34顺位卡死 — complete

- [x] 选秀准备/进行中、扩军交易、扩军选秀、选秀后、自由市场和季前赛终端移除旧的 440/472px 内容限宽，铺满手机版可用宽度并保留内部滚动。
- [x] 扩军选秀当前球队阵容新增全部、PG、SG、SF、PF、C 第一位置筛选。
- [x] 修复 AI 球队选秀签约时已满 21 人导致事务失败的问题：AI 会在签新秀前按“非新秀优先、公开球员价值最低”清理一个名额，并计入应有保障工资死钱；用户球队仍保留手动裁员权。
- [x] 新增真实 2026 第34顺位孟菲斯推进及快进回归测试。

Current request (2026-09-21): 扩军选秀大会的位置筛选只按第一位置归类。

### 扩军选秀第一位置筛选 — complete

- [x] 位置筛选、位置人数统计和选人后自动跳队统一只匹配 `player.position`；如 `SG/PG` 只在 SG 下显示，不再重复进入 PG。
- [x] 新增双位置球员回归用例，并将原有筛选断言更新为第一位置口径。
- [x] 扩军选秀相关 21 项测试、TypeScript 检查、生产构建和 ZIP 完整性检查通过；已覆盖正式包，SHA-256 为 `89963d3e9a8b2acbd6f2a226cf8bbb0a588610a874e6eb78d45eb9249e748c10`。

Current request (2026-09-21): 所有弹框都要居中显示。

### 全局弹框居中 — complete

- [x] 事件、比赛详情、球员详情、球队选择、裁员确认、存读档、存档冲突与交易中心统一为视口水平/垂直居中卡片。
- [x] 弹框统一采用接近满宽的 430px 上限、8px 横向安全间距、完整边框与四角圆角；高度受可用视口及上下安全区限制。
- [x] 赛后数据中心继续保持独立全屏页面，不纳入弹框规则。
- [x] TypeScript 检查、生产构建、`git diff --check` 与弹框/存档相关 18 项测试通过；已覆盖正式 ZIP，79 个条目且 `unzip -t` 通过，SHA-256 为 `8e18f733de86f938f719a6fd875556a3815a1215f65f182019195040cfcd5958`。

Current request (2026-09-21): 居中的存档/读档弹框宽度偏窄。

### 存读档弹框宽度 — complete

- [x] 居中存读档弹框最大宽度由 390px 放宽至 430px，手机左右留白由 18px 收紧为 8px，并继续兼容横向安全区。
- [x] 生产构建与 ZIP 完整性检查通过；已覆盖 `release/篮球经理_联盟扩军时代.zip`，SHA-256 为 `861a4518bf6f543ff493d4193f92e40363c76c0d06ceb4d4f3c57b95e85634f7`。

Current request (2026-09-21): 存档和读档保持居中，修复首页版本文案与“继续上次进度”重叠。

### 存读档居中与首页版本文案 — complete

- [x] 首页读档、新开档槽位、游戏内存读档及存档冲突恢复为带安全边距的居中卡片；其他详情弹层继续使用全宽底部抽屉。
- [x] 首页版本文案移出绝对定位，按正常布局排列在操作按钮下方；短屏设备允许首页纵向滚动，避免内容重叠或裁切。
- [x] TypeScript 检查、Vite 生产构建、`git diff --check` 及存档/界面相关 14 项测试通过。
- [x] 已覆盖正式包 `release/篮球经理_联盟扩军时代.zip`；79 个条目、根目录为 `index.html`、`unzip -t` 通过，大小 7,867,941 bytes，SHA-256 为 `ccb5d7a1da41ab89478781e7c011d6c451bf5a8a2cfc2df84775595ebbae4d4f`。

Current request (2026-09-21): 产品仅提供手机版，将所有页面统一优化为全宽移动端布局。

### 全页面移动端宽度统一 — complete

- [x] 首页、序章、扩军流程、2026 新秀选秀、自由市场、季前赛、常规赛与赛后页统一为 `100%` 设备宽度和 `100dvh` 视口高度，移除最终生效的 390/430/440/472/520px 页面限宽。
- [x] 全局内容统一采用 12px 左右安全边距，并让顶栏、底部五项导航适配横向安全区和刘海屏。
- [x] 首页存档选择、事件、比赛详情、球员详情、存读档、球队选择、裁员确认与交易中心统一为全宽底部抽屉；赛后技术统计继续在表格内部横向滚动。
- [x] TypeScript 检查、Vite 生产构建与 `git diff --check` 通过；全量 Vitest 135/136 通过，唯一失败为既有 `career.test.ts` 完整赛季模拟超过 30 秒。
- [x] 已尝试启动本地预览进行 320/390/430px 截图验收，但当前沙箱禁止监听本地端口，权限审批服务又返回 503；因此本轮未完成真实浏览器截图，不误报视觉验证通过。
- [x] 已重新构建并覆盖唯一正式包 `release/篮球经理_联盟扩军时代.zip`；ZIP 根目录直接包含 `index.html`，共 79 个条目，`unzip -t` 通过，大小 7,867,835 bytes，SHA-256 为 `9bd654a50b6f78dc2861c9e84b7f5758dbf89f5f80e5f6b2ee0db901a4b2b2af`。

Current request (2026-09-20): 扩军选秀薪资增加工资帽/奢侈税线/第一土豪线/第二土豪线进度条；选秀池支持位置筛选；选人后自动跳到下一支可选球队并跳过不可选球队。

### 扩军选秀薪资与选人效率 — complete

- [x] 薪资卡接入 `CapSheetService.total`，新增由联盟财务配置驱动的四阈值进度条与当前档位。
- [x] 可用球员增加全部、PG、SG、SF、PF、C 筛选，主位置和副位置均参与匹配。
- [x] 自动跳队规则按球队顺序跳过无可选球员球队；当前筛选位置有候选时优先跳到对应球队，无对应位置时回退到下一支有任意可选球员的球队。
- [x] 指令回调返回提交结果，仅在扩军选择原子保存成功后切换球队。
- [x] 新增视图规则测试 3 项；扩军领域相关测试合计 15 项通过，TypeScript 检查通过。
- [x] 全量 Vitest 35 个文件、127 项测试全部通过；TypeScript/Vite 生产构建通过并同步静态 `h5` 产物。
- [x] 已尝试按 `develop-web-game` 流程启动本地页面与 Playwright 截图；开发服务器可用，但 Chromium 被 macOS Mach-port 权限拒绝，备用浏览器控制又缺少 Codex 授权令牌，因此本轮无法完成 390×844 截图与真实点击验证，未将其误报为通过。

Current request (2026-09-20): 游戏内存/读档界面与首页读取存档样式保持一致。

### 游戏内存读档样式统一 — complete

- [x] 游戏内存/读档抽屉改为与首页读取存档一致的居中深色卡片、青色强调、槽位信息卡和主操作按钮风格；保留保存/读取页签及原有功能。
- [x] TypeScript/Vite 生产构建通过，静态 `h5` 已同步；Playwright 受 macOS Mach-port 权限限制无法启动 Chromium 截图。

Current request (2026-09-20): 打包包含首页、存档读取和合同数据更新的新版本。

- [x] 已生成且仅保留 `release/篮球经理_联盟扩军时代.zip`；后续 ZIP 统一且仅使用此文件名，不附带版本号或说明。ZIP 根目录直接包含 `index.html`，`unzip -t` 75 个条目完整性校验通过，SHA-256 为 `92f4924706eb12ab28972a2c3bbe0483be6f82a18520b622e61dcdff4a1fd83f`。

Current request (2026-09-20): 首页“读取存档”应停留在首页，不能先进入新游戏的建队页面。

### 首页读取存档 — complete

- [x] 首页“读取存档”改为原地打开存档选择层，不再创建或显示建队流程作为中间页面。
- [x] 选择层列出三个槽位的球队名、赛季、日期、战绩和阶段；仅点击已有存档的“读取并继续”才进入该存档，空槽位不可点击。
- [x] TypeScript/Vite 生产构建通过，静态 `h5` 已同步；Playwright 受 macOS Mach-port 权限限制无法启动 Chromium 截图。

Current request (2026-09-20): 首页改名为“篮球经理：联盟扩军时代”，并将操作指引改为游戏简介。

### 首页游戏简介文案 — complete

- [x] 首页标题更新为“篮球经理：联盟扩军时代”。
- [x] “开始前说明”调整为“游戏简介”，说明扩军起点、球队经营、选秀/交易/自由市场/赛季和联盟历史等核心体验，不再以操作步骤为主。
- [x] TypeScript/Vite 生产构建通过，静态 `h5` 已同步；Playwright 受 macOS Mach-port 权限限制无法启动 Chromium 截图。

Current request (2026-09-20): 首页移除 “SYSTEM EXPANSION_ERA” 英文标识，并补充简洁游戏说明。

### 首页文案与新手说明 — complete

- [x] 移除首页背景上的 `SYSTEM // EXPANSION_ERA` 英文伪元素标识。
- [x] 首页新增“开始前说明”：说明扩军开局流程、赛季页/管理/市场的主要用途，以及自动保存和手动存读档入口。
- [x] 调整首页说明与主操作按钮的垂直间距，保持手机竖屏首屏可读；TypeScript/Vite 生产构建通过，静态 `h5` 已同步。Playwright 仍受当前 macOS Mach-port 权限限制，无法启动 Chromium 完成截图检查。

Current request (2026-09-20): 存档读取页应展示可区分存档的关键信息；“继续上次进度”必须恢复最近一次保存的实际进度。

### 存档识别与最近进度恢复 — complete

- [x] 存档服务新增槽位摘要：球队名称、赛季、游戏内日期、战绩、阶段、保存时间与修订号；读取页不再用当前页面信息伪装其他槽位。
- [x] 读取页每个槽位显示真实球队名、赛季日期、战绩和阶段；空槽位明确标记为“空存档”，顶部提供“继续最近进度”。
- [x] 启动页“继续上次进度”会按保存时间/修订号选择最新槽位并直接恢复，同时恢复该槽位为当前操作槽；没有存档时留在启动页并给出提示。
- [x] 存档服务新增回归测试覆盖槽位摘要和最近存档恢复；针对该用例通过，TypeScript/Vite 生产构建通过，静态 `h5` 已同步更新。Playwright 在此受限桌面环境启动 Chromium 时被 macOS Mach-port 权限拒绝，无法生成本轮截图。

Current request (2026-09-20): 将所有已存在本地 2K 评分、但此前未进入能力投影层的薪资球员接入游戏，并解释剩余未识别项。

### 2K 评分补全与薪资匹配 — complete

- [x] 将 Felix Okpara、Tamar Bates、Jaron Pierre Jr. 的完整 2K 评分写入可被游戏初始化读取的补充能力数据，并重新生成合同快照。
- [x] 进一步核对四个未识别项后，确认 Labaron Philon、John Tonje、Taj Gibson、Kasparas Jakučionis 均有本地 2K 完整资料；同样已补入能力数据并添加确定性薪资映射。
- [x] 合同快照从 436 份增至 443 份，未匹配来源球员从 11 人降为 4 人；泰·吉布森的名单快照缺席场景允许由已导入合同的球队信息兜底归队。
- [x] 新增回归测试覆盖七名补充资料的合同和可用能力档案；20 项定向测试及 TypeScript/Vite 生产构建通过，静态 `h5` 已同步更新。

Current request (2026-09-20): 当前比赛日应显示对手名而非“当前”；未来选择日期使用不同于当前日期的高亮；单日与单场模拟更快，单场不超过约一秒。

- [x] 当前比赛日保留青色高亮；若当天有比赛，日期卡显示对手队名，不再显示“当前”文字。
- [x] 未来选择目标日改为紫色高亮，与当前日的青色高亮清晰区分；选择当前日不会覆盖当前高亮。
- [x] 单日模拟帧间隔降至 160ms；单场模拟按帧数动态加速，完整逐日过程最多约 0.9 秒。
- [x] TypeScript、Vite 生产构建和静态 `h5` 产物更新通过。浏览器自动化无法再次打开本地 `file://` 页面（工具安全策略拒绝），保留上一轮 390×844 已验证的日历布局检查。

Current request (2026-09-20): 日历应区分已度过、当前和未来日期；明确四种模拟操作；选中日期后才可模拟至该节点；逐日播放模拟过程；已结束比赛日打开战报。

### Calendar state and simulation controls — complete

- [x] 已度过日期统一灰显，当前日期以青色描边和脉冲高亮；日历自动横向定位当前日期，未来日期保持正常可选。
- [x] `1 天` 逐自然日模拟一天，`1 场` 模拟至下一场本队比赛，`5 场` 快进五场本队比赛；三个流程都复用逐日高亮动画。
- [x] `至节点` 改为仅在日历选中今天/未来日期后开放，并逐自然日模拟至该日期；完成后清空目标选择并刷新当前日期。
- [x] 点击已经结束的比赛日会直接打开赛后数据中心；已通过 Chrome 交互验证 10/20 战报打开/返回，以及选中 10/23 后三日、一场的目标模拟。
- [x] TypeScript、Vite 生产构建与静态 `h5` 产物更新通过；390×844 截图确认当前日期居中可见、过去日期灰显。

Current request (2026-09-20): 扩军选秀中，球队被保护球员默认收起。

- [x] 移除保护名单 `<details>` 的初始展开属性；玩家仍可点击摘要展开或收起。
- [x] TypeScript 与 Vite 生产构建通过，静态 `h5` 产物已同步更新。

Current request (2026-09-20): 赛后数据中心不能以下拉弹框承载；应打开可完整滚动的页面、顶部提供返回按钮，且 MVP 长姓名缩小但不换行。

### Postgame data center full-page flow — complete

- [x] 移除赛后数据中心的遮罩弹框与固定卡片高度，改为覆盖视口的独立页面；顶部“← 返回”会回到原赛季页。
- [x] 页面正文改为独立纵向滚动区，底部采用 `env(safe-area-inset-bottom)` 留白，避免最后一行被 iPhone Home Indicator 遮挡。
- [x] MVP 姓名新增按容器可用宽度实时缩小（15px 至 8px）的适配，固定单行显示。
- [x] 在 Chrome 390×844 实测：技术统计区 `scrollHeight 1285px`、`clientHeight 770px`，End 键到达 `scrollTop 515px` 且剩余 `0px`；返回按钮移除完整页并恢复赛季中心。MVP 计算样式为 `white-space: nowrap`。
- [x] TypeScript 与 Vite 生产构建通过。

Current request (2026-09-20): 以 `gemini-code-1789868438083.html` 为参考，将常规赛“赛季”页按其赛博日历、对阵、模拟与双栏资讯结构重建；保留现有真实比赛详情，未接入的战术入口必须禁用。

### Season-page demo reconstruction — in progress

- [x] 已读取参考 HTML 与当前赛季页的数据/命令边界；参考图像因本机 `file://` 浏览器安全策略不可自动截屏，改以用户提供 HTML 的原始结构和样式为视觉基准。
- [x] 已将真实赛程转换为单行横向日期卡，接入单日、单场、五场和下一事件模拟。
- [x] 已将真实的下一场、近赛果、伤病与球员赛季表现映射到参考稿卡片；“查看战报”保留并打开真实赛后数据中心。
- [x] 已在 Chrome 390×844 实测赛季首屏、模拟一场并打开真实技术统计；未发现控制台错误。TypeScript 与 Vite 生产构建通过。
- [ ] 全量 Vitest 119 项中 118 通过；既有 `career.test.ts` 完整赛季模拟在 30 秒阈值超时，未由本次 UI 变更引入。

Current request (2026-09-18): 以 `gemini-code-1789729220723.html` 为视觉参考，将现有已实现能力接入固定五项导航（赛季、管理、市场、联盟、生涯）；未实现能力保留禁用入口；保留比赛详情/赛后数据中心。

### V1.52 — 五项固定导航与赛博终端重构（进行中）

- [x] 常规赛主界面改为固定 `赛季｜管理｜市场｜联盟｜生涯`：阵容、合同、培养、资产归入管理；交易、报价与交易记录归入市场。
- [x] 现有赛程日历、单日/五场/事件模拟、球队适配度、交易询价、联盟排名、历史和成就均沿用真实状态与命令；未具备的 Trade Block、编辑训练与跨阶段市场入口明确禁用。
- [x] 保留从赛程/最近赛果打开的赛后数据中心；本地 Playwright 截图验证弹层含真实比分、分节、MVP 与技术统计。
- [x] 常规赛五个一级页及休赛期自由市场均已截图核验；底部安全区导航常驻，阶段无关入口可见但禁用。
- [x] TypeScript、Vite 生产构建、全量 Vitest（34 文件 / 116 测试）通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.52_五导航赛博终端版.zip`；ZIP 根目录直接包含 `index.html`，`unzip -t` 完整性校验通过，SHA-256 为 `fea31a267043081e294c52fd6eb2b603e4b2dbfc30562c9f75c7c2dd80e50ca6`。

Current request (2026-09-18): 比赛详情的赛后数据中心采用 `gemini-code-1789722755611.html` 的 UI。

Current request (2026-09-18): 修复部分球员头像为黑色、不可见的问题。

Current request (2026-09-18): 赛后数据中心的长球员名应自动缩小字号并始终单行显示。

Current request (2026-09-18): 修复“模拟下一日”无明显反馈，以及五场逐日动画出现两个日历高亮框的问题。

### V1.51 — 日历单日推进与动画焦点修复

- [x] “模拟下一日”改为专用流程：结算后自动切换到当前日期所在月份、定位下一场未赛的本队比赛，并明确显示已推进到的自然日期。
- [x] 五场逐日动画期间取消旧的手动选中态，仅保留当前自然日的“模拟中”动画框；动画结束后再同步定位下一场未赛比赛。
- [x] TypeScript、Vite 生产构建和三赛季无头生涯回归通过（每季 1,312 场）。
- [x] 已生成 `release/篮球经理_扩军时代_V1.51_日历模拟修复版.zip`，压缩包完整性校验通过，SHA-256 为 `8fdad1841afd3a83124b7675918c8d9329fa788b90c1c2092b05f7ef7a8dd2b4`。

### V1.50 — 赛后技术统计长姓名单行适配

- [x] 技术统计表的球员姓名按首列实时可用宽度计算字号；短名字保留 12px，长名字最低缩小至 8px，始终与位置标签保持同一行。
- [x] 使用 `ResizeObserver` 监听页面宽度变化，横竖屏或窗口尺寸变化后会再次适配，不会截断为两行。
- [x] TypeScript 与 Vite 生产构建通过；全量测试 115/116 通过，唯一失败仍是既有的完整赛季模拟 30 秒超时（与本次 UI 无关）。
- [x] 已生成 `release/篮球经理_扩军时代_V1.50_赛后长姓名单行版.zip`，压缩包完整性校验通过，SHA-256 为 `17b445792f56ad17130abf298ae7f53542eef1dace7af5e2c3e94dbe067d90ed`。

### V1.49 — 球员头像可见性兜底

- [x] 图集头像新增逐格亮度检测：黑色/异常格和加载失败会自动切换为高可见青色人物占位图。
- [x] 没有本地图集映射的程序化球员同样展示占位头像，球员详情不再留下黑色空块。
- [x] TypeScript、图集映射测试 2/2、Vite 生产构建通过；官方 Playwright 打开季前球员卡，截图确认占位头像正常显示。
- [x] 已生成 `release/篮球经理_扩军时代_V1.49_头像可见性修复版.zip`，75 个条目完整性校验通过，SHA-256 为 `f677f0f3121b09e3b3b7130f908280db75a60e00b739c55a77bbdedb28edcec0`。

Current request (2026-09-18): 读取存档成功后应关闭读档页面。

### V1.48 — 读档后自动收起

- [x] 读档回调改为显式返回成功状态；只有成功载入存档才关闭抽屉，空槽位、数据版本不兼容或读取失败会保留页面并显示原因。
- [x] 成功读档先清除启动时的自动读档标记，再更新游戏阶段；即使存档切换至不同流程，重建页面后也不会重新弹出读档抽屉。
- [x] TypeScript、Vite 生产构建通过；官方 Playwright 截图确认存/读档抽屉正常打开和渲染。
- [x] 已生成 `release/篮球经理_扩军时代_V1.48_读档自动关闭版.zip`，75 个条目完整性校验通过，SHA-256 为 `f6507dea2dd0cf71f2495c661ea1b158ceed7ea52dbfc2a48d9ec535bd3ce93b`。

Current request (2026-09-18): 赛后数据中心必须 100% 还原 `gemini-code-1789722755611.html` 的 UI 模板。

### V1.47 — 赛后数据中心模板级还原

- [x] 以模板 HTML 的原始布局与数值重建：网格背景、顶部 20px 起始位置、440px/20px 圆角容器、Header、三列比分卡、分节比分、MVP 金框与技术统计宽表。
- [x] 清除通用比赛弹层造成的继承冲突，卡片不再被压缩或互相遮挡；滚动内容维持模板的 16px 节奏。
- [x] 保留真实球队、真实比分和技术统计数据；项目未保存正负值时使用中性“—”，不伪造 +/-。
- [x] TypeScript、Vite 生产构建通过；官方 Playwright 截图与模板截图对照，视觉验收 95/100（差异仅为动态球队/赛果数据）。
- [x] 已生成 `release/篮球经理_扩军时代_V1.47_赛后数据中心模板还原版.zip`，75 个条目完整性校验通过，SHA-256 为 `a504fbece681f81e24e328a6187ecc5fbb83bdc17face07e0853e11cfcc29303`。

Current request (2026-09-18): 常规赛日历模拟应逐日推进，跨月自动翻页后继续，不能按本队比赛直接跳日。

### V1.46 — 逐日赛程推进

- [x] “模拟下一日”改用单日联盟模拟；休息日、所有联盟比赛日与本队比赛日都会按自然日期前进。
- [x] “逐日推进 5 场赛程”预先生成逐日动画帧，按日期播放并仅在本队比赛日结算战绩；每一帧自动切换日历到对应月份。
- [x] 日历在当前动画日期显示“休息日/模拟中”，跨月后自动显示新月份的日历，再继续推进。
- [x] TypeScript、Vite 生产构建通过；官方 Playwright 实测 13 个自然日动画帧、5 场目标赛程，中途截图验证休息日不改变战绩，跨月截图验证自动切换至 11 月且比赛日卡同步定位到 11 月 2 日。
- [x] 已生成 `release/篮球经理_扩军时代_V1.46_逐日赛程推进版.zip`，75 个条目完整性校验通过，SHA-256 为 `0a28df2d88f6dc12373715eccac452992b04d503c2af7f6a866edfdc7868c0de`。

### V1.45 — 赛后数据中心

- [x] 比赛详情重构为参考稿的赛后数据中心：比赛总览、胜负与加时状态、分节比分、本场 MVP、主客两队完整技术统计。
- [x] 技术统计直接读取比赛引擎的真实上场时间、得分、篮板、助攻、抢断、盖帽、命中率和失误；项目未保存正负值，因此不伪造 +/-。
- [x] 分节数据适配历史存档：旧赛果未保存时以“历史记录未保存”和“—”呈现；后续模拟生成的比赛显示真实分节得分。
- [x] 修正比分横幅主队与比赛状态占位重叠；统计表支持横向滚动，球员列保持固定。
- [x] TypeScript、Vite 生产构建通过；官方 Playwright 客户端截图验证比赛详情、比分横幅与技术统计表渲染。
- [x] 已生成 `release/篮球经理_扩军时代_V1.45_赛后数据中心版.zip`，75 个条目完整性校验通过，SHA-256 为 `3e380426b909a2ee9fd3dddaeed5b220e14dc417975edc6cd0b8c5977691af6c`。

Current request (2026-09-18): “推进 5 场赛程”采用 `gemini-code-1789720818415.html` 的逐日推进动画。

### V1.44 — 五场赛程逐日推进动画

- [x] 复用参考稿的 700ms 逐场节奏：当前日青色脉冲高亮，已完成日期即时标记真实胜负/比分，战绩与按钮进度同步更新。
- [x] 动画先在内存生成确定性的五场赛程预览，结束后才一次性原子保存；若中途触发强制事件，保持原有模拟停止规则。
- [x] 将动画进度暴露给 `render_game_to_text`，自动化测试可读取已完成场次、总场次与当前比赛 ID。
- [x] TypeScript 与生产构建通过；官方 Playwright 客户端截图验证“模拟中 3/5”中间帧，以及结束后战绩 4-2 和经理事件正确出现。
- [x] 已生成 `release/篮球经理_扩军时代_V1.44_五场赛程动画版.zip`，75 个条目完整性校验通过，SHA-256 为 `6b6b348d530ef98d3eda3a212a28adb542038988b8d460a15eca985f9fd5eb1d`。

Current request (2026-09-18): 常规赛中球员诉求事件应提供真正的经理决策，不应只显示“确认”。

### V1.43 — 球员诉求经理决策

- [x] 士气与角色事件从单一“确认”改为“回应诉求 · 提升角色”与“维持当前轮换”两种明确选择，分别结算球员士气 +12 / -8。
- [x] 单选事件在 UI 中明确标为“事件通知”，避免再将纯确认误称为“经理决策”。
- [x] 对已有存档中的旧单选士气/角色事件提供兼容升级：页面重载后立即显示两项选择，选择时按新效果结算并写回事件记录。
- [x] 事件服务测试 6/6、TypeScript 与 Vite 生产构建通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.43_经理决策版.zip`，75 个条目完整性校验通过，SHA-256 为 `349cc4391f9741785c836146184567ecb383ed3d3dc125b7b552f67a530ee34a`。

Current request (2026-09-18): 常规赛五个主页面采用 `gemini-code-1789720818415.html` 的赛博终端 UI，并确保现有功能完整覆盖。

### V1.42 — 常规赛终端化

- [x] 赛程、阵容、管理、联盟、生涯五个页面统一为参考稿的深色网格背景、青色描边卡片、橙色行动色与五栏底部导航。
- [x] 保留并复核全部既有功能：赛程切换与比赛详情、单日/五场/事件/赛季模拟、球员详情、交易询价与薪资资产、东西部排名及生涯成就。
- [x] 阵容页补齐常驻的市场评级、球队声望、球迷支持、自由球员吸引力四项经营指标；赛程页补回可点击打开比赛详情的最近赛果。
- [x] TypeScript 和 Vite 生产构建通过；官方 Playwright 客户端逐页切换、截图检查通过，推进 5 场赛程实测从 1-0 更新至 4-2 并产生正确的待处理事件。
- [x] 已生成 `release/篮球经理_扩军时代_V1.42_常规赛终端UI版.zip`；ZIP 根目录直接包含 `index.html`，75 个条目完整性校验通过，SHA-256 为 `1a352751816f0f1ccfa70edb1c8d3212710fad6ddeef7016c673a9fde23cf717`。

Current request (2026-09-18): 季前准备常规赛名单将能力值放在球员名后，年龄放在合同年限后。

### V1.41 — 季前名单信息顺序优化

- [x] 名单第一行调整为“球员名 · 能力值”，第二行调整为“年薪 · 合同年限 · 年龄”。
- [x] 移除右侧独立能力/年龄两列，缩短横向占用并保持整行可点击查看球员卡。
- [x] TypeScript 与名单服务测试 4/4 通过。
- [x] 官方 Playwright 客户端截图确认信息顺序、截断和操作区布局正常；点击名单行仍可打开球员信息卡，Console 无错误。
- [x] Vite 生产构建通过；已生成 `release/篮球经理_扩军时代_V1.41_季前名单排版版.zip`，75 个条目完整性校验通过，SHA-256 为 `f6de29d2cf527a0134e279492fb2b1fc7120cbac6bf18300c211601550cc5a12`。

Current request (2026-09-18): 移除右上角没有实际功能的“模式”占位。

### V1.40 — 顶栏无效入口精简

- [x] 确认“模式”仅为无点击事件、无状态绑定的静态标签。
- [x] 从所有流程共用的 `GameChrome` 顶栏移除“模式”及其未使用图标，只保留有效的“存/读档”入口。
- [x] TypeScript、名单相关测试 12/12 与 Vite 生产构建通过。
- [x] 官方 Playwright 客户端截图核验右上角只保留“存/读档”，顶栏对齐正常且 Console 无错误。
- [x] 已生成 `release/篮球经理_扩军时代_V1.40_顶栏精简版.zip`；75 个条目完整性校验通过，SHA-256 为 `7d9d11b50051c0394f806fcbc1092731c27cc275473a5f8132fdef6868587898`。

Current request (2026-09-18): 修复里夫斯被错误放入自由市场；季前名单显示能力与年龄，点击展示球员卡，裁员增加二次确认。

### V1.39 — 现役合同归属与季前名单交互

- [x] 定位里夫斯误入自由市场的根因：官方名单快照归属湖人，但离线合同构造器无来源地随机生成了球员选项，扩军选项阶段又将其判定为拒绝执行。
- [x] 离线现役名单不再伪造球员/球队选项；现役快照球员保持 `STANDARD` 合同与当前球队归属，真实合同选项仍由带薪资明细的数据源提供。里夫斯按 NBA/Lakers 2026 年 7 月官方续签信息同步为 4 年、总保障 1.85 亿美元。
- [x] 季前常规赛名单新增能力值与年龄；球员主体可点击打开现有完整信息卡。
- [x] 裁员改为二次确认，明确展示裁员后名单人数、自由球员状态与剩余保障死钱；取消不产生任何 Command，确认仍通过原子名单 Command。
- [x] 官方 Playwright 客户端截图核验能力/年龄、完整球员卡和裁员确认弹层；浏览器实测取消后保持 15 人，确认后降至 14 人且被裁球员消失，Console 无错误。
- [x] TypeScript、Vite 生产构建、合同/名单/自由市场相关测试 17/17 与 Stage 4 固定种子 10/10 通过。
- [x] 全量测试 113/114 首轮通过；唯一失败为 30 秒季节模拟超时，使用 60 秒阈值单独复跑 4/4 通过。
- [x] 最终 `file://` 离线 H5 已直开截图验证，未产生 Console 错误。
- [x] 已生成 `release/篮球经理_扩军时代_V1.39_季前名单与合同修正版.zip`；ZIP 根目录直接包含 `index.html`，75 个条目完整性校验通过，SHA-256 为 `2624cf36ba00e616dac3eb28ac5cd2e347e439fd581f0150ee89af177a4c768a`。

Current request (2026-09-18): 选秀后休赛期的球员交易中心采用 `gemini-code-1789716714697.html` 的赛博询价弹层 UI。

### V1.38 — 休赛期交易中心询价终端

- [x] 交易中心展开后改为 440px 遮罩弹层，恢复参考稿的关闭按钮、标题说明、位置标签、排序/薪资筛选、筹码栏、报价滚动区与底部主操作。
- [x] 新增按主/副位置、薪资区间筛选，以及适配度、薪资、综合能力变化排序；筛选只影响展示，不增加询价 Counter。
- [x] 保留原有生成、刷新和接受交易 Command；交易成功后若原筹码已离队，立即隐藏过期报价，避免重复点击。
- [x] 浏览器实测生成 3 个动态报价、PG 筛选收窄至 1/3、刷新产生新报价、接受交易、关闭弹层；Console 无错误或警告。
- [x] TypeScript、Vite 生产构建、交易服务 5/5 测试及 Stage 4 10/10 固定种子回归通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.38_休赛期交易终端版.zip`；ZIP 根目录直接包含 `index.html`，75 个条目完整性校验通过，SHA-256 为 `4992b3d67b99549ff16182ca92740a4c4609272e3d57565970e1811c5a4a5106`。

Current request (2026-09-18): 新秀选秀页面还原为 `gemini-code-1789705799201.html` 的紧凑赛博终端 UI，同时保留逐签模拟与下一签快进。

### V1.37 — 新秀选秀参考稿还原

- [x] 按参考稿恢复顶部控制栏、橙色行动状态条、紧凑双仪表、内联规则说明、榜单标题与无外框滚动球员列表。
- [x] 修正旧主题对新秀编号、评级背景、卡片高度和按钮尺寸的样式串扰；编号恢复青色，评级区保持透明。
- [x] 将“下一签快进”收进顶部控制栏；玩家签位时隐藏次要操作，使页面结构与参考稿一致，继续/暂停/快进逻辑保持不变。
- [x] 浏览器实测快进停在 #6、玩家选人、继续至 #10、暂停并快进至 #38；43 人可选、已完成 37/64，Console 无错误或警告。
- [x] TypeScript、Vite 生产构建、选秀服务 8/8 测试及 Stage 4 10/10 固定种子回归通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.37_选秀参考稿还原版.zip`；ZIP 根目录直接包含 `index.html`，75 个条目完整性校验通过，SHA-256 为 `667767dbe4f9da1364f6ea4a5f6bb63b6d846f77a3ad9599a28018e6e114a1f6`。

Current request (2026-09-18): 选秀榜单采用参考图的紧凑赛博列表；玩家完成选择后既可继续自动模拟，也可快速跳到自己的下一个签位。

### V1.36 — 选秀榜单与下一签快进

- [x] 选秀榜单改为青色描边容器、深色紧凑候选行、排名方块、位置/年龄/即战力信息、球探评级与发光“选择”按钮。
- [x] 玩家选秀完成后恢复“继续自动模拟”入口，可随时暂停；逐签选择仍通过独立命令写入存档。
- [x] 新增“快速模拟”入口，自动逐签结算并停在玩家的下一个签位；没有后续玩家签位时可继续模拟至选秀结束。
- [x] 浏览器实测从第 #6 顺位选人后继续模拟至第 #13 顺位，再快进并准确停在第 #38 顺位；Console 无错误或警告。
- [x] TypeScript、Vite 生产构建及 Stage 4 10/10 固定种子回归通过；全量测试 112/113 通过，唯一超时的赛季模拟用例单独复跑 4/4 通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.36_选秀快进版.zip`；ZIP 根目录直接包含 `index.html`，75 个条目完整性校验通过，SHA-256 为 `47c4c65e48a4fa3a78c6084130edb503bacbdd93e91a703f09a0c636d0e76c7c`。

Current request (2026-09-18): 选秀后、自由市场和季前准备页面分别采用 `gemini-code-1789713114729.html`、`gemini-code-1789713127703.html`、`gemini-code-1789713133719.html` 的赛博终端 UI。

### V1.35 — Stage 4 休赛期三页面终端化

- [x] 选秀后页面改为完成状态 Banner、本队新秀列表、2×2 薪资概览、阶段提示与底部“进入自由市场”操作栏。
- [x] 自由市场改为日期/人数顶栏、三项仪表、每日结算控制、可滚动自由球员卡片、联盟动态与折叠交易中心；报价、撤回、RFA 和市场结算逻辑保持不变。
- [x] 季前准备改为培养/名单仪表、培养说明、可滚动名单卡片、训练下拉与裁员操作，并保留 14～15 人名单锁定规则。
- [x] 浏览器逐页截图核验完成；选秀后进入市场、市场日推进、结束市场进入季前、训练选择和锁定名单进入常规赛均已走通。
- [x] 34 个测试文件 / 113 项测试、Vite 生产构建与 Stage 4 10/10 固定种子全流程回归通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.35_休赛期终端UI版.zip`；ZIP 根目录直接包含 `index.html`，75 个条目完整性校验通过，大小 6,969,040 bytes，SHA-256 为 `80bb5001ea8084b0e59c0fcd5b87cc6b54f83e1b6b3478d34b130ba89c1fdb37`；生产 H5 已通过 `file://` 离线直开截图验证。

Current request (2026-09-18): 新秀选秀大会改为参考 `gemini-code-1789699632980.html` 的动态逐签模拟，支持开始、暂停、继续，并在玩家顺位停下。

### V1.34 — 新秀选秀逐签动态模拟

- [x] 新秀选秀不再一次性跳过全部 AI 顺位；每个 AI 选择通过独立可重放命令写入状态，并继续沿用固定种子选人逻辑。
- [x] 选秀大厅新增开始、暂停和继续控制；AI 每签先展示实时播报，再以 400ms 卡片退场动画推进到下一顺位。
- [x] 模拟到达玩家球队时自动停止，显示行动提示并启用球员选择；玩家选择后可继续模拟后续 AI 顺位。
- [x] 浏览器实测暂停不推进、恢复后继续，并在第 #6 顺位自动停止；Console 无错误或警告。
- [x] 34 个测试文件 / 113 项测试、Vite 生产构建及 Stage 4 10/10 固定种子全流程回归通过。

Current request (2026-09-18): 修复扩军选秀大会中“球队被保护球员”展开后名单显示不全的问题。

### 扩军保护名单完整展开修复

- [x] 移除最终主题样式重新引入的 `116px` 高度上限，展开后的保护名单由页面整体滚动承载，不再产生截断或嵌套滚动。
- [x] 浏览器实测收起再展开后显示 8/8 名保护球员；保护网格 `clientHeight` 与 `scrollHeight` 均为 189px，`max-height: none`、`overflow-y: visible`，Console 无新增错误。
- [x] 34 个测试文件 / 112 项测试、Vite 生产构建及扩军流程 10/10 固定种子回归通过。

Current request (2026-09-18): 头像改为按 NBA Player ID 下载 NBA 官方真人头像；离线 H5 保持不依赖服务，并以已有 2K 本地图片兜底。

Current request (2026-09-18): 新秀选秀准备使用 `gemini-code-1789697475846.html` 的赛博终端数据概览 UI；实际选秀大厅使用 `gemini-code-1789699632980.html` 的 LIVE 选秀终端 UI。

Current request (2026-09-18): 球员详情卡采用 `gemini-code-1789704436192.html` 的赛博终端卡片 UI。

Current request (2026-09-18): 扩军选秀大会的当前球队阵容在收起时显示 PG/SG/SF/PF/C 人数；所有玩家可见的位置均使用标准缩写。

Current request (2026-09-18): 扩军交易协议影响文案保持单行；扩军选秀大厅采用 `gemini-code-1789705799201.html` 的终端 UI，并为联盟球队选择添加动画。

Current request (2026-09-18): 恢复上一版扩军选秀池 UI，撤销刚刚的终端紧凑卡与球队切换动画，保留既有功能和交易协议单行文案。

### V1.29 — 扩军选秀池 UI 回退

- [x] 已回退 V1.28 对扩军选秀池的紧凑终端卡、序号、推荐标签以及球队选择入场/退出与内容切换动画。
- [x] 恢复上一版的选秀池信息层级；中文名、标准位置缩写、年龄、年薪、合同、OVR、当前阵容、保护名单、自动跳队及选人逻辑均继续保留。
- [x] 保留扩军交易协议影响文案的单行显示规则。
- [x] TypeScript、生产构建与 10 个固定种子扩军流程回归通过；唯一发布包已覆盖为此回退版本。

### V1.30 — 扩军选秀大会截图布局校正

- [x] 用户截图中的红框为批注，不作为页面元素；已移除错误加入的红色边框。
- [x] 扩军选秀大会恢复深色/青蓝描边的标题、球队薪资栏、行动播报、保护名单与可用球员卡片层级；当前球队阵容面板继续保留在薪资栏下方。
- [x] 可用球员卡直接显示标准位置、中文名、综合、年龄、年薪、合同年限与“选中球员”操作，不再使用 V1.28 的序号紧凑卡。
- [x] 当前源码 `?fixture=expansion-draft` 场景已通过浏览器无障碍树和截图核验；生产构建通过。

### V1.31 — 新秀选秀大厅终端卡片校正

- [x] 新秀选秀大厅按 `gemini-code-1789705799201.html` 的终端结构优化：模拟顶栏、行动提示、双数据仪表、真实选秀规则说明、榜单标题和独立滚动榜单保持一致。
- [x] 新秀卡改为参考稿的单行结构：左侧顺位/中文名/标准位置/年龄/即战力，右侧潜力评级与球探置信度，并提供紧凑选择按钮；非玩家回合按钮明确禁用并显示“等待”。
- [x] 浏览器 `?fixture=rookie-draft` 已截图核验；生产构建与 10 个固定种子 Stage 4 全流程回归通过。

### V1.32 — 扩军选秀卡片全宽对齐

- [x] 保护名单网格和选秀池卡片不再各自占用内部滚动条宽度，统一与上方薪资、阵容、播报及球队切换卡的左右边界对齐。
- [x] 页面整体承担纵向滚动，保护球员和可选球员卡始终使用完整内容宽度。
- [x] 扩军选秀浏览器截图已核验，生产构建通过。

### V1.28 — 扩军选秀终端与球队选择动画

- [x] “接受后将占用下一次可用扩军签位，并锁定该队唯一损失名额。”在交易协议卡上改为强制单行，超窄视口采用省略而不折行。
- [x] 扩军选秀大厅已更新为终端顶部控制栏、行动状态条、薪资仪表、实时推荐标题与紧凑球员卡；每张卡显示标准位置、年龄、合同、OVR 与选中入口。
- [x] 球队选择器添加入场/退出过渡，点击球队时关闭面板并对受保护名单与可选球员区应用内容切换动画；`prefers-reduced-motion` 环境自动停用动画。
- [x] TypeScript、位置展示单元测试、生产构建及 10 个固定种子扩军流程回归通过。

### V1.27 — 标准位置缩写与阵容折叠摘要

- [x] 集中位置展示函数已统一输出 PG、SG、SF、PF、C；选秀、交易、阵容、球员详情和其他复用该函数的界面会同步采用标准缩写，不改变内部 Position 数据。
- [x] 扩军选秀的当前球队阵容展开时保持原有位置人数面板；收起时摘要改为 `PG n · SG n · SF n · PF n · C n`，并随每次选人实时刷新。
- [x] 新增位置缩写单元测试；类型检查、生产构建以及 10 个固定种子扩军流程回归通过。

### V1.26 — 球员详情终端卡片

- [x] 玩家详情弹层改为 420px 赛博终端卡：官方真人头像、单行中文名、PG/SG 等位置徽标、浮动 OVR、四项身体数据、合同条、八维能力条和底部性格标签均使用参考原型层级。
- [x] 详情卡保留原有交易和球队角色操作区，所有能力、合同、性格、耐伤数据仍直接来自原游戏状态。
- [x] TypeScript、生产构建与数据同步 4/4 测试通过；扩军选秀固定场景已确认中文名、能力、合同信息及“选中球员”入口均正常。

### V1.25 — 新秀选秀双终端界面

- [x] 新秀准备页已替换为终端式 hero、四项数据面板、球探/真实选秀提示与霓虹主操作按钮；原 `PREPARE_ROOKIE_DRAFT` 事务不变。
- [x] 选秀大厅已替换为模拟控制栏、LIVE 播报、当前/用户签位仪表盘与实时推荐卡片；只有轮到用户签位时卡片的“选择”按钮显示并启用。
- [x] 浏览器固定场景实测：`stage4-intro` 显示 80 人、64 签位、#6/#38；`rookie-draft` 显示第 #6 顺位、用户行动提示、75 人可选新秀与全部选择入口。
- [x] TypeScript、生产构建与 10 个固定种子 Stage4 全流程回归通过（64 个签位、自由市场、交易、15 人名单、1312 场比赛）。

### V1.24 — NBA 官方真人头像完成验证

- [x] 统一数据模块新增 `nbaOfficialHeadshotUrl` / `nbaOfficialHeadshotPath`，只接受 NBA Player ID，并提供 `npm run data:sync-official-portraits` 无密钥单独同步入口。
- [x] NBA 官方 CDN 的 260×190 PNG 头像按本地 650 名球员的 NBA Player ID 下载成功 650/650；数据集中 650/650 `portraitPath` 均指向本地 `./player-portraits/nba-{id}.png`。
- [x] 清理 627 张未引用的旧 2K 图片；`public/` 和生产 `h5/` 均只保留 650 张官方头像，资源目录从约 42 MB 降至约 11 MB。
- [x] 同步模块 4/4 测试、TypeScript 检查、Vite 生产构建及 10 个固定种子扩军流程回归通过；构建后 H5 保持相对资源路径。
- [x] 已生成 `release/篮球经理_扩军时代_V1.24_NBA官方真人头像版.zip`，压缩包完整性检查通过，含 650 张官方头像，SHA-256 为 `7eff4ea4c064e29f20c9e5709fe6054be6908cccc5629102265a971618bdf5a9`。

Current request (2026-09-18): 说明真实球员中文名逻辑；从 NBA2K API 获取球员照片并下载为离线资源，在球员详情卡展示；生产版本必须直接通过 `h5/index.html` 访问，不依赖启动服务。

## V1.23 — 中文名口径、离线球员照片与直接 H5 访问

1. 保持中文名优先级：已有中文名 → NBA 中国 Player ID 快照 → 人工全名覆盖 → 仅在每个词均已验证时组合译名 → 原英文名，禁止臆造音译。
2. 扩展统一 NBA2K API 同步模块，读取 `playerImage`，只为能映射到本地数据集的真实球员下载图片；本地文件路径写回版本化快照和球员数据。
3. 图片保存到 `public/player-portraits/`，由 Vite 原样复制进 `h5/player-portraits/`；API Key 和远程图片 URL 不进入 H5 运行时逻辑。
4. 球员详情卡增加自适应照片区；缺图、加载失败和程序化球员自动回退到原有纯文字布局。
5. 验证 `h5/index.html` 使用相对路径、经典 IIFE 脚本并可由 `file://` 直接打开；运行官方 Playwright、截图检查、全量测试、构建和 ZIP 完整性校验。

### 中文名、离线照片与直接 H5 访问完成验证

- [x] 中文名继续按“原中文 → NBA 中国 Player ID 快照（536 人）→ 人工全名覆盖 → 全词已验证组合 → 保留英文”的顺序显示，不生成不确定音译。
- [x] 统一 NBA2K API 同步模块接入 `playerImage`；627/627 个可映射真实球员照片全部下载成功，远程 URL 不写入运行时数据。
- [x] 照片保存到 `public/player-portraits/`，构建后完整复制到 `h5/player-portraits/`；图片目录约 34 MB，球员数据仅保存 `./player-portraits/...` 相对路径。
- [x] 球员详情卡新增照片区并保持中文名、球队位置与 OVR 层级；无照片或程序化球员自动使用原纯文字布局。
- [x] 官方 Playwright 用 `file:///Users/uwa/nbaGame/nbaManagerSimulation/h5/index.html?fixture=1` 直接加载生产 H5，状态为 `TEAM_CREATION`，无 Console error；无需启动服务。
- [x] 扩军选秀 fixture 打开真实球员详情，照片成功显示；`render_game_to_text` 与卡片中的姓名、位置、OVR 一致，无 Console error。
- [x] `visual-verdict` 94/100，结果保存于 `.omx/state/v123-player-portraits/ralph-progress.json`。
- [x] 同步模块 3/3、全量 32 文件 / 109 项测试、TypeScript/Vite 构建与扩军 10/10 固定种子全部通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.23_中文名与离线球员照片版.zip`；676 个条目完整性检查通过，SHA-256 为 `88e5cc0092089a74200e414b6f8510295edf83b67e6cd14d88338dfa49adb4c9`。

Current request (2026-09-18): 接入 NBA2K API，并把球员档案抓取、能力映射、质量校验和离线数据生成统一成可重复执行的模块；H5 运行时不请求外网，API 密钥不进入源码或发布包。

## V1.22 — NBA2K API 离线球员数据同步

1. 新增单入口数据同步命令，从环境变量读取 API Key，一次请求获取指定 2K 版本的现役球员完整档案。
2. 将 API 字段标准化为版本化离线快照；优先复用已有精确分类能力，缺失时由 35 项 2K 属性确定性推导六大分类能力。
3. 复用统一应用层把 OVR、八维能力与耐伤写入游戏数据，但始终保留 NBA 官方离线名单中的球队归属和 Player ID。
4. 写入前校验人数、官方名单覆盖率、平均 OVR、90+ 人数与官方 Top 100 误差；校验失败时不覆盖现有离线数据。
5. 增加纯函数测试、同步报告和维护文档；完成真实 API 同步后运行全量测试、构建、离线 H5 检查并重新打包。

行为保护：API 仅在开发构建时调用；浏览器与发布 ZIP 中不包含密钥、请求代码或运行时网络依赖。

### NBA2K API 离线同步完成验证

- [x] 新增 `data:sync-players` 与 `data:sync-players:dry-run` 单入口；API Key 仅从 `NBA2K_API_KEY` 读取，旧的分离抓取/应用脚本已删除。
- [x] 真实同步生成 `nba2kapi-2k27-2026-09-16` 快照：650 名球员、35 项能力；全项目对齐 627 人。
- [x] NBA 官方名单中已有数据行的 561 人对齐 558 人，覆盖率 99.5%；检测到并忽略 38 处 API 球队归属差异，球队与 Player ID 继续以官方离线名单为准。
- [x] 联盟平均 OVR 75.07、90+ 共 21 人；官方 Top 100 匹配 100/100，OVR 平均绝对误差为 0；杰伦·布伦森保持 96 OVR。
- [x] 增加同步模块 3 项纯函数测试、版本化同步报告、内容 SHA-256 指纹、环境变量与维护文档；项目与 H5 均未发现 API Key。
- [x] 同步模块测试 3/3、全量 32 文件 / 109 项测试、TypeScript/Vite 生产构建、扩军 10/10 固定种子全部通过。
- [x] 本地静态服务器加载生产 H5，页面显示“本地球员数据已载入”；生产包没有 NBA2K API 运行时 fetch 调用。
- [x] 已生成 `release/篮球经理_扩军时代_V1.22_NBA2K_API离线数据版.zip`；48 个条目完整性检查通过，SHA-256 为 `c885557fbac7c6f4cbd911dfc08184e42ccc20122d2a752e131dbb80f19853fe`。

Current request (2026-09-17): 修复扩军交易桌底部“锁定协议并进入扩军选秀”操作区遮挡报价卡；用 NBA 官方 2026–27 名单纠正球队归属（包括克莱·汤普森→热火、CJ·麦科勒姆→老鹰）；移除会把真实英文名伪造为错误中文名的兜底音译。

## 扩军交易桌遮挡 + 当前 NBA 名单校准计划（2026-09-17）

1. 将底部确认区改为报价滚动容器之外的正常布局，报价列表独立滚动并为确认区保留空间，避免覆盖最后一张协议卡。
2. 从 NBA 官方 League Roster 页面提取 2026–27 当前球员 ID 与球队缩写，按 NBA Player ID 精确覆盖本地 2025–26 评分数据的球队归属；不做模糊姓名匹配。
3. 只保留仍出现在 NBA 官方当前名单中的本地评分球员；2026 新秀等尚无 2025–26 评分样本的球员本轮不伪造能力数据，30 队仍须各有至少 9 名可用于扩军保护的真实球员。
4. 为已确认的常见球员增加权威中文名映射；缺少人工映射的英文名直接保留英文，不再生成看似真实但实际错误的中文音译。
5. 补充名单归属和姓名展示回归测试，运行类型检查、全量测试、扩军 Headless、生产构建与官方 Playwright；视觉截图通过 visual-verdict 后重新打包。

### 扩军交易桌遮挡 + 当前 NBA 名单校准完成验证

- [x] 底部确认区取消 sticky 覆盖，报价列表独立滚动；390px 手机壳截图确认按钮与报价卡不重叠，操作区完整处于安全区内。
- [x] 新增 NBA 官方 League Roster 2026–27 快照，共 597 名当前球员、覆盖 30 队；运行时按 NBA Player ID 精确匹配 493 名已有 2025–26 评分样本的在册球员，各队至少 11 人。
- [x] 克莱·汤普森当前归属热火、CJ·麦科勒姆当前归属老鹰的服务层断言通过；不在官方当前名单的旧球员不会进入新建存档。
- [x] 删除字母拼音兜底算法；CJ·麦科勒姆、克莱·汤普森使用人工确认映射，其余未确认中文译名保留官方英文原名，不再生成虚假的中文名。
- [x] 球员详情来源改为“NBA 官方当前名单”，明确球队归属来自 2026–27 官方名单、能力来自 2025–26 赛季模型；Playwright 打开详情验证无 Console error。
- [x] `visual-verdict` 96/100，结果持久化于 `.omx/state/roster-authenticity/ralph-progress.json`。
- [x] `npm run typecheck`、扩军 10 Seeds Headless、生产构建通过；全量测试以单 worker 稳定通过 32 个文件 / 101 项（默认并发曾有 1 个 82 场赛季模拟用例因 35 秒超出 30 秒门槛，单独与串行复跑均通过）。
- [x] 已生成 `release/篮球经理_扩军时代_V1.9_官方名单与交易桌修正版.zip`；48 个条目完整性检查通过，SHA-256 为 `1acd708d055581404542a5b79fc4428c2f58b053ef57ced9b1cac49ebf549ea7`。

## 2026-09-17 真实球员数据与历史原型 Draft

- [x] 新增离线 `nba_api` 采集器：基础、高阶、球员生物信息、拼抢、突破、接球投篮与整体防守；H5 运行时不直接请求 NBA Stats。
- [x] 生成 `nba-player-dataset.json`：2025-26 赛季 582 名球员、2005-06～2024-25 历史赛季、35 个退出现役名单的历史高水平模板，模型与数据版本均可追溯。
- [x] 虎扑当前阵容继续作为姓名/球队/中文资料/薪资/合同唯一来源；按显式虎扑 ID、英文名或“当前球队 + 唯一球衣号码”确定性映射到 `nba_api`，否则使用虎扑基础统计的确定性降级模型。
- [x] 潜力、耐伤、性格和八项能力明确标记为游戏推演；禁止模糊姓名静默关联。
- [x] 2026 首届 Draft 保持纯程序化；2027 起从未使用模板中按 Seed 随机抽取最多 3 个“传奇原型”混入 80 人 Draft Class，原身份不展示、单存档不重复、模板耗尽自动回退程序化新秀。
- [x] 选秀公开 DTO 移除 `truePotential`、成长率、成长波动和历史来源 ID；AI 排序不读取隐藏成长字段。
- [x] 存档 Schema 升至 v17，并迁移历史来源去重账本。
- [x] 全量 `npm test`：31 个测试文件 / 93 项测试通过；TypeScript 与生产构建通过。
- [x] 上限改为随机 3 人后，全量 31 文件 / 94 测试、2 Seeds × 两赛季 Headless 闭环、TypeScript、生产构建与官方 Playwright 均通过；两个 Seed 的 2027 届都准确生成 3 个历史原型。
- [x] 最终 `h5/index.html` 2.84 MB（gzip 598.43 KB），官方 Playwright 以 `file://...?fixture=1` 验证建队 → 权益抽签交互、`render_game_to_text` 状态一致且无 Console error。

Current request (2026-09-17): 按照 `/Users/uwa/Downloads/gemini-code-1789609631793.html` 原型，整体优化当前游戏页面。

Current request (2026-09-17, supersedes prior UI reference): 按照 `/Users/uwa/Downloads/gemini-code-1789619463016.html` 原型，在保证功能不变的情况下 100% 还原。

Current request (2026-09-17): 保留当前暖色日常经营 UI，以 V1.5 原型承载完整功能结构，并吸收 v9 coreflow 在权益抽签、选秀、比赛结果和重大事件中的深色舞台感。

## 暖色经营 + 深色舞台 UI 收口计划（2026-09-17）

1. 不修改 Engine、Command、Save Schema、阶段状态机、`data-testid` 或 `render_game_to_text`，只调整展示壳、语义类名与 CSS。
2. 日常赛程、阵容、管理、联盟、生涯继续使用暖米色；扩军权益、新秀/扩军选秀使用深色舞台壳。
3. 比赛详情和强制事件使用深色沉浸式底部抽屉，球员详情、存档与普通经营弹层继续保持暖色。
4. 在 V2.0 令牌区集中新增舞台色、文字色、边框和阴影变量，避免继续散落新的硬编码颜色。
5. 每轮使用官方 Playwright 客户端生成截图与文本状态，运行 visual-verdict 并持久化评分；低于 90 分继续修正。
6. 最终运行 TypeScript、全量测试、生产构建与 390×844/桌面视口检查，并更新 `h5/index.html`。

行为保护：本轮不改变模拟、交易、选秀、合同、自由市场、伤病、事件或存档业务逻辑。

### 暖色经营 + 深色舞台完成验证

- [x] 扩军权益、扩军选秀、新秀选秀接入 `scene-stage`；建队、合同、交易、自由市场、名单管理仍保持暖色经营界面。
- [x] 比赛详情与强制事件接入深色舞台抽屉，比赛赢家、关键比分、本场最佳与主要决策层级已强化。
- [x] 官方 Playwright 覆盖权益抽签、新秀选秀与重大事件，均生成截图和 `render_game_to_text` 状态，无 Console error；比赛详情另以 390px 实际浏览器交互确认。
- [x] `visual-verdict` 第二轮评分 92/100，结果持久化于 `.omx/state/ui-hybrid/ralph-progress.json`。
- [x] 最终 `npm run typecheck` 通过；`npm test` 为 31 个测试文件 / 95 项测试全部通过。
- [x] 最终 `npm run build` 通过并更新 `h5/`；仅保留既有的单包体积警告。

Current request (2026-09-17): 取消队徽选择，将用户提供的西雅图与拉斯维加斯队徽设为城市默认队徽，并把 v9 的大幅城市卡片选择结构植入当前暖色 UI。

## 固定城市队徽 + v9 城市选择页计划（2026-09-17）

1. 保留 `CREATE_EXPANSION_TEAM` 指令与存档字段，城市仍提交固定 `presetId`，不修改状态机或旧存档结构。
2. 每座扩军城市仅保留一个默认品牌方案，玩家球队和电脑球队都使用对应城市的固定 PNG 队徽。
3. 建队页删除队徽方案选择器，改为两张可触控的大城市卡片；卡片内展示城市、气质文案、默认队徽与选中状态。
4. 队名输入与高级配色继续保留在城市卡片之后，沿用当前暖米色组件、橙色主操作和安全区规则。
5. 使用官方 Playwright 完成西雅图/拉斯维加斯切换与提交链路，检查截图、`render_game_to_text`、Console，再运行 visual-verdict、全量测试与生产构建。

行为保护：本轮不改变权益抽签、扩军交易、选秀、赛季模拟、合同或存档 Schema。

### 固定城市队徽 + 城市选择页完成验证

- [x] 用户提供的两张 1254×1254 PNG 已保存为 `seattle-default.png` 与 `las-vegas-default.png`，并随生产 `h5/` 离线发布。
- [x] 西雅图和拉斯维加斯各只保留一个默认品牌；玩家和电脑扩军队均按城市使用固定队徽，不再随机选择旧方案。
- [x] 建队页删除队徽方案列表，改为 v9 式双城市大卡；保留当前暖色外壳、橙色主按钮、队名输入和高级配色。
- [x] 修正默认球队名称重复拼接城市的问题，最终全名为“西雅图 翡翠潮 / 拉斯维加斯 电压”。
- [x] 390×844 实测两张城市卡、队徽确认、队名输入和主按钮均在首屏；城市切换会同步队徽、默认名称和提交城市。
- [x] `visual-verdict` 评分 93/100；结果持久化于 `.omx/state/city-defaults/ralph-progress.json`。
- [x] 最终 `npm run typecheck` 通过；`npm test` 为 31 个测试文件 / 96 项测试全部通过；`npm run build` 通过。
- [x] 生产 `h5/index.html?fixture=team-creation` 已用官方 Playwright 复查，等待图片解码后页面、两张 PNG 与 `TEAM_CREATION` 文本状态一致，无 Console error。
- [x] 已生成 `release/篮球经理_扩军时代_V1.6_城市队徽版.zip`；压缩包以 `index.html` 为根目录，完整性检查通过。

Current request (2026-09-17): 城市选择页不展示默认品牌；球队名称不再预填或随城市切换，完全使用用户输入。

## 自定义球队名称收口（2026-09-17）

1. 城市卡片移除“默认品牌”文案和额外品牌摘要，只保留城市、城市气质、固定队徽与选中状态。
2. 球队名称初始为空，切换城市不覆盖用户输入；名称不足 2 个字符时禁止提交。
3. 保留城市固定 `presetId` 作为内部队徽/配色契约，但不向玩家展示品牌概念。
4. 验证“输入名称 → 切换城市 → 名称仍保留 → 提交后球队全名使用用户输入”的完整链路。

### 自定义球队名称完成验证

- [x] 城市卡片已移除“默认品牌”和品牌摘要，只展示城市、城市文案、固定队徽与选中状态。
- [x] 球队名称默认留空且完全由用户输入；切换西雅图/拉斯维加斯不会再覆盖已输入名称，少于 2 个字符时创建按钮保持禁用。
- [x] 新增服务层回归测试，确认提交“雨城先锋”后球队名称为“雨城先锋”，完整名称为“西雅图 雨城先锋”，不会回退到城市预设名称。
- [x] 390×844 浏览器实测“输入山海飞跃 → 切换拉斯维加斯 → 提交”链路，扩军权益页正确显示“拉斯维加斯 山海飞跃”，Console 0 error。
- [x] `visual-verdict` 评分 94/100；结果持久化于 `.omx/state/custom-team-name/ralph-progress.json`。
- [x] `npm run typecheck` 通过；`npm test` 为 31 个测试文件 / 97 项测试全部通过；`npm run build` 通过。
- [x] 生产 `h5/index.html?fixture=team-creation` 已由官方 Playwright 复查，画面与 `TEAM_CREATION` 状态一致，未生成 Console error 文件。
- [x] 已生成 `release/篮球经理_扩军时代_V1.6.1_自定义球队名称版.zip`；压缩包以 `index.html` 为根目录，48 个条目完整性检查通过，SHA-256 为 `fc777beed7ab9227bd059b0e1045cbdf376620a21f05b00eb383ba717903bb29`。

Current request (2026-09-17): 扩军权益页同时展示两队并增加 50/50 抽签动画；权益确认后自动完成合同选项后台结算并进入扩军选秀准备；重做扩军交易桌、把操作按钮放到底部，并在扩军流程使用中文球员显示名。

## 扩军抽签与交易桌重构计划（2026-09-17）

1. 保留冻结需求规定的固定 Seed、一次性抽签结果、赢家优先选 Package 与合同选项先于扩军池的规则，不允许 UI 重新随机或跳过领域校验。
2. 权益页同时展示西雅图和拉斯维加斯，以动画揭晓已写入 GameState 的 50/50 结果；玩家中奖时仍手动选择权益包，AI 中奖时展示自动分配结果。
3. 新增一个原子 Command，把权益确认、Option Phase 后台结算、保护名单冻结和交易报价生成合并为一次提交；普通新生涯不再停留在“合同选项已经结算”说明页。
4. 交易桌按 v9 的卡片信息层级重做：报价类型、吸收球员、合同成本、补偿签与风险说明清晰分区，详情/接受按钮位于卡片底部，进入选秀按钮位于页面最底部。
5. 扩军流程统一通过中文显示名函数输出球员名；已有中文名原样保留，离线英文数据生成稳定、唯一的中文音译显示名，不改变内部 Player.name 与存档结构。
6. 增加状态机和显示名测试，并用官方 Playwright 覆盖抽签动画、权益确认、交易桌和进入扩军选秀的完整链路；每轮截图执行 visual-verdict。

行为保护：扩军抽签结果、合同资格、交易承诺、补偿资产、保护名单、蛇形顺序和存档字段不变；仅合并过渡 Command 与重构 UI。

### 扩军抽签与交易桌完成验证

- [x] 权益页同时展示西雅图与拉斯维加斯，点击后播放 50/50 聚光灯动画，并高亮 GameState 中已经固定的抽签赢家；动画不会重新生成结果。
- [x] 玩家中奖时两个权益方案同时显示并可直接选择；AI 中奖时展示自动分配结果。权益确认通过 `ENTER_EXPANSION_DRAFT_PREP` 原子完成 Package 分配、合同选项结算、保护名单冻结和报价生成。
- [x] “合同选项已经结算”不再作为普通新生涯的独立阻塞页；交易桌明确显示它已在后台完成，保证仅有效合同进入扩军池。
- [x] 扩军交易桌完成结构重做：阶段摘要、规则说明、报价类型筛选、球队与球员信息、合同成本、补偿资产和风险说明分区展示；详情/接受按钮位于卡片底部，进入选秀按钮位于完整报价列表之后。
- [x] 扩军流程统一使用稳定中文球员显示名，已有中文名不变；新增测试确认内置数据集全部球员的 UI 显示名不含英文字母，内部姓名和存档不变。
- [x] 浏览器完整验证“开始抽签 → 揭晓赢家 → 选择方案甲 → 后台合同结算 → 接受 1 份报价（0/5 → 1/5）→ 进入扩军选秀”，Console 0 error / 0 warning。
- [x] `visual-verdict` 最终评分 95/100；结果持久化于 `.omx/state/expansion-draw-trade/ralph-progress.json`。
- [x] `npm run typecheck` 通过；`npm test` 为 32 个测试文件 / 99 项测试全部通过；`npm run build` 通过。生产 `file://` 包状态为 `TEAM_CREATION`，未生成 Console error 文件。
- [x] 已生成 `release/篮球经理_扩军时代_V1.7_扩军抽签交易桌版.zip`；压缩包完整性检查通过，大小 5,418,452 bytes，SHA-256 为 `49e9b8789f2d0fe3b0b0fc1fce10150c3205746d79569d23a5f970f7bf1cbbdc`。

## V2.0 暖色原型还原计划（2026-09-17）

1. 以新 HTML 的 `390×844` 固定画布、暖米色设计令牌、顶栏、五栏底部导航和底部抽屉为唯一视觉基准。
2. 保留现有 Engine、Command、存档 Schema、`data-testid`、`render_game_to_text` 与所有阶段判断，只调整 React 结构中的展示壳和 CSS。
3. 原型已有页面逐项复刻；原型未包含的合同选项、新秀选秀、自由市场、伤病与事件页面使用同一设计系统扩展，不删除任何现有功能。
4. 每个有意义的 UI 变更后运行官方 `develop-web-game` Playwright 客户端，实际查看截图、文本状态和 Console；使用 `visual-verdict` 与原型截图做差异门禁。
5. 最终运行 TypeScript、全量测试、生产构建与离线资源检查，并重新覆盖发布 ZIP。

行为保护：本轮不修改模拟、交易、选秀、合同、自由市场、伤病、事件或存档业务逻辑。

## UI 原型重构计划（2026-09-17）

1. 保持 Engine、Command、存档和所有现有 `data-testid` 行为不变，先锁定当前测试/构建基线。
2. 抽出统一 App Chrome：顶部版本/阶段/数据状态、移动端安全区、赛季页底部 5 栏导航。
3. 将全局视觉 token 改为原型的 Slate 暗色体系，主操作用篮球红，次操作用竞技蓝，强调信息用活力金。
4. 统一扩军、选秀、自由市场、常规赛、详情弹层的卡片、按钮、表格、状态标签、滚动区域和触控尺寸。
5. 在 390×844 与桌面视口运行官方 Playwright 客户端，检查截图、交互状态和 Console；随后运行 typecheck、tests、build。

行为保护：本轮不改变模拟公式、阶段状态机、数据结构、存档 Schema 或交易/选秀/自由市场规则。

### 2026-09-17 UI 重构进展

- [x] 读取并解析原型源码，确认 390px 移动端 App 壳、强流程页与赛季 5 Tab 两种信息架构。
- [x] 新增统一 `GameChrome` 与受控 `SeasonNavigation`；赛季页已拆为首页、阵容、管理、联盟、生涯 5 个真实视图。
- [x] 扩军与 Stage 4 全流程接入同一顶部版本/阶段/数据状态条。
- [x] 全局换用 Slate 暗色 + 篮球红/竞技蓝/活力金 token，统一卡片、按钮、表单、状态、列表、底部导航与安全区。
- [x] 保留 Engine / Command / Save / `render_game_to_text` / `advanceTime` 及现有 `data-testid` 契约。
- [x] 改造前基线：30 个测试文件 / 87 项测试通过；TypeScript 通过。
- [ ] 浏览器逐态截图、Tab 交互、Console 与 390×844 / 桌面响应式巡检。
- [ ] 最终 tests / typecheck / build 回归与 `h5/index.html` 更新。

### 2026-09-17 UI 重构完成验证

- [x] 官方 Playwright 客户端已覆盖源码入口：扩军建队、常规赛首页、阵容、管理、联盟、生涯、季前名单；各次运行均生成 screenshot + `render_game_to_text` state，未生成新的 Console error 文件。
- [x] 390×844 实测修复名单横向溢出（Document 398px → 375px），球员详情改为 88dvh 底部抽屉并保持双列能力值。
- [x] 最终 `npm run typecheck` 通过。
- [x] 最终 `npm test`：30 个测试文件 / 87 项测试全部通过。一次与 build 并行运行时长期赛季用例超时，单独与串行复跑均通过，未发现逻辑回归。
- [x] 最终 `npm run build` 通过；`h5/index.html` 536.55 kB（gzip 163.12 kB）。
- [x] 最终生产包 `h5/index.html?fixture=1` 已用官方 Playwright 客户端复查，画面为新版原型风格，状态为 `TEAM_CREATION`，无新 Console error。

本轮剩余风险：未改变玩法逻辑；现有发布 ZIP 未自动覆盖，正式重新上传前如需 ZIP 应基于新版 `h5/` 重新打包。

# 篮球经理：扩军时代 — 开发进度

## 当前范围

- Stage 0：独立项目、Skills 就绪、平台能力盘点与 Adapter 边界。
- Stage 1：fixture Dataset、League/Player/Team/GameState/Save/Seed。
- Stage 2：82 场赛程、确定性模拟、Box Score、排名、Play-In/Playoffs、赛季推进。
- Stage 3：扩军建队、权益、保护、交易与 28 人扩军选秀闭环。
- Stage 4：首届 Rookie Draft、工资帽、自由市场、玩家常规交易、Waive/Dead Money、开季名单锁定、AI↔AI Trade 与合同 League Year Rollover 已完成。
- Stage 5（完成）：长期选秀、成长/衰退、训练、伤病、Emergency Roster、Awards/All-Star、名人堂、历史压缩、GM Career 与基础 Achievements 已接通。
- Stage 6（完成）：56 个数据驱动 Event Definition、可序列化 Event Queue、重大事件暂停、漫画式事件卡、比赛详情与快速模拟摘要已接通；扩军开场与夺冠事件使用自有专属插画。
- Stage 7（进行中）：Team Core / Fit、AI Direction Cooldown 与 Save Revision Sync 已接通；Headless 长期回归与性能优化继续进行，20 Seeds × 30 Seasons 和 1000 Season equivalents 尚未完成。

## 约束

- 唯一规则源：`/Users/uwa/Downloads/篮球经理_扩军时代_需求文档_V1.5_开发最终冻结版.md`。
- Headless Engine 与本地回归使用 fixture；虎扑环境开局时必须先同步真实阵容，不允许静默降级。
- Engine 与 React/DOM 解耦，必须可以通过 Headless Runner 验证。

## TODO

- [x] 创建 `basketball-franchise`、`basketball-balance` Skills；官方 Python 验证器缺少 PyYAML，已用 Ruby YAML 解析与 TODO/名称检查完成等价结构校验。
- [x] 更新保留的两个篮球 Skills，移除旧 Skill 引用。
- [x] 建立 React/TypeScript 项目、平台能力盘点、LocalStorage Adapter 与三层边界。
- [x] 实现 Stage 1 fixture Dataset、GameState、Save Schema、FNV-1a 64 与 xoshiro128** 随机合同。
- [x] 实现 Stage 2 的 82 场赛程、同一比赛引擎、Box Score、排名、Play-In/Playoffs、赛季推进与 Headless Runner。
- [x] 添加 29 项自动测试；构建通过；固定 Seed 连续 3 赛季通过。
- [x] 完成 Playwright 截图/state 输出、390×844 手机视口、按钮交互、保存和 Console 回归。
- [x] 接入用户提供的 30 队 HoopChina CDN Logo、中文队名、场馆和日/夜色值；因 CDN 浏览器热链返回 403，已缓存为同源静态资源。
- [x] 为 Seattle / Las Vegas 各制作 4 套预审核扩军 Brand Preset；玩家选择，AI 按 Career Seed 确定性选择。
- [x] Stage 3：建队、权益抽签、强制 Option Phase、合格球员保护名单、Expansion Trade、Pool Feasibility、28 签蛇形 Expansion Draft、自动保存与选秀前 Checkpoint。
- [x] 将生产构建改为虎扑工坊标准 `h5/` 静态目录；入口、JS/CSS、38 个 Logo 资源均使用包内相对路径，不再包含本地服务器提示或回环地址。
- [x] 将构建后的 JS/CSS 内联到 `h5/index.html`；根目录双击入口无服务器跳转到显式 fixture 预览，避免 `file://` 加载 TypeScript/ES Module 导致空白页。
- [x] 保留 480 名确定性虚构球员资料用于 Headless 与 `?fixture=1` 开发测试；正式虎扑开局不使用该名单。
- [x] 在扩军交易、扩军选秀和阶段总结中显示球员姓名，并增加球员详情弹窗与旧存档 Player Core 自动迁移。
- [x] 接入虎扑 `teamPlayerList` 与 `teamSalaryInfo`：按请求保护分两批同步 30 队真实名单、位置、赛季表现、年龄和多年薪资。
- [x] 用真实赛季数据派生游戏能力值；性格、耐伤等非接口字段明确标记为游戏推演，不再把虚构名单作为虎扑环境的静默降级。
- [x] 球员详情按需调用 `playerInfo` 补取真实身高、体重、出生日期和国籍；不加载头像，并缓存同一球员请求。
- [x] Stage 4 首段：生成确定性 80 人程序化 2026 Draft Class，完成 2 轮 64 签，支持玩家 #5/#6 与 #37/#38 选择，其余签位由只读取公开球探信息的 AI 自动完成。
- [x] 首轮固定 120% Rookie Scale 的 4 年合同（后两年 Team Option）、次轮 2 年最低薪合同（次年 Team Option）自动生成；16 名落选秀进入 UFA 池。
- [x] 新增 `CapSheetService`，统一汇总 Active Salary、Dead Money、Cap Hold、Offer Reservation 与 Incomplete Roster Charge；Stage 4 UI 展示与 Engine 同源的工资帽结果。
- [x] 自由市场状态机：ACTIVE/ACCEPTED/REJECTED/EXPIRED/WITHDRAWN/SIGNED_OFFER_SHEET、固定 3 日 Decision Window、最多 5 份报价、Early Accept、Deadline Tie-break 与 Cap Reservation。
- [x] RFA：Qualifying Offer Cap Hold、Proposal、Signed Offer Sheet、2 日 Matching Window、玩家 MATCH/DECLINE 阻塞决策与 AI 确定性匹配。
- [x] AI 每队每日最多生成 1 份（规则上限 3 份）合法报价；与玩家共用 CapSheetService、TransactionPolicyService、Offer Utility 与结算事务。
- [x] Stage 4 自由市场 UI：球员池、UFA/RFA 标签、建议起薪、报价/撤回、Day 结算、Cap 实时占用、RFA 决策与联盟动态。
- [x] 常规交易：3 个动态报价、主动刷新 Counter、重复询价 Seed、双方 Salary Match、选秀权唯一所有权与未来七年连续首轮限制校验。
- [x] Waive / Dead Money、AI 自动裁至 15 人、玩家显式最低薪补齐确认、14～15 人 Opening Roster Lock。
- [x] 扩军 Career 主流程现已从建队完整衔接至 32 队、每队 82 场、联盟 1312 场的第一赛季。
- [x] `ContractLifecycleService` 成为唯一 League Year Rollover 入口：累计 Service Roster Days、结算 Service Years、延续 Bird Years、推进合同年份、自动处理 Player Option / AI Team Option，并阻塞玩家未决 Team Option。
- [x] 合同到期统一转入 UFA / RFA，Option Phase 完成后创建不重复的 Bird UFA / RFA Cap Hold；普通签约、交易与 Waive 分别按 V1.5 更新 Bird Rights。
- [x] AI↔AI Trade 按固定 Seed、每 7 个游戏日及 Deadline Window 评估，双方共用 SalaryMatchValidator，排除玩家球队并执行每队每季最多 3 笔的硬上限。
- [x] 常规赛在截止日后自动进入 `REGULAR_POST_DEADLINE`；比赛日为所有在队 Standard Contract 球员累计 `service_roster_days`。
- [x] 后续 Draft Lottery：16 支未进季后赛球队按 Config 权重无放回抽前 4，剩余按战绩倒序和固定 Seed Tie-break；选秀权 Owner 从资产表读取。
- [x] 存档 Schema 升至 v6；旧存档会补全 AI Trade / Contract Lifecycle 默认状态和当前赛季未来 7 年缺失选秀权，不覆盖已有 Ownership。
- [x] 扩军开局已可完整跑到 2027-28 第二赛季：合同选项 → 未来 Lottery / 80 人 Draft Class → 64 Picks → FA → 名单锁定 → 1312 场新赛季。
- [x] 全联盟球员生命周期：按出生日期/快照年龄源推进年龄，使用 Potential、Development Rate、Volatility、上场时间、球队/轮换角色、健康、IQ 与固定 Seed 逐属性成长或衰退；运动能力/终结/外防更早衰退，投射/组织/IQ 更稳定。
- [x] 退役状态机：按年龄、OVR、健康、角色、合同、老将失业时间、性格与 `hash(season_seed, "retirement", player_id)` 确定性判定；退役后清理球队、Cap Hold 与 Offer Reservation，并保留生涯快照和历史索引。
- [x] 每届 80 人未来新秀供给与退役流出已纳入人口指标；Headless 输出 Active / Free Agent / Living / Retired / Rookie Inflow / Retirement Outflow / 90+ / Max / P95 OVR。
- [x] 玩家训练重点：季前最多指定 2 人，支持综合/投射/组织/防守/内线/运动能力；仅调整成长权重，年度结算后消费并重置，裁员会自动撤销指定。
- [x] 存档 Schema 升至 v16；旧档自动补训练、伤病、奖项、历史赛季、成就、GM Career、Event Queue / Effect、Emergency Salary Ledger、Team Core、季后赛统计、AI GM Profile 与球员 Health / Morale 默认状态，不覆盖已有经理状态。
- [x] 伤病系统：五档伤病、年龄/耐伤/分钟/Fatigue/既往伤病风险、自动轮换、核心重大伤病暂停与确认恢复。
- [x] Emergency Active Roster：玩家少于 8 人暂停、AI 自动补至 8 人、少于 5 人禁止开赛并使用程序化 Replacement fallback；临时合同按名单锁定日计薪，恢复后自动结束且零 Dead Money。
- [x] 常规赛 Awards / All-Star（分区各 12 人）、Finals MVP、冠军荣誉和全联盟名人堂模拟；现实球员缺历史数据时保持 HOF ineligible。
- [x] 赛季历史归档：常规赛 Lightweight Results、玩家 82 场完整详情、全部季后赛完整 Box Score、最终排名与用户季后赛里程碑。
- [x] 14 项 V1 基础 Achievements 与 GM Career / 王朝评分；Draft / Trade History 由事务服务写入。
- [x] 56 个有效 Event Definition 与 Event Queue：优先级排序、一次性/冷却、强制暂停、幂等确认、未知类型安全跳过、Save 恢复与渐变漫画卡。
- [x] 玩家模拟按钮改为推进到下一场本队比赛；“模拟 5 场”按玩家赛程推进并输出本段战绩、排名变化、主要球员、伤病与事件摘要。
- [x] 玩家最近 5 场可打开完整比赛详情，展示分节比分、OT、本场最佳和双方球员 Box Score；分节得分严格与最终比分守恒。
- [x] 季后赛为球员单独累计 Postseason Stats，并沿用伤病与 Emergency Roster 规则；新赛季常规赛/季后赛统计分别清零，历史常规赛快照保持不变。
- [x] Team Core 持久化 Market Rating / Franchise Reputation / Fan Support；逐场胜负与连胜连败、季后赛节点、战绩及 MVP/ROY 按 V1.5 公式更新，Free Agent Attraction 使用四项派生公式。
- [x] `TeamFitService` 输出组织、空间、侧翼防守、护框、篮板、位置尺寸、替补深度与球权兼容，比赛引擎使用 -5～+5 有限修正；球队页与交易报价展示当前/Projected Fit。
- [x] AI GM Profile 持久化 Personality / Direction，扩军 AI Strategy 映射固定性格与方向；常规 AI 方向按战绩评估并执行 60 天 Cooldown。
- [x] Save Sync 支持离线连续 Revision、固定 `sync_base_revision`、安全 Fast-forward、同 Hash 归一化、双向修改显式冲突与本地/云端二选一；覆盖云端前生成并校验 Conflict Backup。
- [x] Checkpoint 按 Career Slot 滚动保留最新 3 份，Previous Valid Save 纳入损坏恢复，并提供 Slot 全部持久化内容的字节占用统计。
- [x] 虎扑正式容器使用 `ColorboxAI.storage` Cloud Adapter，按官方单次 200KB 限制将 gzip 数据分块写入；本机离线分支使用 IndexedDB，localStorage 仅保留给无虎扑 SDK 的本地预览。
- [x] 历史压缩按 V1.5 保留最近完整赛季；更早赛季只长期保留玩家季后赛、全部 Finals、Game 7、队史首胜、重大伤病与纪录比赛。
- [x] 常规赛球队页补齐完整阵容、Role、OVR、Potential Grade、年龄、合同、健康、常规赛数据与球员详情；交易截止日前可直接打开交易中心并查看 Projected Fit。
- [x] 扩军开场与夺冠事件接入两张自有故事插画，不含官方联盟/球队 Logo、官方奖杯或可识别真实人物。
- [x] 球员状态补齐 Health / Morale：伤病按严重度降低健康并在康复后回满；士气按胜负、连胜/连败及出场情况变化，比赛引擎严格使用 V1.5 的 0～-1.5 效率惩罚曲线，高士气不提供额外加成。
- [x] 常规赛交互补齐“模拟到下一事件”，遇 Event Queue、重大伤病、Emergency Roster 或赛季结束立即停止；保留“模拟至季后赛”作为附加快捷入口。
- [x] 常规赛页面补齐玩家 82 场完整日历、联盟得分/篮板/助攻领袖、MVP Race、32 队列表与联盟新闻；球员详情补齐 Traits、生涯数据、角色调整和交易入口。
- [x] 球队角色调整进入 `RosterService` 事务与命令幂等体系，Engine 强制 `FRANCHISE_CORE` 最多 3 人。
- [x] Event Effect 执行器接入 GameState：Choice Effect 随 Event Instance 快照并按 `event_instance_id:effect_id` 幂等执行，支持 Player Morale / Form、Fan Support、Franchise Reputation 与 League Log；旧事件存档自动迁移。
- [x] Event Trigger 补齐日初交易截止日、玩家伤病/复出与每 6 场一次的确定性 Breakout / Slump / Role / Morale 检查；日初强制事件处理后会在同一日期继续结算，不漏比赛。
- [x] Player Fatigue / Form 从静态字段升级为完整逐场闭环：分钟、年龄、运动能力与背靠背决定疲劳增长，休息日恢复，近期表现更新 Form；季后赛同样执行，所有系数集中在 `SIMULATION_CONFIG`。
- [x] 新增统一版本化 `GAME_CONFIG` 入口，完整覆盖比赛模拟、成长/退役/名人堂、AI GM、交易/选秀权价值、自由市场、工资帽、Draft、伤病、随机事件、联盟平衡目标、成就奖励与扩军 Package；原有主要魔法数字已接回配置。
- [x] 新增配置结构校验与中文调参说明；权重和为 100、工资线顺序、合同年限、Draft 供给、Pace 范围与退役年龄档位均有自动门禁。
- [x] 第二轮配置化清理：攻防 Impact、轮换/OT、Box Score 骨架与守恒修正、Form/Fatigue、Team Fit、球队核心、奖项、各位置 OVR、赛程质量、CBA 名单/薪资匹配/Cap Hold/新秀合同、Emergency、历史压缩和 Dynasty Score 均已移除主要魔法数字；冻结的 82 场与状态机结构保持不可调。

## 已验证

- `npm test`：28 个测试文件、85 项测试全部通过。
- `npm run build`：生产构建为单文件 `h5/index.html` 493.04 kB（gzip 151.34 kB），完整 `h5/` 约 2.9 MB；包内无 localhost / 源码入口引用。
- `pnpm headless:career -- --seed 12345 --seasons 3`：2026-27 至 2028-29 全部完成，每季 1312 场。
- 浏览器：模拟下一比赛日后 SEA 由 0-0 变为 1-0；`render_game_to_text` 与页面一致；Console 0 error/warning。
- Logo 回归：西部与东部排名页图片均加载完成，naturalWidth / naturalHeight 有效，Console 0 error/warning。
- Stage 3 浏览器：从建队完整走到 28 Picks；补充验证强制 Option Phase 与交易页；SEA / LVG 各 14 人；390×844 与桌面布局通过；Console 0 error/warning。
- `pnpm headless:expansion -- --seeds 20`：20/20 个 Seed 完成，均为 SEA / LVG 各 14 人且无重复来源球队。
- Stage 3 变更后重跑 `pnpm headless:career -- --seed 12345 --seasons 3`：旧 Stage 2 三赛季流程继续通过。
- 根目录 `index.html` 仅作为 Vite 源码入口；正式交付入口固定为 `h5/index.html`，生产包不包含开发服务器提示或本地地址。
- `h5/index.html` 已通过 `/h5/` 子路径静态托管验证；4 个首屏扩军 Logo 全部加载，`render_game_to_text` 正常，Console 0 error/warning。根目录仍是开发源码入口，提交工坊时使用 `h5/` 目录。
- 球员资料浏览器回归：交易页姓名/合同正常，详情弹窗可展示体测、合同、八项能力与性格信息；390×844 布局及 Console 0 error/warning。
- 真实名单启动门禁回归：无虎扑 SDK 时显示明确错误且不进入虚构名单；仅 `?fixture=1` 允许本地测试，两个入口均无 Console 错误。
- `file:///.../nbaManagerSimulation/index.html` 直接打开回归：自动进入 `h5/index.html?fixture=1`，`render_game_to_text` 为 `TEAM_CREATION`，Console 0 error/warning。
- Stage 4 浏览器回归：从 Team Creation 完整操作到扩军选秀结束，再完成 Rookie Draft #5 与 #37；最终 64 签、16 名落选秀、玩家名单 16/21，工资帽 UI 与 Engine 一致；Console 0 error/warning。
- `npm run headless:stage4 -- --seeds 20`：20 个固定 Seed 验证 80 人选秀班、64 个唯一 Pick、开市前固定 16 名落选秀、三天自由市场结算与无过期 ACTIVE Offer。
- 自由市场浏览器回归：载入选秀后存档、进入 Day 1、报价后 Cap Space 从 $53.5M 降至 $43.6M、固定 D3 Deadline、结算至 Day 4 后签约并释放 Reservation；Console 0 error/warning。
- 官方 `develop-web-game` Playwright 客户端已运行并检查 `output/stage4-fa-client/shot-0.png` 与 state JSON；随后使用完整浏览器链路覆盖自由市场交互。
- `npm run headless:stage4 -- --seeds 20`：更新后的完整首赛季经理路径 20/20 通过，均为 64 Picks、FA Day 4、玩家交易成功、15 人名单与 1312 场赛程。
- `npm run headless:manager-loop -- --seeds 2`：两个固定 Seed 均从扩军开局完整进入 2027-28，完成第二届 64 Picks、未来 Lottery / Draft Class、自由市场、14 人名单锁定与 1312 场新赛程。
- `npm run headless:career -- --seed 12345 --seasons 3`：统一合同滚动与 AI Trade 接入后，三赛季仍全部完成 1312 场并产生冠军。
- Option Phase 浏览器回归：官方 Playwright 客户端验证 Team Option 提交后 `pendingTeamOptions: 1 → 0`；浏览器完整点击“执行 → 完成 Option Phase”，成功进入 2027-28 `OFFSEASON_PRE_DRAFT`，画面与状态一致且无新 Console 错误。
- Stage 5 完整回归：`npm test` 为 17 个测试文件、52 项测试全通过；`npm run typecheck` 与生产构建通过，单文件 `h5/index.html` 约 397 kB（gzip 约 124 kB），包内无 localhost / `/src/main.tsx` 引用。
- 调参后单 Seed 10 赛季生命周期回归：初始平均 OVR 69.32，十年后 69.52（漂移 +0.20）；90+ 球员 9 人，最高 93.13，位于 V1.5 的 5～25 人目标区间；第十年新秀流入 80、退役流出 81。
- 生命周期 UI 回归：Option Phase 展示成长/衰退/重点培养/退役/联盟均值并可继续进入下一届 80 人 Draft；季前页面成功指定“投射 + 防守”两名重点培养球员，第三人选择器自动禁用，桌面截图布局通过。
- 伤病/Emergency UI 回归：官方 Playwright 验证重大伤病确认、可用球员 7 人暂停、补齐至 8 人后解除阻塞；状态 JSON 与页面一致且 Console 0 error。
- Awards/History UI 回归：赛季荣誉、All-Star、名人堂、GM 等级/王朝分与 14 项成就卡布局通过；Console 0 error。
- Event UI 回归：Priority 100 冠军事件只显示一个 Modal，确认后 Queue 清空且阻塞解除；Event state 与页面一致，Console 0 error。
- 比赛详情/批量摘要回归：分节比分、Box Score、最近比赛入口与“5 场 · 3胜2负 / 排名 2→5 / 主要球员 / 伤病 / 事件”摘要均通过官方 Playwright 检查。
- Stage 6 回归：`npm test` 22 个文件 / 65 项全通过；生产构建为单文件 `h5/index.html` 447.88 kB（gzip 139.33 kB），包内无 localhost 或源码入口引用。
- 新版三赛季 Headless：`stage6-regression` 连续完成 2026-27 至 2028-29，每季 1312 场并正确产出冠军。
- 新版十赛季生命周期：平均 OVR 69.32 → 69.49（+0.17），90+ 球员 9 人，最高 93.13，第十年新秀流入 / 退役流出 80 / 80，产生 1 名名人堂入选者。
- `npm run headless:benchmark -- --season-equivalents 3 --seasons-per-career 3`：3/3 Season equivalents、3,936 场常规赛、275 场季后赛、0 严重一致性错误；平均 13.55 秒 / Season。Runner 已支持后续 20×30 与 1000 Season equivalents 门禁。
- Team Core / Fit 浏览器回归：官方 `develop-web-game` Playwright 客户端展开球队卡，8 项 Fit、Market / Reputation / Fan Support / FA Attraction 与 `render_game_to_text` 一致；Console 0 error。
- Save Sync 定向回归：11 项测试通过，覆盖离线 Revision 链、Fast-forward、同 Hash 不冲突、双向冲突、云端覆盖前 Backup、3 Checkpoint 滚动与 Slot Size 统计。
- 新版 10 赛季增强型回归：13,120 场常规赛、919 场季后赛、0 严重一致性错误；8 支不同冠军，720 名新秀流入、419 名退役、1 名名人堂球员；十年原始 JSON 峰值 6.37 MB。
- 原生 gzip 存储实测：单赛季原始 GameState 2.19 MB，实际存储 0.30 MB；Colorbox 分块适配测试确认每次官方 Storage 写入低于 200KB 且可完整重组。
- 开场/夺冠视觉回归：官方 Playwright 分别检查 `?fixture=opening` 与 `?fixture=event`，专属插画、事件文案与 Modal 布局正常，Console 0 error。
- 球员状态 UI 回归：官方 Playwright 展开常规赛阵容卡，15 人 Health / Morale、合同与赛季数据正常展示；`render_game_to_text` 同步输出 Team Fit Health，Console 0 error。
- Health / Morale 接入后的一季增强型 Headless 回归：1,312 场常规赛、92 场季后赛、0 严重一致性错误；原始 GameState 2.20 MB、gzip 存储 0.30 MB。
- 最新连续三赛季增强型 Headless 回归（含 Fatigue / Form / Event Effect）：3,936 场常规赛、283 场季后赛、0 严重一致性错误；160 名新秀流入、66 名退役、3 支不同冠军、最高 91.13 OVR；原始 GameState 峰值 3.18 MB、gzip 存储峰值 0.49 MB。
- “模拟到下一事件”浏览器回归：从 1-0 自动推进至 3-0 后停在 `streak_winning_003`，没有越过待处理 Event，Console 0 error。
- Event Effect 浏览器回归：确认冠军事件后 Queue 清空、Fan Support 58 → 59、Free Agent Attraction 同步重算，Console 0 error；接入后完整一季仍为 1,312 场常规赛、92 场季后赛、0 严重一致性错误。
- Fatigue / Form 接入后完整一季增强型回归：1,312 场常规赛、102 场季后赛、0 严重一致性错误；原始 GameState 2.24 MB、gzip 存储 0.31 MB。
- 配置化回归：`npm test` 为 29 个测试文件 / 86 项全通过；`npm run build` 通过，单文件 `h5/index.html` 506.32 kB（gzip 155.66 kB）。
- 配置化后固定 Seed 三赛季增强型 Headless：3/3 Season equivalents、0 严重一致性错误；160 名新秀流入、66 名退役、平均 OVR 69.15、4 名 90+、gzip 存储峰值 0.49 MB。
- `game-config.v2` 回归：30 个测试文件 / 87 项全通过，生产构建 `h5/index.html` 523.44 kB（gzip 160.15 kB）；固定 Seed 三赛季为 3,936 场常规赛、275 场季后赛、0 严重一致性错误，平均 OVR 69.15、4 名 90+、gzip 峰值 0.49 MB。

## 后续阶段

- Stage 7：继续运行 20 Seeds × 30 Seasons 和 >=1000 Season equivalents 平衡回归；当前仍只完成单 Seed × 10 赛季门槛验证，未把单 Seed结果冒充发布级回归。
- Release 门禁：用户已确认 30 队 Logo 为虎扑授权素材，Logo 门禁解除。真实球员开档前历史荣誉数据仍未由当前 Skills 提供，继续保持 HOF ineligible 而不虚构。
- 虎扑 AI 工坊上传包已于 2026-09-17 按全中文界面最终构建覆盖更新：`release/篮球经理_扩军时代_V1.5_虎扑AI工坊.zip`，ZIP 根目录直接包含 `index.html`，共 41 个文件、约 2.0 MB；文件清单与 `h5/` 逐项一致，`unzip -t` 完整性校验通过，SHA-256 为 `9b2024f715c8b17833af747fb403b5157c0cdae309f3d302b864f26c289e2228`。
- 上传包交付前回归：30 个测试文件 / 87 项测试全通过，TypeScript 与 Vite 生产构建通过；官方 `develop-web-game` Playwright 客户端从最终 `h5/index.html?fixture=1` 加载扩军开局，截图与 `TEAM_CREATION` 状态一致。按 `audit-hupu-web-security` 检查源码请求、外域资源、动态执行和设备能力，未发现禁止项。

## 全中文界面回归（2026-09-17）

- [x] 扩军第三阶段、经理系统第四阶段、常规赛首页、阵容、管理、联盟、生涯、球员详情、比赛详情、伤病、紧急名单、事件与存档冲突全部改为中文界面。
- [x] 新增集中式界面词典，统一阶段、位置、轮换角色、球队角色、合同状态、选项类型、伤病、属性、特质、奖项、分区、赛区、金额与事件类别显示；内部枚举值和存档协议保持不变。
- [x] 虎扑 30 队城市名、扩军城市与品牌名改为中文；测试名单中的虚构球员姓名作为人物专名保留原名。
- [x] 服务层英文错误码和动态交易日志在 UI 出口统一转为中文，避免异常路径、合同动态和联盟新闻泄漏内部英文枚举。
- [x] `npm run typecheck` 通过；`npm test -- --run --testTimeout=120000` 为 30 个测试文件、87 项测试全通过；`npm run build` 通过，生产单文件 `h5/index.html` 为 547.51 kB（gzip 167.00 kB）。默认 30 秒时限在当前高负载环境下触发整季模拟超时，单文件与全套测试放宽时限后均通过。
- [x] 官方 Playwright 客户端覆盖创建球队、合同选项、季前名单、休赛期历史、重大伤病、紧急名单、冠军事件与常规赛场景，未生成 Console error 文件；浏览器无障碍树复查首页、阵容和比赛详情的所有界面标签均为中文。
- [x] `visual-verdict` 以原型适配后的上一轮截图为视觉基线，最终评分 97 / 100，通过 90 分门槛；结果保存在 `.omx/state/chinese-ui/ralph-progress.json`。

## Gemini 原型 V2.0 视觉还原（2026-09-17）

- [x] 以 `/Users/uwa/Downloads/gemini-code-1789619463016.html` 为唯一视觉基准，将 390×844 移动端外壳、米白背景、橙色主操作、圆角卡片、顶部工具栏、五项底部导航和底部抽屉完整应用到现有游戏。
- [x] 创建球队首屏按原型重排为 01/06 步骤结构、城市双选、球队名输入、四款队徽方案和底部主按钮；原有城市、品牌、配色与建队指令逻辑保持不变。
- [x] 存档/读档迁移至顶部“存/读档”抽屉，仍支持三个槽位；阵容、管理、联盟、生涯、扩军权益、合同选项、扩军交易、扩军选秀、伤病、事件、比赛详情和球员详情统一为同一视觉系统。
- [x] 官方 `develop-web-game` Playwright 客户端覆盖原型首屏、扩军权益推进、常规赛五个导航页、存档抽屉、伤病弹层及最终 `file://` 构建产物；状态 JSON 正常且未生成 Console error 文件。
- [x] `visual-verdict` 最终评分 92 / 100，达到 90 分通过门槛；结果保存在 `.omx/state/v2-prototype/ralph-progress.json`。剩余差异仅为本地 SVG 图标与浏览器中文字体的像素级渲染差异。
- [x] `npm run typecheck` 通过；`npm test -- --run --testTimeout=120000` 为 31 个测试文件、94 项测试全部通过；`npm run build` 通过，生产单文件 `h5/index.html` 为 2,865.54 kB（gzip 602.71 kB）。
- [x] 最终构建未引用外部脚本、样式或开发入口；发布 ZIP 根目录直接包含 `index.html`，与 `h5/` 的 41 个文件逐项一致，`unzip -t` 完整性校验通过。
- [x] 已覆盖 `release/篮球经理_扩军时代_V1.5_虎扑AI工坊.zip`，大小 2,542,904 bytes，SHA-256 为 `3e9744476df37ec3c6ed858ba0bdef6cbcb2b5ad97229029266fa5cf496dc554`。

## 本地静态预览与虎扑交付一致性（2026-09-17）

- [x] 本地和虎扑默认入口统一为发布包内同一个 `index.html`；本地可直接以 `file://` 双击打开，无需 Vite、HTTP 服务或 URL 测试参数。
- [x] 默认阵容统一读取构建包内的 `nba-player-dataset.json`，本地与虎扑均使用同一数据版本 `nba-api-2025-26-2026-09-17`；30 支现有球队各固定载入 15～18 名球员，排序、能力、潜力和合同均为确定性结果。
- [x] 移除默认启动对虎扑实时阵容接口的依赖，避免本地与虎扑因网络、接口更新时间或返回顺序产生可见差异；开发专用伤病、事件等 fixture 仍只在开发模式显式参数下启用。
- [x] 官方 Playwright 客户端直接打开最终 `file:///.../h5/index.html`，无需参数即可进入全中文“创建加盟球队”页面；状态为 `TEAM_CREATION`，无 Console error 文件。
- [x] 新增静态数据一致性测试；最终 `npm test -- --run --testTimeout=120000` 为 31 个测试文件、95 项测试全部通过，`npm run build` 通过。
- [x] 已再次覆盖 `release/篮球经理_扩军时代_V1.5_虎扑AI工坊.zip`；41 个文件与 `h5/` 逐项一致，`unzip -t` 通过，大小 2,540,631 bytes，SHA-256 为 `ca48d7a118dcd96eb3e6cb37d0692b59ac20c231c54a1f81900e28bdadf99988`。

## 原型结构级还原纠偏（2026-09-17）

用户反馈：上一轮偏重颜色与皮肤，页面模块、布局层级和原型交互结构没有逐页还原。本轮以此为缺陷而非新需求处理。

- [x] 补齐原型启动加载页与游戏首页，静态包本地和虎扑保持同一入口。
- [x] EX-01～EX-04 逐页采用原型结构：步骤标题、权益结果卡、球队选择抽屉、保护/未保护名单、双列交易资产卡。
- [x] 赛程页改为“球队摘要 + 月历 + 比赛日详情 + 双操作”布局，保留所有模拟能力。
- [x] 阵容页改为契合度摘要 + 原型球员卡列表；管理页改为交易/工资帽双分段；联盟页改为表格；生涯页改为奖杯评级卡。
- [x] 存读档改为自动存档卡 + 3 个槽位卡，球员详情、球队选择器、事件与比赛详情统一为原型底部抽屉。
- [x] 每轮通过官方 Playwright 截图、状态 JSON、Console 和 `visual-verdict` 对比，最终再跑 390×844、全量测试、构建和 ZIP 覆盖。

行为保护：保留队徽、真实球员静态数据、Engine / Command、存档 Schema、`data-testid` 和所有业务回调；只重排展示结构。

最终验证：`visual-verdict` 为 97 / 100；`npm run typecheck` 通过；31 个测试文件、95 项测试全通过。为同时满足本地 `file://` 直开与虎扑发布安全门禁，生产构建拆为 `h5/index.html` + 本地经典脚本 `h5/assets/game.js`，不包含可执行内联脚本、外域资源或绕过 SDK 的业务请求。最终 ZIP 根目录共 42 个文件，与 `h5/` 清单逐项一致；从 ZIP 解压后的 `file://` 截图与本地 `h5/index.html` 截图 SHA-256 完全一致。发布包大小 2,545,440 bytes，SHA-256 为 `3fb28fd3ef829bdd9140dc872ae510e18057dbf2bb88bbfaad5bc14146047b3c`。
## 2026-09-17：扩军抽签、交易桌与选秀信息优化

- [x] 权益抽签改为本球队在两份不同协议之间进行可复现随机抽取，并展示抽签动画与最终分配。
- [x] 扩军交易桌移除球员头像、压缩协议卡片，让报价区域独立纵向滚动，并把进入扩军选秀按钮固定在底部。
- [x] 扩军选秀球队选择器补全 30 支原球队；修复未保护球员姓名/合同信息截断。
- [x] 保护名单改为默认展开且可收起；选秀页显示当前扩军队工资、空间和阵容规模。
- [x] 增补领域测试，完成类型检查、单测、构建、浏览器交互与视觉验收，重新打包发布 zip。

### 完成验证

- 权益协议通过新 `DRAW_RIGHTS_PACKAGE` 指令按固定种子抽取；刷新、读档和指令重试不会重抽，另一份协议自动分配给电脑球队。
- 交易桌卡片无球员头像，报价区可独立上下滚动，底部确认操作保持固定；390×844 截图已人工检查。
- 球队抽屉实测包含东部 16 队、西部 14 队，并保留雄鹿、鹈鹕等当前无可选球员的球队入口与空状态。
- 保护名单默认展开且可收起；未保护球员完整展示中文名、位置、综合、年龄、工资和合同年限。
- 选中球员后，扩军队保障工资、工资帽占用、帽下空间和阵容人数实时联动；浏览器 Console 0 error。
- `visual-verdict` 评分 95/100，结果保存于 `.omx/state/expansion-compact-flow/ralph-progress.json`。
- 全量测试 32 个文件 / 100 项全部通过；10 个固定种子扩军 Headless 均完成 28 签、两队各 14 人；TypeScript 与生产构建通过。
- 发布包 `release/篮球经理_扩军时代_V1.8_协议抽签与选秀优化版.zip` 完整性检查通过，大小 5,420,070 bytes，SHA-256 为 `21f59786c4241b2f570ce2781049dc1e249203de9ea3223cc447afe86f0b00b8`。
## V1.10 — 2026–27 中文名单与数据口径

Current request (2026-09-17): 参考 `noRegrets` 的真实球员数据组织方式，让现役球员统一显示中文名；球队阵容归属严格使用 2026–27 赛季名单，不能回退到 2025–26 球队关系。

Plan:

1. 保留现有 NBA 官方 2026–27 roster snapshot 作为球队归属唯一来源；2025–26 只用于最近完整赛季的能力建模。
2. 引入 NBA 中国官网现役球员中文名快照，按 NBA Player ID 与 2026–27 名单精确绑定；不使用猜测式自动音译。
3. 参考 `noRegrets` 的分层思路，明确区分中文名来源、当前名单来源和能力数据来源，并补齐覆盖率测试。
4. 执行类型检查、构建、测试和移动端页面验证，再输出 V1.10 zip。

Completed:

- [x] 参考 `noRegrets` 的 Player ID / `name_zh` / ratings 分层方式，但没有复用其 2008–09、2011–12、2021–22 历史名单覆盖当前阵容。
- [x] 新增 NBA 中国现役名单中文名快照；与 NBA 官方 2026–27 roster 和本地能力数据按 NBA Player ID 精确合并。
- [x] 26–27 当前名单中 493 名具备能力数据的球员中文名覆盖 493/493；名单归属仍以 26–27 roster 为准，能力值明确为最近完整 2025–26 赛季模型。
- [x] 中文名已覆盖扩军交易桌、扩军选秀、球队阵容、球员详情、自由市场、交易报价、训练、伤病、奖项、名人堂和比赛技术统计。
- [x] 浏览器实测扩军交易桌显示“亚历克斯·卡鲁索”“CJ·麦科勒姆”“马拉基·布兰纳姆”等中文名，CJ 所属亚特兰大老鹰；底部锁定按钮正常可见。
- [x] `tsc --noEmit`、Vite build、32 个测试文件 102 项测试、扩军 headless 10 seeds、Stage 4 headless 10 seeds 全部通过。

## V1.11 — 直达首页与权益自主选择

Current request (2026-09-17): 本地数据不再需要伪加载页；取消扩军权益随机抽签和抽签动画，完整写明两份权益，让玩家自主选择后进入扩军交易桌。

Plan:

1. 默认入口直接进入主菜单，删除定时进度条、加载文案和 splash 状态。
2. 新建扩军球队后始终等待玩家选择方案，不再预抽赢家或为玩家随机分配协议。
3. 权益页改为两张可选择卡片，清楚展示扩军选秀、新秀首轮、新秀次轮顺位及适合策略；电脑球队自动获得另一方案。
4. 选择后通过原子命令完成权益分配、合同选项后台结算与交易桌准备；补充状态机测试并执行浏览器交互、截图、Console、全量测试和构建。

Completed:

- [x] 删除 splash、定时器、进度条和加载文案；默认入口现在直接显示“篮球经理：扩军时代”主菜单。
- [x] 移除 `DRAW_RIGHTS_PACKAGE` 随机指令与抽签动画；建队后两套方案均保持待选状态，由玩家明确选择。
- [x] 方案甲写明“扩军选秀第 1 顺位 / 新秀首轮第 6 顺位 / 新秀次轮第 38 顺位”，方案乙写明“扩军选秀第 2 顺位 / 新秀首轮第 5 顺位 / 新秀次轮第 37 顺位”，并补充适用策略说明。
- [x] 确认后玩家获得所选方案，电脑扩军队自动获得另一方案；合同选项后台结算并直接进入扩军交易桌，旧存档已结算方案仍可继续。
- [x] 官方 Playwright 客户端完成 390×844 权益页截图；浏览器实测选择方案乙、确认并进入交易桌，Console 0 error；默认入口确认无加载页。
- [x] `visual-verdict` 评分 94/100，结果保存于 `.omx/state/rights-choice/ralph-progress.json`。
- [x] `tsc --noEmit`、Vite 生产构建、32 个测试文件 102 项测试和扩军 headless 10 seeds 全部通过。

## V1.12 — 球员详情卡参考稿还原

Current request (2026-09-17): 删除球员详情底部的数据来源说明；球员卡片按照 `/Users/uwa/Downloads/gemini-code-1789639264532.html` 的样式还原。

- [x] 扩军交易桌、扩军选秀和常规阵容共用同一套参考球员卡片，避免不同页面继续出现两种详情样式。
- [x] 还原 380px 深色卡片、金色数据来源标签、姓名/球队/位置、渐变 OVR 徽章、四项基础资料、三项合同、双列能力条和三项底部特征。
- [x] 完整移除“中文名来自 NBA 中国现役名单……”等底部说明，也移除详情弹层中的虎扑资料同步提示与请求。
- [x] 常规阵容的角色设置和交易报价入口作为独立操作区保留，不改变原有经理功能。
- [x] 官方 Playwright 客户端分别生成参考稿和游戏实装截图；最终视觉结构、颜色、间距和层级一致，Console 未产生错误文件。
- [x] `visual-verdict` 评分 96/100，结果保存于 `.omx/state/player-card-v111/ralph-progress.json`。
- [x] 生产构建通过；全量测试在 120 秒长测上限下为 32 个测试文件、102 项全部通过；源码与构建产物均确认不再包含被删除的提示文案。

## V1.13 — 扩军电影序章与交易桌参考稿还原

Current request (2026-09-17): 开始新游戏后用电影方式介绍 NBA 扩军背景，明确森林狼转入东部、西雅图与拉斯维加斯两支新队加入西部；扩军交易桌按照 `/Users/uwa/Downloads/gemini-code-1789639653872.html` 还原，继续使用当前队徽。

- [x] “开始新游戏”改为先进入四幕电影序章：联盟 30→32、森林狼西部→东部、西雅图与拉斯维加斯加入西部、玩家接掌扩军球队。
- [x] 序章支持 5.2 秒自动推进、章节点击、继续按钮和跳过序章；最终进入原有创建球队流程，读取存档与继续游戏不重复播放。
- [x] 序章使用现有球馆背景、森林狼正式队徽和两支扩军队默认队徽，并提供减少动态效果兼容。
- [x] 扩军交易桌按参考稿还原标题、已接受徽章、三项统计、橙色提示、三段筛选、报价卡、合同三列、说明条和按钮比例；球队继续显示当前真实队徽。
- [x] 保留此前要求的独立报价滚动区与固定底部提交卡，720px 和 844px 高度均不会遮挡报价按钮。
- [x] 浏览器完整验证开始游戏→电影序章→创建球队，以及筛选报价、查看球员、接受协议；接受后计数 0/5→1/5、报价 30→29。
- [x] 新增联盟重组测试，确认森林狼为东部、两支扩军队为西部，东西部各 16 队。
- [x] `visual-verdict` 评分 95/100，结果保存于 `.omx/state/v113-cinematic-trade/ralph-progress.json`。
- [x] TypeScript、生产构建、32 个测试文件 103 项测试和扩军 headless 10 seeds 全部通过。

## V1.14 — Cyber Terminal 全局主题与建队体验修正

Current request (2026-09-17): 全局统一为赛博高科技 / 战术终端风；球队命名显示简明规范并修复纯数字名称无法进入游戏却无提示的问题；首页增加背景图；扩军交易桌直接显示球员综合。

- [x] 在 `:root` 建立页面、卡片、文字、边框、霓虹青、毒液绿、日志栏、圆角和字体 Token，并把旧版变量映射到新主题。
- [x] App Bar、底部导航、卡片、数据格、抽屉、状态栏、主次按钮、表单和扩军流程统一为深蓝半透明战术终端视觉。
- [x] 首页使用电影序章球馆图作为背景，增加暗色渐变、系统标识和霓虹青主操作层级。
- [x] 建队页直接显示 2～20 字符规则；输入 `111` 时显示“球队名称不能仅由数字组成”、标记无效并禁用创建按钮。
- [x] 扩军交易桌在每份协议的球员姓名旁显示独立“综合”高亮徽章，无需打开球员详情。
- [x] 官方 Playwright 客户端完成首页、建队页、交易桌截图；浏览器实测纯数字校验；`visual-verdict` 94/100。
- [x] TypeScript 与 Vite 生产构建通过；32 个测试文件 103 项测试全部通过；扩军 headless 10/10 固定种子通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.14_赛博终端主题版.zip`；48 个条目完整性检查通过，SHA-256 为 `8a33306772e13a2b288ba01a86709f9db6406bf6b921d2d8ee1abfe72213411a`。

## V1.15 — 扩军选秀大会参考稿还原

Current request (2026-09-17): 扩军选秀使用 `/Users/uwa/Downloads/gemini-code-1789641341737.html` 的 UI。

- [x] 按参考稿重构为“标题与签位进度 → 扩军球队薪资卡 → 实时播报 → 保护名单 → 紧凑选秀池”的纵向信息结构。
- [x] 进度徽章实时显示 0/14～14/14；薪资卡同步显示保障工资、工资帽占用、帽下空间与联盟工资帽。
- [x] 保护名单默认展开、支持折叠并在固定高度内滚动；球员卡直接展示中文名、位置、综合、年龄、年薪和剩余合同。
- [x] 保留项目原有的 30 队切换抽屉，并压缩为参考稿风格的终端控制条。
- [x] 浏览器实测保护名单折叠、切换至老鹰、选择球员；签位 0/14→1/14，保障工资与帽下空间同步更新。
- [x] `visual-verdict` 94/100，结果保存于 `.omx/state/v115-expansion-draft/ralph-progress.json`。
- [x] TypeScript、生产构建、32 个测试文件 103 项测试和扩军 headless 10/10 固定种子全部通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.15_扩军选秀大会还原版.zip`；48 个条目完整性检查通过，SHA-256 为 `291d9820291a14f32188e703af8fdb1ef57023bedf5f8c65d15bb5eef66434ce`。

## V1.16 — 赛博球队选择弹窗与自动跳队

Current request (2026-09-17): 扩军选秀大会的球队选择页面使用指定参考 UI；一个球队选完后自动跳到下一支球队继续选择。

- [x] 用户给出的 `gemini-code-1789643348295.html` 不存在，采用 Downloads 中时间戳最接近的 `/Users/uwa/Downloads/gemini-code-1789643350232.html` 作为参考稿。
- [x] 球队选择弹窗还原 SYS 终端标题、青色切角、双联盟列、状态指示灯、选中高亮、禁用球队和 30 队滚动列表。
- [x] 使用当前真实队徽与球队中文名；无可选球员的球队灰化并禁止选择。
- [x] 从当前球队选中球员后，按照东部→西部的球队顺序自动跳转到下一支仍有可选球员的球队。
- [x] 浏览器实测凯尔特人选人后签位 0/14→1/14，当前球队自动变为黄蜂；凯尔特人节点禁用，黄蜂节点高亮。
- [x] `visual-verdict` 97/100，结果保存于 `.omx/state/v116-team-selector/ralph-progress.json`。
- [x] TypeScript、生产构建、32 个测试文件 103 项测试和扩军 headless 10/10 固定种子全部通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.16_球队选择与自动跳队版.zip`；48 个条目完整性检查通过，SHA-256 为 `1deb9e159746d9a07a05340346a939438ff2a66f6a2828efe338a8415efd8933`。

## V1.17 — 真实球员 OVR 分层校准

Current request (2026-09-17): 删除球员详情中的“NBA 官方当前名单”；参考 noRegrets 的能力实现，修正杰伦·布伦森仅 69 等明显失真的综合能力。

- [x] 根因确认：旧模型把八项属性等权平均，导致控卫的内防、篮板与组织、投射权重完全相同；联盟最高仅 83.12、90+ 为 0，不符合冻结文档要求。
- [x] 五个位置改用独立属性权重；OVR 采用 noRegrets 已验证的赛季产量分层公式（得分、篮板、助攻、出场时间），八项属性仍作为比赛模拟输入。
- [x] 真实球员保存生产层级校准差值，后续成长和衰退仍能随属性动态变化；旧存档与程序化球员没有校准值时自动沿用属性 OVR。
- [x] 杰伦·布伦森 69→92；约基奇 96、亚历山大 95、字母哥 94、东契奇 96、库里 91、塔图姆 90。
- [x] 582 名球员平均 OVR 75.4，90+ 共 25 人，满足联盟平均 68～76、90+ 约 5～25 人的平衡目标。
- [x] 球员详情移除“NBA 官方当前名单”，数据来源标签统一改为中性的“离线球员资料”。
- [x] 32 个测试文件 / 106 项测试、TypeScript 与 Vite 生产构建、扩军 10/10 固定种子全部通过。
- [x] 官方 Playwright 客户端打开扩军选秀球员详情；截图确认旧标签已移除、OVR 正常显示，`render_game_to_text` 与页面一致且无 Console error。
- [x] 已生成 `release/篮球经理_扩军时代_V1.17_真实球员能力校准版.zip`；48 个条目完整性检查通过，SHA-256 为 `3e767d957ec5af2f02a32e25155cff2aaf06f1646d95e335ee8701336fb18634`。

## V1.18 — NBA 2K27 Top 100 OVR 对齐

Current request (2026-09-17): 获取 NBA 2K 球员能力值与项目比较，并尽量对齐。

- [x] 使用 NBA 2K27 官方 Top 100 页面，不以第三方评分站作为权威来源；快照记录游戏版本、发布日期、抓取日期和官方 URL。
- [x] 官方 100 人中 97 人与项目 2025–26 能力数据精确匹配并完全对齐；哈利伯顿、欧文、利拉德因没有 2025–26 比赛样本，未伪造八项能力强行补入。
- [x] 对齐前 97 人平均绝对误差 2.26，64 人误差不超过 2；对齐后已匹配球员误差为 0。
- [x] 布伦森 92→96、文班亚马 91→97、约基奇 96→97、亚历山大 95→97、塔图姆 90→93、卡鲁索 73→81。
- [x] Top 100 之外继续使用 noRegrets 风格的赛季产量分层；八项属性仍是比赛模拟输入，OVR 只作为 UI 摘要与经营判断。
- [x] 对齐后 582 人平均 OVR 75.36、90+ 共 20 人，仍满足冻结平衡目标。
- [x] 新增可重复运行的官方榜单校准流程、数据源标记和对齐报告；未来更换榜单快照后无需改业务代码。
- [x] 32 个测试文件 / 107 项测试、TypeScript、Vite 生产构建、Python 构建器语法与扩军 10/10 固定种子全部通过。
- [x] 官方 Playwright 客户端打开球员卡片，截图与 `render_game_to_text` 的 OVR 一致，Console error 为 0。
- [x] 已生成 `release/篮球经理_扩军时代_V1.18_NBA2K27能力对齐版.zip`；48 个条目完整性检查通过，SHA-256 为 `d6363352e8b2cecc98b7f1a8ca50d1045dc6fcf6b544b7acbe4e50053ec9d18b`。

## V1.20 — 扩军选秀当前阵容视图

Current request (2026-09-18): 扩军选秀大会页面需要能看到当前扩军球队的阵容，以便结合已有球员和位置缺口做选择。

- [x] 在扩军选秀池上方增加默认展开、可收起的当前阵容面板；面板位于球员池滚动区外，选人后仍保持可见。
- [x] 实时展示五位置人数，以及已选球员的中文名、位置、综合和年薪。
- [x] 已选球员可直接打开现有球员详情，空阵容提供明确引导。
- [x] 32 个测试文件 / 108 项测试、TypeScript、Vite 生产构建与扩军 10/10 固定种子全部通过。
- [x] 官方 Playwright 与应用内浏览器完成真实选人链路：0/14→1/14、PG 0→1、薪资实时更新并自动跳到下一支球队；Console error 为 0。
- [x] `visual-verdict` 95/100；结果保存于 `.omx/state/v120-current-roster/ralph-progress.json`。
- [x] 已生成 `release/篮球经理_扩军时代_V1.20_选秀阵容视图版.zip`；48 个条目完整性检查通过，SHA-256 为 `a45ab50ebcb355d4ca87e62b6f4f2e246b95b076eb5ee5530db3648c60cb0cad`。

## V1.21 — 2026 真实新秀选秀重演

Current request (2026-09-18): 将开局阵容恢复到 2026 选秀前；普通球队优先按真实选秀结果选人，扩军球队可以截胡；被截胡后由电脑球队按剩余真实榜单顺延补位，且球员不能重复归属。

- [x] 建立 2026 年 60 名真实新秀、原始签位、最终签权归属与中文名数据。
- [x] 从开局 2026–27 阵容移除整届选秀池球员，避免选秀前已在队与重复归属。
- [x] 首届选秀改为 80 人真实优先选秀池、64 个签位和 5/6、37/38 扩军签位。
- [x] 普通球队按真实结果执行；目标已被扩军队截胡时，按剩余真实榜单顺延补位。
- [x] 增加真实顺位、截胡级联、唯一归属、16 名落选秀与未来赛季回归程序化的回归测试。
- [x] 新秀选秀页补充真实结果与截胡规则；浏览器实测截胡科阿·皮特后从 #6 自动推进到 #38，控制台无错误。
- [x] TypeScript、Vite 生产构建、32 个测试文件 / 109 项测试与 Stage 4 无头 10/10 固定种子全部通过。
- [x] 已生成 `release/篮球经理_扩军时代_V1.21_2026真实选秀重演版.zip`；48 个条目完整性检查通过，SHA-256 为 `956864f5813ea39e26e72c492d125290fb586b622b22d091ac9a7fce24ca32fc`。

## V1.33 — 真人头像精灵图发布包

Current request (2026-09-18): 在保留全部真人头像的前提下，将发布包文件数降至虎扑 300 个文件限制以内。

- [x] 650 张 NBA 官方真人头像保留在 `tools/data/portrait-source/`，不再逐张复制进发布 H5。
- [x] 新增可重复运行的 `npm run data:build-portrait-atlas`：将头像按 NBA Player ID 稳定排序，生成 26 张横向图集以及运行时 ID 映射；球员详情卡以 CSS 背景定位裁切对应头像。
- [x] 头像数据同步完成后自动重建图集；项目运行时不请求 NBA 或 2K 外网接口。
- [x] 新增图集映射单测；TypeScript、生产构建、扩军 10/10 与 Stage 4 10/10 无头回归通过。
- [x] 仅保留 `release/篮球经理_扩军时代_V1.33_头像精灵图版.zip`：75 个条目、26 张图集、0 张逐张头像、6.6 MB。
Current request (2026-09-20): 将用户提供的 `5_player_salary_history_from_2026_27.xlsx` 转为本地 JSON，并接入当前游戏的初始合同数据。

### 2026-27 薪资与合同快照 — complete

- [x] 新增可复跑导入器 `tools/data/import_2026_27_salary_contracts.py`：读取用户提供工作簿的“球员历年薪资”页，将“万”美元单位转换为美元整数，保留逐年薪资、球队/球员选项、RFA/UFA 到期标记与来源 ID。
- [x] 生成版本化本地数据 `src/data/nba-2026-27-contracts.json`。436 名球员已通过 NBA ID、中文名和球队快照唯一对齐（343 自动、93 经源码内显式 ID 校验）；11 名无法唯一匹配或缺少当前能力档案的源行作为审计记录保留，绝不按同姓猜测套用。
- [x] 根据用户确认补齐猛龙小安德烈·杰克逊（1 年 241 万美元）与独行侠小莫里兹·约翰逊（4 年 3,271 万美元）；后者原先缺少能力档案，已补入基于本地 NBA 2K27 快照的离线补充档案，因此会实际进入达拉斯的初始阵容。
- [x] 根据用户确认补齐马刺 Maliq Brown（1 年 68 万美元）和骑士 Meleek Thomas（4 年、末年球队选项）。两人均已有本地 NBA 2K27 完整评分；此前只因主能力数据集遗漏而未入档，现已补入离线档案并在当前赛季名单替换时去除旧选秀池残留。
- [x] 根据用户确认，国王 Dylan Cardwell 与 76 人 Kentavious Caldwell-Pope 不再碰撞；同时拆分 Jalen/Caleb Wilson、Cormac Ryan/Ryan Rollins、Jayden Quaintance/Derik Queen。合同快照不再存在 `duplicate_target` 记录；相关数据测试 19/19 与生产构建通过。
- [x] 默认离线建档已使用这份合同快照：命中球员带入 `salaryByYear`、`guaranteedByYear`、`optionByYear`、合同起止赛季与来源合同 ID；未匹配球员继续使用原有安全合同兜底。示例：坎宁安 4 年 2.14 亿美元，里夫斯 4 年 1.7999 亿美元。
- [x] 通过 17 项名单/球员数据/合同生命周期测试，`npm run build` 通过并同步静态 `h5` 产物。Vite 仍提示单 bundle 体积超过 500 kB（既有静态单文件发布结构）。

## 存档槽位新建流程

Current request (2026-09-20): 新游戏开始前先选择存档槽位，使后续保存位置明确。

- [x] 首页“开始新游戏”改为打开三个生涯槽位选择页，而非直接进入建队流程。
- [x] 空槽位可直接创建新生涯；已存在存档的槽位会展示球队、赛季、日期、战绩和阶段，并需二次确认覆盖。
- [x] 新建时将所选槽位传入游戏会话，后续自动保存与手动保存默认写入该槽位。
- [x] 已重新生成唯一发布包 `release/篮球经理_联盟扩军时代.zip`，包含该流程；`unzip -t` 通过，SHA-256 为 `88ccc02d869b032538ae17575ff28fa71c9e03df6ed647a6c5c0d3bb62a8e6d6`。

## 真实球员双位置修正（已被后续 2K 位置规则取代）

Current request (2026-09-20): 修正雄鹿迈尔斯·特纳被显示为大前锋/小前锋的问题，并说明当前球员位置的生成方式。

- [x] 确认主位置由离线 NBA 数据模型按身高和比赛数据推断，真实球员副位置此前错误复用了虚构球员的相邻位置随机规则。
- [x] 真实球员建档现在优先解析官方阵容快照中的双位置组；官方未提供双位置时才保留原有确定性兜底。
- [x] 此阶段曾将迈尔斯·特纳合并为 PF/C；后续接入 2K 主副位置后，已按用户要求改为 2K 快照的单位置 `C`。
- [x] 定向球员数据/名单测试 18/18、完整测试 35 个文件 / 128 项测试与 TypeScript/Vite 生产构建全部通过。

## NBA 2K 主副位置数据接入

Current request (2026-09-20): 将 NBA 2K 的 PG/SG/SF/PF/C 主副位置正式写入球员数据集，并增加合法性校验与人工覆盖表。

- [x] 球员数据集升级到 schema v2，正式保存 `position`、`secondaryPosition`、`positionSource` 与版本化 `positionModelVersion`。
- [x] 位置同步优先级统一为“人工覆盖 > 合法 NBA 2K 主副位置 > 原有推断兜底”；单位置球员不再随机生成虚构副位置。
- [x] 新增版本化人工覆盖表，修正 2K 快照中的重复位置和非法 `S` 位置码；迈尔斯·特纳按后续要求直接采用 2K 的单位置 `C`，不再人工覆盖。
- [x] 严格校验位置数量、五位置枚举、重复主副位置、覆盖目标 NBA ID/姓名和覆盖原因；错误数据会中止同步而非静默写入。
- [x] 新增无需联网的 `npm run data:sync-player-positions`，主数据集与补充名单共用同一位置管道。
- [x] 主数据集 625 人直接采用 2K、3 人采用覆盖表、22 人因当前 2K 快照无对应记录而保留推断；补充名单 9 人采用 2K、1 人采用覆盖表、0 人需要推断。
- [x] 数据管道测试 7/7、完整测试 35 个文件 / 129 项测试、TypeScript 与 Vite 生产构建通过；并行验证时赛季慢测试曾因 CPU 竞争超时，串行复跑 4/4 通过。

Current request (2026-09-20): 迈尔斯·特纳严格采用 2K 位置，并列出仍使用推断位置的球员。

### 特纳 2K 位置与推断名单核对 — complete

- [x] 本地 NBA 2K27 快照将迈尔斯·特纳标为单位置 `C`；已删除其人工 PF/C 覆盖并重新同步数据集。
- [x] 特纳的数据集位置现为 `C`、副位置 `null`、来源 `NBA2K`；人工覆盖仅保留用于修复非法或重复 2K 位置值的条目。
- [x] 重新核对 22 名 `INFERRED` 球员，均为当前 2K 快照无名称匹配的单位置记录，未发现非法或重复位置。
- [x] 数据管线 7/7、名单与数据集定向测试 19/19、生产构建通过；全量 129 项中完整赛季用例因 30 秒阈值超时，隔离串行复跑 4/4 通过。

## 扩军选秀球队筛选与球员池展示 — complete

Current request (2026-09-20): 扩军选秀大会支持“全部”联盟球队并默认选中；球员信息显示所属球队；球员按综合能力从高到低排序；全部视图隐藏保护球员，选择具体球队后才展示保护名单。

- [x] 新增可复用的扩军选秀球队/位置筛选与 OVR 降序排序逻辑；同 OVR 使用稳定球员 ID 作为次序，避免渲染抖动。
- [x] 选秀页默认球队改为“全部联盟球队”，选择器加入全部入口；全部视图汇总 30 支球队的未保护球员，单队视图保留原有自动跳转逻辑。
- [x] 全部视图不渲染保护名单；切换具体球队后渲染默认收起的保护名单，并按 OVR 排序；切回全部后保护区消失。
- [x] 球员行补充球队名称，保留位置、综合、年龄、年薪和剩余年限字段；空态文案区分全部筛选与单队筛选。
- [x] 定向扩军筛选测试通过；TypeScript、Vite 生产构建与 `git diff --check` 通过。构建仍仅提示既有单 bundle 体积警告。
- [x] Playwright 实测 `/app.html?fixture=expansion-draft`：默认全部视图 263 人，前三名 OVR 为 90/85/84，全部列表单调不升且每行含球队名；保护区数量为 `0 → 1（单队默认收起）→ 0`；Console error 为 0。移动端 390×844 截图保存于 `output/expansion-draft-team-mobile.png`，桌面首屏截图保存于 `output/expansion-draft-all-final/shot-0.png`。
- [x] 全量回归串行结果：34 个测试文件 / 130 项通过；唯一默认失败为既有 `src/game/season/career.test.ts` 长测的 30 秒阈值超时，单独以 60 秒阈值复跑后 4/4 通过。扩军固定种子无头回归 10/10 通过。
- [x] 已重新构建并覆盖 `release/篮球经理_联盟扩军时代.zip`；包含 79 个条目，`unzip -t` 通过，SHA-256 为 `cefce9d298bc90fee72573a43ebbe541c2f3eebe21bd8516095dce75708fdb53`。

## 存档/读档弹框宽度统一

Current request (2026-09-21): 将首页存档弹框与游戏内存档弹框统一为首页“开始新游戏”按钮的移动端内容宽度。

- [x] `home-load-menu` 与 `save-drawer` 统一为 `min(320px, calc(100vw - 70px))`，在 390px 手机宽度下与首页操作按钮同宽，并保留居中显示。
- [x] TypeScript 检查与 Vite 生产构建通过；已覆盖正式 ZIP，79 个条目、`unzip -t` 通过，大小 7,868,225 bytes，SHA-256 为 `26ce71af2d00998bcee50d8bcf9182710cf556c4238f12212604f20ef07c90b9`。

Current request (2026-09-21): 扩军选秀大会当前球队阵容的位置筛选按钮直接显示各位置球员数量。

- [x] 移除位置筛选上方独立的 PG、SG、SF、PF、C 人数块，改为在“全部 / PG / SG / SF / PF / C”筛选按钮内显示人数。
- [x] 数量按球员第一位置统计，筛选逻辑与选秀池一致；保留当前筛选高亮和空位置提示。
- [x] 已同步静态 `h5` 产物并覆盖正式包 `release/篮球经理_联盟扩军时代.zip`。

Current request (2026-09-21): 手机端新秀选秀准备页面需要真正铺满 WebView 可用宽度。

- [x] 新秀选秀准备页移除外层左右安全边距，让页面终端卡片边到边显示。
- [x] 卡片内部文字、指标和按钮继续使用安全区内边距；原生 App 顶部栏与底部 Home 指示条不属于网页，保持由宿主控制。
- [ ] 尚未完成真实手机截图验收；需在虎扑 App WebView 中确认最终视觉边界。

Current request (2026-09-23): 核对其余球员照片是否串人，解释并修复头像黑底。

### 球员头像来源与透明背景

- [x] 635/635 图集格与对应 PNG 原图逐像素近似比对通过；透明通道误差为 0。
- [x] 621 张 NBA 来源 PNG 与同球员 ID 的 NBA 官方 CDN 文件逐文件 SHA-256 一致；14 张补充照片的来源姓名与名单姓名一致，来源页记录在 `tools/data/portrait-source/supplemental-portrait-sources.json`。没有逐人进行脸部人工辨认。
- [x] JPG 图集把 PNG 透明区域填成黑色；改为保留透明通道的 WebP 图集，并移除发布目录旧 JPG。
- [x] 球员卡片本地 Playwright 截图已查看，透明区域显示卡片底色；头像定位测试 5/5、生产构建通过。
- [x] 全量测试 154/154 通过；正式 ZIP 已覆盖，73 个条目、26 张透明 WebP 图集、无旧 JPG，CRC 校验通过。

Current request (2026-09-23): 球员照片背景使用深蓝灰 `#243B53`。

### 头像背景颜色统一

- [x] 在 `src/styles.css` 增加共用 `--portrait-bg`，球员详情卡与选秀列表真人头像共同使用；默认占位头像维持原有处理。
- [x] TypeScript/Vite 生产构建及 `git diff --check` 通过；Playwright 分别查看自由球员详情卡和新秀选秀列表截图，无新控制台错误。
- [x] 已覆盖 `release/篮球经理_20260923球员名单更新.zip`；73 个条目，CRC 校验通过，SHA-256 `bed946cb7c925dd4f1ab6bf4c09aa20026a8d696fc19f439836727cb9fc36de0`。

Current request (2026-09-23): 选秀后结果页去掉外边框，西雅图和拉斯维加斯队徽在所有页面保持圆形。

### 选秀结果与扩军队徽

- [x] 选秀后 `post-draft-terminal` 移除外边框、圆角和外发光；内部模块边框保留。
- [x] 扩军队徽图片统一圆形裁切，解决默认 PNG 白色方角；SEA/LVG 在赛季页面的图片或文字回退徽标均使用圆形容器，扩军总结页圆形容器同步处理。
- [x] Playwright 截图核对选秀后主页面、完整选秀结果第 5/6 顺位和赛季页面文字回退徽标；生产构建与 `git diff --check` 通过。
- [x] 已覆盖 `release/篮球经理_20260923球员名单更新.zip`；73 个条目、CRC 校验通过，SHA-256 `97189a01fafb2b83fbae69d4ae907b755d2d95fb123a0695f4018f39175050b8`。

Current request (2026-09-23): 自由球员市场的联盟动态遮盖球员，季前页面去掉外边框。

### 自由市场与季前页面布局

- [x] 自由球员列表在滚动容器里被 Flex 压至 100px，卡片溢出后覆盖联盟动态；列表改为按内容占位，使动态排在最后一张球员卡之后。
- [x] 季前页面最外层去掉边框、圆角和阴影，保留内部模块边框。
- [x] Playwright 在 390px 宽页面结算一天后确认最后一张球员卡底部 6077px、联盟动态顶部 6088px；两页截图已查看，无新增控制台错误。
- [x] 生产构建及 `git diff --check` 通过；完整测试 153/154 通过，唯一赛季循环用例触及默认 30 秒超时，单独以 90 秒超时重跑 4/4 通过。
- [x] 已覆盖 `release/篮球经理_20260923球员名单更新.zip`；73 个条目、CRC 校验通过，SHA-256 `acea0e87afd26db174a776ccdcc9393f4203c1594b125e61f40e25bc00a6c18d`。

Current request (2026-09-23): 解释球员耐伤、性格、市场偏好的来源，并核查选秀头像黑块与位置占位。

### 球员隐藏属性与选秀头像

- [x] 核对数据路径：2K 全量资料球员的耐伤来自 `Overall Durability`；缺 2K 的球员使用固定 ID 推算值；受伤的虎扑在队条目开档耐伤直接设为 25。性格与市场偏好由球员 ID 和开档种子生成，属于游戏模拟值。
- [x] 本地 2026 选秀预览共 80 人，23 人有离线照片、57 人显示位置占位；其中 60 名正式选秀球员有 20 人有照片。现有图集的 20 张正式新秀图片均非全黑空图。
- [x] 选秀列表头像增加图集加载失败回退；浏览器拦截图集请求后头像显示 SG 等位置字母，不再留下深色空块。正常图集截图仍显示真人照片。
- [x] 生产构建、相关测试 18/18、`git diff --check` 通过；正式 ZIP 已覆盖，73 个条目、CRC 校验通过，SHA-256 `30d42d9a19e350691962189d370a76e023f8c918d211b7d704c04568313ef43d`。

Current request (2026-09-23): 性格和市场偏好应有实际计算与影响；所有无照片球员统一使用默认无头像样式。

### 市场偏好与默认头像

- [x] 核对冻结规范：`market_preference` 是对大城市曝光的偏好强度，报价 Market Preference 权重为 3；原实现用球队缩写哈希代表市场，球队 `marketRating` 也按缩写哈希生成。
- [x] 改为由球员性格和开档年龄计算 0～100 的大市场偏好，并用球队明确的城市市场评分计算匹配；六种性格通过总权重保持 100 的调整改变自由球员报价取舍。旧存档加载时重算偏好与市场评分。
- [x] 选秀名单无照片或图集加载失败统一显示与球员详情同款默认剪影，不再显示 PG/SG 等位置字母。
- [x] 浏览器核对选秀列表默认剪影、图集请求失败回退、球员详情“大市场偏好”字段；无新增控制台错误。
- [x] 生产构建、完整测试 159/159、`git diff --check` 通过；两组固定种子的选秀→自由市场→交易→名单锁定流程通过。
- [x] 已覆盖 `release/篮球经理_20260923球员名单更新.zip`；73 个条目，CRC 校验通过，SHA-256 `327ea20c63091bbd40bc93f26be1f40ca1183f94eed6bbdf44f711f225c363e4`。
- [ ] 尚未按平衡规范完成 20 Seeds × 30 Seasons 与至少 1000 Season equivalents 的长期分布校准；城市市场评分目前是明确的游戏调参值，不是人口或营收统计。

Current request (2026-09-23): 新秀选秀页面与主游戏、扩军选秀页面统一视觉样式。

### 新秀选秀页面样式统一

- [x] 将选秀准备页和正式选秀页改为扩军选秀同款深蓝直角面板、青色描边与主按钮；移除正式选秀页的霓虹紫色控制台元素和无操作的“选秀夜交易”按钮。
- [x] 保留圆形球队徽标、候选球员默认头像、独立滚动列表和底部操作区；顺位状态按等待、模拟、我方选择准确显示。
- [x] 对比扩军选秀页，检查 390px 手机和 1280px 桌面截图；准备页进入选秀、位置筛选、详情弹层、模拟至我方顺位与选人推进均通过浏览器验证，未出现页面脚本错误。
- [x] 生产构建（含 TypeScript 检查）、选秀服务测试 13/13、`git diff --check` 通过；正式 ZIP 已覆盖，73 个条目、CRC 校验通过，SHA-256 `50bbaaa402f50152dbbddb1e34a015f78bdae156189a0c08ec45a0a0574b859c`。

Current request (2026-09-24): 解释自由球员市场的“期望年薪”，并修正已退役的威斯布鲁克仍在市场的问题。

### 自由球员报价说明与退役名单

- [x] 核对报价公式：自由球员列表展示 `getRecommendedFreeAgentOffer` 的第一年建议报价，基于 `publicPlayerValue`、`valueFloor=48`、每价值点 85 万美元，受最低/最高工资限制并按 1 万美元取整；它不是球员公开报价，也不是薪资合同工作簿的签约薪资。
- [x] 自由球员卡片文案从“期望年薪”更正为“建议年薪”。
- [x] NBA 官方 2026-08-12 退役公告确认拉塞尔·威斯布鲁克退役；本地自由球员追踪快照仍含 `201566`，因此误入开档市场。增加带来源编号的退役名单，从快照及投影数据移除他，并在后续导入、名单校对中排除。
- [x] 旧版 bundled 存档中仍为未签约自由球员的威斯布鲁克，加载时从球员、市场与报价引用中移除；若已被玩家签入球队，保留存档中的游戏进程。
- [x] 新开档自由球员 55 人、相关 Vitest 40/40、导入器测试 3/3、生产构建和 `git diff --check` 通过。
- [x] 已覆盖 `release/篮球经理_20260923球员名单更新.zip`，73 个条目、CRC 校验通过，SHA-256 `e2d040543442561e3e38a249d8a86e4bde1ca85e7bee3dcdd7dd49ded2b110d1`。

Current request (2026-09-24): 核对默认头像、自由球员身份、凯兰·博斯韦尔 2K 能力来源，并修正过高的自由球员建议年薪。

### 自由球员定价校准

- [x] 默认无照片头像核查：球员详情沿用青色剪影；选秀候选列表近期从黑块/位置字母占位统一为同款剪影，未在本次修改头像样式。
- [x] 开档自由球员快照 55 人均有 NBA 球员 ID；其中 50 人有完整 2K27 数据，5 人使用引擎估算。首届选秀后的市场还会加入未被选中的真实 2026 候选球员；后续赛季可出现程序生成球员。
- [x] `nba:1643573` 凯兰·博斯韦尔的 68 OVR 来自本地 NBA2K API 第三方快照（`NBA_2K27_FULL_PROFILE`）；NBA 官方显示其与夏洛特黄蜂签订双向合同。
- [x] 复现旧定价：68 OVR、21 岁博斯韦尔建议首年约 2982 万美元且被承诺首发。根因是把含年龄和零合同加分的交易价值直接换算年薪。新增回归测试先复现 3009 万美元失败。
- [x] 将自由球员建议年薪改为按当前 OVR 分档线性插值，年龄仅 ±5% 微调，仍遵守引擎最低/最高工资；建议和 AI 承诺角色按 OVR 判断，球员选项共用同一定价函数。版本更新为 `balance.v6-free-agent-pricing` / `game-config.v5`。
- [x] 调整后博斯韦尔建议首年 211 万美元、轮换角色；开档自由球员 70～74 OVR 建议薪资中位数约 360 万美元。完整测试 161/161、最终相关测试 11/11、构建、两组固定种子贯穿至第二赛季经理流程和 `git diff --check` 通过。
- [ ] 本次未完成 20 Seeds × 30 Seasons 和 1000 Season equivalents 的长期薪资分布校准；旧存档里已经提交的合同报价会按原值保留。
- [x] 已覆盖 `release/篮球经理_20260923球员名单更新.zip`，73 个条目、CRC 校验通过，SHA-256 `bc9d2fa64c8731dc09f307f5c608072b54b3a1378e3eea6261c55c2e500c2fc4`。

Current request (2026-09-24): 扩军选秀轮次提示、玩家协议签位停留、电脑球队选人动态与选秀结果页全宽。

- [x] 玩家所有扩军签位均停留等待确认；交易协议指定签位只允许确认指定球员。最近选秀动态明确列出两支扩军队的选择。
- [x] 操作提示改为中文并显示当前玩家签位；30 支老球队的筛选器统一命名为“球员来源球队”，不再误称全联盟。
- [x] 选秀结果页面全宽、无外边框；390px 与 1280px 视口实测无横向溢出。
- [x] 全量测试 162/162、10 个种子扩军流程、生产构建和视觉核对通过。
- [x] 已覆盖 `release/篮球经理_20260923球员名单更新.zip`，73 个条目、CRC 校验通过，SHA-256 `6cd98b51923cdad8f501652b22c85a8c5981b7c6aad067f3e621433c591c78e0`。

Correction (2026-09-24): 用户指出需要加宽的是“选秀后休赛期”页面，不是“查看完整选秀结果”弹层。

- [x] 结果弹层恢复居中、最多 430px 的原宽度；选秀后休赛期内容去掉多余的 16px 内侧横向留白，铺满页面 12px 安全边距内的可用宽度。
- [x] 390px 和 1280px 浏览器量测与截图核验均无横向溢出；生产构建和 `git diff --check` 通过。
- [x] 已重新覆盖 `release/篮球经理_20260923球员名单更新.zip`，73 个条目、CRC 校验通过，SHA-256 `0d772f3b650a89982ad416f5ed1137ee9f1d672d73c7e0aa1c54a36e94303d63`。

Current request (2026-09-24): 球龄来源纠错与自由市场 16 人名单误导。

- [x] 真实球员开档球龄改为优先使用 NBA 官方资料的已完成赛季；泰·吉布森按 2009 年首季校正为 17 年，旧存档一次性迁移，后续游戏球龄继续增长。资料缺失时按年龄估算并在球员卡标记“球龄（估）”，不再对真实球员随机分配。
- [x] 自由市场正确展示休赛期 21 人上限与常规赛 15 人上限；开档 16 人可报价，签约可跳过，待定报价退出时明确提示撤回。
- [x] 浏览器验证 16/21 时 62 位自由球员报价按钮可用；完整测试 167/167、10 种子贯穿第二赛季经理流程、生产构建、视觉核对、`git diff --check` 通过。
- [x] 原先 435 名开档真实球员缺少官方球龄字段的问题已完成逐人核查；离线官方球龄快照现覆盖全部 570 名开档真实球员（含自由球员），年龄估算为 0 人。
- [x] 已覆盖 `release/篮球经理_20260923球员名单更新.zip`，73 个条目、CRC 校验通过，SHA-256 `c5aec5b6b8d989edb969a869b600e6ed85d3d92f4f8ea2cfb89a39577e4edf98`。

Current request (2026-09-24): 核查其余球员的球龄。

- [x] 读取 434 名此前未覆盖球员的 NBA 官方个人页，补上 1 名此前页面缺字段球员的 NBA 官方统计页资料；官方资料缓存共 577 人。
- [x] 以官方经验年数为基础，以首次 NBA 常规赛至 2025–26 的可能赛季数为上限，保留伤病缺席赛季的球龄；577 人中纠正 41 个提前计入新赛季的展示值。开档 570 名真实球员全部采用核实值，覆盖 55 名自由球员，年龄估算 0 人。
- [x] 旧版 `service.2026.v1` 存档升级为 `v2` 时保留玩家游玩期间累积的球龄。核查过程见 `docs/NBA_SERVICE_YEARS_AUDIT_2026-09-24.md`。
- [x] 完整测试 169/169、生产构建、Python 抓取脚本语法检查和 `git diff --check` 通过；ZIP 已覆盖，共 73 个条目、CRC 校验通过，SHA-256 `91fec2d1e5bdab43cf588032f5ee3c3e72836d171447e0bd47615e369b7458a0`。

Current request (2026-09-24): 首届扩军新秀选秀后保持现实球队实际选秀结果，处理扩军球队提前选走现实新秀。

- [x] 首届选秀现实签位继续绑定已核实的 2026 原定球员及选秀夜最终球队；扩军球队可自由选择真实新秀。
- [x] 原定新秀被扩军球队提前选走时，仅该现实签位改用等能力、等位置、等潜力的虚拟球员，保留原定球员的唯一归属，不再级联挪用下一位真实新秀。
- [x] AI 选人预览与最终结果一致；快速模拟使用相同规则。结果页保持普通球员展示，不显示补位标记。
- [x] 完整测试 170/170、生产构建、固定种子贯穿第二赛季经理流程及 `git diff --check` 通过；ZIP 已覆盖，共 73 个条目、CRC 校验通过，SHA-256 `23c62c18915e29746513d04d13be6b5fb07d498047e43c09c0fef8361eb9a3b7`。

Current request (2026-09-24): 退役球员巴图姆、自由球员报价拦截原因与名单规则单行显示。

- [x] NBA 官方确认尼古拉斯·巴图姆已于 2026-09-21 退役；从开档自由球员快照及能力投影移除，并加入退役过滤和旧存档清理。开档真实球员 569 人，其中自由球员 54 人。
- [x] 16 人阵容未达休赛期 21 人上限；当帽下空间为负时，当前引擎按普通自由球员报价规则拒绝报价。页面分别说明薪资空间和名单原因，并在按钮及卡片中显示具体拦截原因。
- [x] 移除冗余“重置筛选”按钮；名单规则保持单行，390px 视口实测无横向溢出。
- [x] 完整测试 172/172、自由球员导入器测试 3/3、生产构建、`git diff --check` 通过；ZIP 已覆盖，共 73 个条目、CRC 校验通过，SHA-256 `9887cb4572d39068308e93b038274689481bc71af6fc1903930540643d0fb981`。

Current request (2026-09-24): 杰伦·杜伦在自由市场中的出现条件，以及报价后第三天结算无反应。

- [x] 开档快照将杜伦标记为 RFA；自由市场列表按未签约状态和位置筛选，不按玩家帽下空间筛选。市场结算签约后从列表消失。
- [x] 用真实开档球员测试复现：同日多笔报价依次签约，活塞名单先达到 21 人，后续报价仍尝试签入活塞，触发异常并回滚当日结算。结算每份报价前现重新校验名额，满额报价失效并释放预留空间；玩家报价失效写入联盟动态。
- [x] 若需玩家处理 RFA 匹配，页面顶部显示提示，结算区直接显示匹配/放弃按钮，避免“结算今日”禁用却无可见操作。
- [x] 完整测试 173/173、定向测试 11/11、5 个固定种子休赛期流程、1 个贯穿第二赛季流程、生产构建和 `git diff --check` 通过。浏览器实际向杜伦报价并连续结算至第 4 天，动态显示其签约后从市场消失。
- [x] ZIP 已覆盖，73 个条目、CRC 校验通过，SHA-256 `0944a706f380b908bfa7b5e2e4600e037158367d4dc0cddd803d1d2aff4e9b32`。

Current request (2026-09-24): 自由市场结算反馈统一进入球队通知栏，常规赛可复用。

- [x] 共用游戏顶部新增“通知”入口和未读/待办角标；自由球员报价签约、失效、未成交、RFA 匹配结果在结算后保存到球队通知，支持逐条或全部标为已读，旧存档自动补齐字段。
- [x] 常规赛的待处理重大伤病、紧急名单和经理事件也显示于通知栏，点击可返回赛季页面处理；RFA 匹配待办可跳至自由市场结算区。
- [x] 390px 浏览器实际完成杜伦报价与第三天结算，通知显示签约成功，标为已读后角标归零；完整测试 177/177、通知与存档定向测试 25/25、3 个种子贯穿第二赛季、类型检查、生产构建和 `git diff --check` 通过。
- [x] ZIP 已覆盖，73 个条目、CRC 校验通过，SHA-256 `c5b9f783ee81e876887a43f0294096afcf4d8b2709fec51a3219b94c9bba08fa`。

Current request (2026-09-24): 季前准备页与游戏统一全宽布局对齐。

- [x] 移除季前准备内容区的旧版横向内边距，并让底部操作栏同宽对齐；沿用页面 12px 安全边距。
- [x] 浏览器实测 1280px、390px、320px 视口：季前准备内容占满可用宽度、无横向溢出；生产构建与 `git diff --check` 通过。
- [x] ZIP 已覆盖，73 个条目、CRC 校验通过，SHA-256 `4e9784cb03279844acf96b2a568bc0466483080eeada6b84d6985fb82373fb2d`。

Current request (2026-09-24): 自由市场拒绝通知说明签约去向；向玩家解释 Utility、推荐合同总额/年限/逐年薪资和重点培养逻辑。

### 自由市场反馈与培养说明 — complete

- [x] 报价通知区分球员拒绝、签约他队、RFA 匹配和名单已满等原因；签约他队时明确球队。
- [x] 由领域层统一输出推荐合同的年限、逐年薪资、总额和保障额，自由市场卡片展示后再默认报价。
- [x] 页面用“签约意愿（Utility）”解释 0～100 评分、85 提前接受和截止日 60 最低线。
- [x] 季前页展示重点培养的结算时点、年龄差异和专项取舍，结算年龄 30+ 时禁用无效选择。
- [x] 自由市场/培养定向测试 22/22、TypeScript、生产构建、`git diff --check` 和 2 个 Stage 4 种子模拟通过；全量测试仅命中 `career.test.ts` 默认 30 秒超时，该文件提高到 90 秒后 4/4 通过。
- [x] 390×844 浏览器实测推荐合同报价、实际合同与签约意愿展示，以及 29 岁球员预计 30 岁时的禁用提示；控制台 0 错误/警告。

Current request (2026-09-24): 自由球员列表卡恢复简洁样式；点击发起报价后在弹窗查看并编辑逐年合同。

### 自由球员可编辑报价弹窗 — complete

- [x] 列表卡恢复球员身份、综合、年龄、建议首年薪资、建议年限和报价状态，不再铺开完整合同。
- [x] 新增居中报价弹窗，展示系统期望逐年薪资，并可编辑年限、各年薪资、保障比例和承诺角色。
- [x] 自定义逐年薪资进入领域报价与最终合同，校验合同年限、薪资上下限及本队/外队年变动上限；按界面“万美元”精度允许 5,000 美元取整容差。
- [x] 完整测试 190/190、生产构建、`git diff --check` 和 2 个 Stage 4 种子模拟通过。
- [x] 390×844 浏览器实测非法逐年薪资阻止提交；合法修改第 4 年薪资、保障比例和承诺角色后成功报价，列表显示实际报价与签约意愿；控制台 0 错误/警告。

Current request (2026-09-24): 不在自由市场页面展示签约意愿说明，仅说明引擎计算逻辑。

### 移除签约意愿说明 — complete

- [x] 移除自由市场顶部 Utility 说明卡和报价弹窗内的重复说明，保留实际报价分数。
- [x] TypeScript、生产构建和 `git diff --check` 通过；Playwright 截图确认标题后直接进入薪资面板，未生成控制台错误文件。

Current request (2026-09-25): 重做常规赛首页，并实现球队 OVR、阵容适配、五位置首发与球员目标分钟。

### 常规赛指挥中心与轮换管理 — complete

- [x] 常规赛首页改为“赛季摘要 → 下一场对阵 → 主模拟按钮 → 赛程条 → 有数据才显示的近期赛果/伤病”；移除未接入的战术预设、胜率预测、焦点球星和重复快讯。
- [x] 球队 OVR 改为分钟加权的全队天赋值并映射到 0～99；争冠级原始阵容可进入 90+，显示沿用球员 OVR 红/橙/黄/绿/蓝色阶。阵容适配继续单独展示，不再冒充 OVR。
- [x] 下一场对阵的双方球队卡都可点击，弹层展示完整球队阵容；本队弹层可直接前往阵容管理。
- [x] 新增 PG、SG、SF、PF、C 五个固定首发槽位，自动避免重复球员；主位置或副位置匹配不扣效果，PF→C 等邻位轻微扣减，PG→C 等远距离错位明显扣减，并实际进入比赛攻防能力计算。
- [x] 新增全名单目标分钟编辑器；常规赛单人最多 40、季后赛最多 42，5～10 人轮换且总计严格为 240。伤病时引擎按目标分钟重新分配，并按位置兼容度自动补首发。
- [x] 轮换方案通过 `SET_ROTATION_PLAN` Command 原子保存；开档、旧存档迁移、交易和伤病后的轮换会自动补齐，避免出现离队球员引用或重复首发。
- [x] `render_game_to_text` 增加球队 OVR、首发与目标分钟，便于自动化检查。
- [x] 新增球队 OVR、首发唯一性、240 分钟、主副位置与严重错位模拟影响等回归测试；完整 Vitest 42 文件 / 218 用例通过，TypeScript、生产构建和 `git diff --check` 通过。
- [x] 按 web-game 工作流使用 Playwright 客户端检查赛季首页截图和文本状态；另在 467×935 移动端实际检查首页、轮换编辑器、对手阵容弹层，并完成一次轮换原子保存，未发现控制台错误。

Current request (2026-09-25): 打包并返回最新 H5 页面。

### 最新 H5 正式发布包 — complete

- [x] `pnpm build` 通过，生成 `h5/index.html` 与最新 `h5/assets/game.js`。
- [x] 使用静态服务器和 Playwright 直接打开生产构建，入口、主脚本和首屏图片均返回 200，页面正常显示，未捕获控制台错误。
- [x] 已用最新 `h5/` 更新唯一正式发布包 `release/篮球经理_联盟扩军时代.zip`；共 74 个文件，ZIP CRC 校验通过，压缩包 SHA-256 为 `da272da680c3e54ec0253481649dcb62a397f5d7fd09971a994811772dba116e`。
- [x] ZIP 内 `assets/game.js` 与构建产物 SHA-256 一致：`36633025d966bb821a24a2dac95dabc8e241bdf58d52c9b97f39a6e01c4c2987`。

Current request (2026-09-25): 修复本地 H5 扩军选秀选择数名球员后黑屏。

### 扩军选秀临时阵容 OVR 黑屏 — complete

- [x] 浏览器复现至第 5 名球员，捕获 `ROTATION_MINUTE_CAPACITY_EXHAUSTED`：球队 OVR 对 5 人临时阵容错误地尝试按单人最多 40 分钟分配完整 240 分钟，React 根组件因此卸载。
- [x] 球队 OVR 在可用球员不足以承载 240 分钟时改用可用球员平均能力，不再创建无解轮换；达到 6 人后恢复分钟加权计算。
- [x] 新增 5 人临时阵容回归测试；球队 OVR 与轮换定向测试 8/8、TypeScript 通过。
- [x] 浏览器实际连续选择至 5/14 与 6/14，页面保持显示，原错误未再次出现；已检查回归截图。
- [x] 全量测试 218/219 通过；唯一失败为整季模拟默认 30 秒超时，该文件提高到 90 秒后 4/4 通过。生产构建和 `git diff --check` 通过。
- [x] 已更新唯一正式 H5 发布包，共 74 个文件且 CRC 校验通过；ZIP SHA-256 为 `2a2993866e0e8497e5f507ee39de65046a1c9be24c8529e2e2e62482e381b65b`，包内主脚本与构建产物 SHA-256 均为 `7476f0880fb256b52b937064f952504804d2d98342e2474556a19e16d8e6a0ed`。

Current request (2026-09-25): 统一开始游戏到进入常规赛之间的底部按钮文字大小。

### 主流程底部操作字号 — complete

- [x] 建立统一的主流程操作字号 token：单个主操作按钮 14px / 900，双按钮与次要操作 12px / 900；保留原有 44～48px 点击高度和安全区布局。
- [x] 覆盖开始游戏、扩军建队、权益选择、扩军交易、扩军完成、新秀选秀、自由市场和季前锁定名单页面，移除页面局部字号造成的 10～15px 跳变。
- [x] 浏览器逐页读取计算样式确认：单主按钮均为 14px，开始页次操作、新秀选秀和自由市场双操作均为 12px。
- [x] 按 web-game 流程检查权益选择与新秀选秀截图，长文案无溢出，未生成控制台错误文件；TypeScript、生产构建和 `git diff --check` 通过。
- [x] 已更新唯一正式 H5 发布包，共 74 个文件且 CRC 校验通过；ZIP SHA-256 为 `cb354eaed9496ec4e217c94e6d059d131ed83652c9a4c290454c49027eb4c8c2`，包内主脚本与构建产物 SHA-256 均为 `4f51a68ce04709b69d47ade7e045b5d319682c2a1a79ad39b09789422bc5652a`。

Current request (2026-09-25): 开始游戏序章动画铺满全屏，并移除 NBA 标识与文案。

### 序章全屏与品牌中性化 — complete

- [x] 移除序章上下黑色电影遮幅，动画容器固定覆盖完整 `100dvh`，背景从屏幕顶边铺到屏幕底边；跳过按钮继续适配顶部安全区。
- [x] 第 1 幕移除 NBA 徽标与 NBA 文案，改为中性的“联盟扩军公告”和 `30 → 32`；第 2 幕移除现役球队名称与官方队徽，改为中性的西部转东部说明。
- [x] 第 3 幕仅保留游戏自有的西雅图、拉斯维加斯扩军队标识；第 4 幕继续使用游戏自有 GM 标识，四幕不再展示 NBA 品牌文案或官方球队队徽。
- [x] 按 web-game 流程使用 Playwright 客户端检查场景截图与文本状态；另在 390×844 手机视口逐幕检查，顶部无黑边、画面无裁切、控制台未生成错误文件。
- [x] 完整 Vitest 42 文件 / 219 用例、TypeScript、生产构建和 `git diff --check` 通过。
- [x] 已更新唯一正式 H5 发布包，共 74 个文件且 CRC 校验通过；ZIP SHA-256 为 `d788b6c5a0d2e84807c9f2ca88034b281fe363c00dd5d2395c02cefa95cf9bdb`，包内主脚本与构建产物 SHA-256 均为 `81c6a5fa143a1a14b236c533077aaa9a78719146f8d1ac2a3b11154674c140f9`。

Current request (2026-09-25): 序章动画的按钮和进度 UI 与当前主流程风格统一。

### 序章操作控件统一 — complete

- [x] 底部操作区改为与主流程一致的米白浮层卡片、18px 圆角与柔和阴影；最大宽度限制为 398px，在全屏画面中保持移动端操作密度。
- [x] “继续 / 开始建立球队”统一为 48px 高、14px / 900 的橙色主按钮；“跳过序章”改为白底深色文字的 36px 次级按钮，并保留顶部安全区。
- [x] 原细线进度改为 1～4 数字分步控件：默认米灰、已看深色、当前步骤橙色描边和自动进度线；补充 `aria-current="step"` 与键盘焦点样式。
- [x] Playwright 客户端截图检查通过；390×844 手机视口确认控件不溢出，点击第 2 幕后再点继续可正确进入第 3 幕。
- [x] 完整 Vitest 42 文件 / 219 用例、TypeScript、生产构建和 `git diff --check` 通过。
- [x] 已更新唯一正式 H5 发布包，共 74 个文件且 CRC 校验通过；ZIP SHA-256 为 `fd46f3cd346623a0c69bd83844a984d674a80384647fc37b7ea27e475322a4f3`，包内主脚本与构建产物 SHA-256 均为 `d9253fa31d88faf2b6c00bc722a4e4cb9e4c27144bcf12079c0872d51016a152`。

Current request (2026-09-25): 撤回序章控件结构改版，仅统一为扩军页面按钮配色。

### 序章控件恢复与配色对齐 — complete

- [x] 撤回米白底栏、数字步骤、放大按钮和白底跳过按钮，恢复原有透明悬浮布局、四段细线进度、胶囊跳过按钮和右下角继续按钮。
- [x] 仅调整控件颜色：主按钮与已看/当前进度统一使用扩军流程橙色 `#e86a38`，悬停色为 `#d75928`；跳过按钮使用扩军流程深色次级色 `#2b2825` 的半透明版本。
- [x] Playwright 客户端截图与 390×844 手机视口检查通过；计算样式确认主按钮和进度高亮均为 `rgb(232, 106, 56)`。
- [x] 完整 Vitest 42 文件 / 219 用例、TypeScript、生产构建和 `git diff --check` 通过。
- [x] 已更新唯一正式 H5 发布包，共 74 个文件且 CRC 校验通过；ZIP SHA-256 为 `051e2cdd13a149c3559d043f40607d82f6451627c484b014393576c53ab380fc`，包内主脚本与构建产物 SHA-256 均为 `c016976f6422adeb9901a88054171296cabfeaeb910abfc04aa3d2055ca4e566`。

Current request (2026-09-25): 序章恢复森林狼队徽，第四幕中文化，并改用扩军选秀底部按钮蓝色。

### 序章球队信息与青蓝控件 — complete

- [x] 第二幕恢复森林狼转区叙事，明确“森林狼，转入东部”和“西部 → 东部”，重新接入开档球队数据中的 `MIN` 本地队徽。
- [x] 第二幕队徽、方向和东西部分区在 390×844 手机视口完整显示，无裁切和横向溢出。
- [x] 第四幕圆形标识由 `GM / EXPANSION ERA` 改为中文“经理人 / 扩军时代”。
- [x] 序章继续按钮、进度高亮和焦点色统一为扩军选秀底部按钮使用的青蓝色 `#00e5f2`；按钮文字改为深色 `#001116`，跳过按钮使用对应深蓝次级色。
- [x] Playwright 客户端截图检查第二幕；浏览器逐幕检查第二幕与第四幕，未发现控制台错误。
- [x] 完整 Vitest 42 文件 / 219 用例、TypeScript、生产构建和 `git diff --check` 通过。
- [x] 已更新唯一正式 H5 发布包，共 74 个文件且 CRC 校验通过；ZIP SHA-256 为 `3107d879e102bec89bab47963f1b120a8fe018ae384f8b28f7eecaf98376f4ba`，包内主脚本与构建产物 SHA-256 均为 `484be3958e91a6fa039db22c5d9c124544cd08f33cbc571a25c64f98f2bc9882`。

Current request (2026-09-25): 四幕序章内所有橙色界面强调统一为青蓝色。

### 序章强调色完全统一 — complete

- [x] 新增序章专用色变量 `--cinematic-accent: #00e5f2` 与悬停色 `#33f5ff`，避免四幕局部色值再次分叉。
- [x] 第 1 幕数量箭头、标题眉标和信息竖线，第 2 幕转区箭头，第 3 幕加号与联盟标签，第 4 幕经理人徽章光晕与分隔线全部由橙色改为青蓝色。
- [x] 继续按钮、进度高亮、焦点环、森林狼队徽描边和东部状态保持同一青蓝体系；球队队徽图片保持原色，不进行滤镜改色。
- [x] 代码扫描确认序章样式区不再存在旧橙色值；Playwright 检查第 1 幕，390×844 浏览器检查第 3、4 幕，视觉显示一致。
- [x] 完整 Vitest 42 文件 / 219 用例、TypeScript、生产构建和 `git diff --check` 通过。
- [x] 已更新唯一正式 H5 发布包，共 74 个文件且 CRC 校验通过；ZIP SHA-256 为 `f9399a133fc1efdf07e8106730a2869ab4920e10fd0c8621bd4770d4847b5384`，包内主脚本与构建产物 SHA-256 均为 `9f4cbc2da23a3b41a03b102d1fd96dd2cdcac934ab614642444c54ad1e70e70d`。

Current request (2026-09-25): 将序章“继续 / 开始建立球队”按钮改为与扩军选秀主按钮一致的直角造型。

### 序章主按钮直角化 — complete

- [x] 将序章主操作按钮圆角从 12px 改为 0；颜色、尺寸、位置、阴影和交互均保持不变。
- [x] “跳过序章”继续保留胶囊造型，维持次级操作与主按钮的视觉区分。
- [x] Playwright 检查第 1 幕“继续”，浏览器检查第 4 幕“开始建立球队”，两处均为直角青蓝按钮。
- [x] 完整 Vitest 42 文件 / 219 用例、TypeScript、生产构建和 `git diff --check` 通过。
- [x] 已更新唯一正式 H5 发布包，共 74 个文件且 CRC 校验通过；ZIP SHA-256 为 `3b72824a7f81b709bf896e1c166a0472105f0cf15220b912a3df14827e8cc258`，包内主脚本与构建产物 SHA-256 均为 `8df6ca63cafb9780c331653463e782fb1d79cbd12d8e28d6c4f26c510754f997`。

Current request (2026-09-25): 精简并重排常规赛首页，日历上移、只保留球队 OVR、重做模拟入口，并将事件迁入顶部通知。

### 常规赛首页信息层级重构 — complete

- [x] 移除返回主页按钮右侧的球队资料，将球队名、战绩、分区排名和当前场次合并到“2026-27 · 常规赛”摘要下方；首页可见英文标签全部中文化，仅保留用户指定的 OVR 术语。
- [x] 将赛程日历移动到下一场对阵上方；日历使用中文球队名，点击未来日期后才显示“模拟至 X月X日”。
- [x] 首页移除阵容适配，只保留球队 OVR；双方球队卡片同步去掉适配值，并将队徽图片限制在同尺寸圆框内居中显示。
- [x] “模拟下一场比赛”下方改为“推进 1 天 / 连续模拟 5 场”两个 1:1 等宽按钮，移除“更多模拟”折叠菜单。
- [x] 删除首页事件卡和模拟结果摘要；事件继续进入顶部球队通知，单选确认类事件可直接处理，多选事件才打开决策弹层。
- [x] Playwright 桌面截图、390×844 手机布局、未来日期显隐和通知事件处理均通过；最终视觉判定 96/100。
- [x] 完整 Vitest 42 文件 / 219 用例、TypeScript、生产构建和 `git diff --check` 通过。
- [x] 已更新唯一正式 H5 发布包，共 74 个文件且 CRC 校验通过；ZIP SHA-256 为 `c8d2448c1fc738ebb229baf53b8594a5465a893e03a37fae55895609b9eea7f2`，包内主脚本与构建产物 SHA-256 均为 `ab7d8c65e78f02b88259b4c63eec943c6e17ab23fab5f0a742eba5a71208b221`。

Current request (2026-09-26): 将常规赛首页压缩为一屏，补齐日历和近期赛果信息，并修复通知事件处理入口。

### 常规赛首页一屏化与通知处理 — complete

- [x] 首页整体间距、日历、对阵卡和操作按钮进一步压缩；390×844 手机视口下页面高度与视口高度均为 844px，不再产生纵向滚动。
- [x] 已结束的日历比赛同时显示对手简称和胜负；近期赛果固定为最新 3 场，并补充月日与主客场信息。
- [x] 双方球队卡改为紧凑横排结构，队徽跨三行垂直居中且图片保持在圆框内；球队名、战绩与 OVR 的层级同步收紧。
- [x] “模拟下一场比赛”文案居中，右侧箭头仅作为方向提示；下方两个批量操作继续保持 1:1 等宽。
- [x] 通知中的待处理事件统一显示“处理事件”，点击后打开明确的确认/决策弹层；实机完成“通知 → 处理事件 → 确认”，处理后通知清空且比赛按钮恢复可用。
- [x] TypeScript、生产构建、`git diff --check` 和 218 个常规测试断言通过；完整赛季回放压力测试的逻辑断言未失败，但当前运行耗时约 76 秒，超过用例内固定的 60 秒超时。
- [x] 最终视觉判定 96/100；已更新正式 H5 发布包，共 74 个文件且 CRC 校验通过。ZIP SHA-256 为 `bdf4488a0792e11f43c1f7e4f8355566cb20aae443998dee30e7479f48548b39`，包内主脚本与构建产物 SHA-256 均为 `2bb1210198e45b01aeee05bfd45778f911984463c9a1e906346e059bef6d37b8`。

Current request (2026-09-26): 存档列表中的赛季阶段枚举全部显示为中文。

### 存档阶段文案中文化 — complete

- [x] 首页“读取存档”和“选择新生涯槽位”不再直接输出内部阶段枚举，统一复用 `phaseLabel` 中文映射；游戏内存/读档抽屉原有中文映射保持一致。
- [x] `REGULAR_PRE_DEADLINE` 显示为“常规赛·交易截止日前”，`REGULAR_POST_DEADLINE` 显示为“常规赛·交易截止日后”；代码扫描确认存档 UI 不再直接展示 `summary.phase`。
- [x] TypeScript、生产构建、Playwright 页面检查、浏览器控制台和 `git diff --check` 通过。
- [x] 已更新正式 H5 发布包，共 74 个文件且 CRC 校验通过。ZIP SHA-256 为 `78ad0705f705983c1a087ec7f76f53bdefd2b895b0b66e675cc5f0d8a2c09770`，包内主脚本与构建产物 SHA-256 均为 `93e6cfbb0d2b232a78574929bb5cb6f1325713b268fa939d5a5b8ab0ea4c30df`。

Current request (2026-09-26): 参考所给对阵图重排球队对阵模块，并确保双方队徽居中。

### 对阵模块参考图布局 — complete

- [x] 对阵区改为左右对称纵向球队栏：队徽、球队名、战绩、OVR 自上而下排列；中间改为 `VS` 与主/客场青蓝胶囊。
- [x] 队徽使用统一的中性圆形底座与固定图片安全区，不再受球队色背景或横向文案布局挤压；球队卡仍可点击查看各自阵容。
- [x] 390×844 视口实测左、右队徽中心与各自 136px 球队栏中心完全重合；页面 `scrollHeight` 与视口高度均为 844px，继续保持一屏无纵向滚动。
- [x] Playwright 桌面截图、手机视口、控制台、TypeScript、生产构建与 `git diff --check` 通过；参考图视觉判定 97/100。
- [x] 已更新正式 H5 发布包，共 74 个文件且 CRC 校验通过。ZIP SHA-256 为 `2fbd64e71752d1a3004a633bae092416879323e74ea6f4e728aacb210c0ad0b7`，包内主脚本与构建产物 SHA-256 均为 `d416e2834770ed52add5ad45d00a5587bb97376e5eeb1fed5a0657d23bb7bbeb`。

Current request (2026-09-26): 参考用户提供的深色球员卡截图重排常规赛“管理 → 阵容轮换”，保留一键自动匹配。

### 阵容轮换参考图布局 — complete

- [x] 将球员按“首发阵容 / 主要替补 / 其他球员”分组；首发按 PG、SG、SF、PF、C 排列，替补按目标分钟排序。
- [x] 采用横向球员卡、圆形现有球员头像、位置徽标与橙色 OVR；右侧保留减号、分钟输入、加号，卡内可切换首发位置。
- [x] 保留“一键自动匹配”，并加入底部固定的“保存轮换方案”按钮；继续使用现有 240 分钟与人数校验。
- [x] TypeScript、生产构建、轮换引擎 8 个测试和 `git diff --check` 通过。
- [ ] 当前环境的浏览器安全策略禁止打开本地 H5 页面，因此未能取得实机截图或执行视觉评分；后续可在可访问浏览器的环境补做视觉检查。

Current request (2026-09-26): 阵容轮换首发卡显示实际位置和球员照片，取消角色下拉；两次点选球员互换首发位置且分钟数不变，卡片与照片框改为直角。

### 阵容轮换点选互换 — complete

- [x] 首发卡在姓名前直接显示 PG / SG / SF / PF / C 实际首发位置；删除“首发/替补”下拉筛选。
- [x] 点击第一名球员进入选中状态，再点击另一名球员交换首发位置或首发/替补身份；分钟仍绑定各自球员，替补之间因无固定首发位置不执行无效互换。
- [x] 轮换卡将球员 `portraitPath` 传入现有头像组件；球员卡和头像边角均改为直角。
- [x] 新增互换逻辑与卡片结构测试；轮换相关 11 个断言、TypeScript、生产构建和 `git diff --check` 通过。
- [ ] 当前环境仍无法访问本地 H5 浏览器页面，未执行实机视觉检查。

Current request (2026-09-26): 阵容轮换中取消“其他球员”分组，将 0 分钟球员并入“主要替补”，伤病球员标记并放在末尾。

### 轮换替补列表合并 — complete

- [x] “主要替补”包含全部非首发球员：有出场时间的排在前面，0 分钟球员随后；不再显示“其他球员”区块。
- [x] 伤病球员显示“伤停 N 场”标签并排在列表最末；其他不可出战球员也明确标记。
- [x] 扩充组件测试覆盖合并分组、0 分钟显示和伤病末位排序；相关 12 个断言、TypeScript、生产构建及 `git diff --check` 通过。
- [ ] 本地 H5 的浏览器视觉检查仍受当前环境限制。

Current request (2026-09-26): 轮换人数不应固定最多 10 人，允许经理使用 11 人或 12 人轮换。

### 轮换人数扩展到 12 人 — complete

- [x] 轮换方案校验和比赛模拟的出场人数上限从 10 人提高到 12 人；手动方案中的第 11、12 名球员可实际获得比赛分钟。
- [x] 自动匹配继续沿用现有 10 人分钟权重，作为默认方案；手动方案可安排 5–12 人，页面人数上限和错误提示同步更新。
- [x] 新增 11 人和 12 人方案的保存、替补预览、比赛分钟验证；相关 14 个针对性断言和 41 个受影响模块断言通过，TypeScript、生产构建及 `git diff --check` 通过。
- [ ] 本地 H5 的浏览器视觉检查仍受当前环境限制。

Current request (2026-09-26): 重做常规赛“管理”的概览、合同薪资、选秀权，并将现有阵容轮换放入四个可见 tab。

### 常规赛管理四页整合 — complete

- [x] 管理页顶部固定为“概览 / 阵容轮换 / 合同薪资 / 选秀权”四个等宽 tab；阵容轮换保留现有一键匹配、点选换位和保存功能。
- [x] 概览展示球队战绩、场均得失分、阵容能力，以及全员赛季数据表；支持按分钟、得分和 OVR 排序，点击球员可查看详情。
- [x] 合同薪资展示工资帽占用、各条薪资线、真实占用构成及球员合同；选秀权按年份列出球队实际持有的首轮、次轮和已承诺资产，交易入口继续位于市场。
- [x] 新增组件测试；完整 Vitest 49 文件 / 249 用例、TypeScript、生产构建和 `git diff --check` 通过。
- [ ] 当前环境的浏览器安全策略禁止打开本地 H5 页面，尚未进行实机视觉检查。



Current request (2026-09-26): 合同薪资页复用扩军页的薪资进度条样式。

### 合同薪资进度条统一 — complete

- [x] 合同薪资页采用扩军页的绿色进度条、四条彩色薪资线刻度和同款图例；占用总额与帽下空间继续使用当前球队真实工资帽数据。
- [x] 移除合同页独立的渐变进度条样式；组件测试、TypeScript、生产构建与 `git diff --check` 通过。
- [ ] 当前环境的浏览器安全策略仍禁止打开本地 H5 页面，尚未进行实机视觉检查。

Current request (2026-09-26): 合同薪资进度条尝试渐变色，并把连胜、连败等无需经理选择的球队动态放进通知。

### 薪资渐变与连胜连败通知 — complete

- [x] 合同页薪资占用条改为沿工资帽刻度展开的青绿至橙红渐变，只露出当前已占用的区间，四条彩色薪资线保持可见。
- [x] 3、5、10 场连胜及连败事件即时自动结算球迷支持度效果，作为未读“球队”通知出现，不再弹出单选确认事件或暂停模拟；旧存档中的待处理连胜/连败事件在读取时迁移至通知。
- [x] 增加新事件和旧存档迁移测试；完整 Vitest 49 文件 / 251 用例、TypeScript、生产构建与 `git diff --check` 通过。
- [ ] 当前环境的浏览器安全策略禁止打开本地 H5 页面，尚未进行实机视觉检查。

Current request (2026-09-26): 赛季中也能查看自由球员，并先按市场审查建议完成一版优化。

### 常规赛市场首版优化 — complete

- [x] 常规赛“自由球员”不再是空的禁用占位，现展示真实未签约球员并支持搜索、位置和排序；签约命令仍按现有引擎阶段限制，仅在休赛期可用。
- [x] 交易报价卡、详情及成交记录明确显示随交易转移的选秀权；报价页改为“询价回应”，标示最近一次系统报价的状态；记录页明确为“我的交易”。
- [x] 修复年薪筛选滑杆的无效数字属性与默认隐藏高薪报价；重复询价使用新的命令 ID；“GM 意愿”和“空间限额”等超过引擎实际能力的文案改成双方适配变化和帽下空间。
- [x] 新增市场页面与附带选秀权记录测试；完整 Vitest 50 文件 / 255 用例、TypeScript、生产构建与 `git diff --check` 通过。
- [ ] 当前环境的浏览器安全策略禁止打开本地 H5 页面，尚未进行实机视觉检查。

Current request (2026-09-26): 统一市场与管理页的布局，并优化交易控制台和交易引擎。

### 市场与交易控制台优化 — complete

- [x] 市场四个 tab 采用与管理页一致的等宽吸顶布局；询价回应与交易记录改为同一套标题、分区与直角列表样式。
- [x] 交易控制台按选筹码、筛报价、看方案排列；球员显示头像，报价展示适配增减，详情展示双方适配及游戏内资产估值变化。
- [x] 引擎逐队比较多名合法候选，综合资产估值、双方阵容适配与确定性刷新差异生成报价；成交前再次校验合法性，并拒绝已承诺选秀权。
- [x] 全部 50 个测试文件 / 265 项用例、TypeScript、生产构建、`git diff --check` 通过；默认球队 15 名球员均可得到 3 份初次与刷新报价。
- [ ] 当前环境的浏览器安全策略禁止打开本地 H5 页面，尚未进行实机视觉检查。

Current request (2026-09-26): 修复交易页球员头像顶部的细条伪影。

- [x] 交易头像样式不再用 `background` 简写覆盖图集的背景尺寸与位置，照片限制在直角头像框内。
- [ ] 当前浏览器策略仍阻止本地 H5 视觉复查。

Current request (2026-09-26): 精简三份交易报价的筛选和重复入口。

### 清理计划

1. 先用组件测试固定“每份报价都直接可见、可以进入详情”的交互。
2. 删除交易台中搜索、位置、薪资、排序和重置状态；保留选筹码、刷新报价与方案详情。
3. 删除与交易页重复的“询价回应”tab，将市场入口调整为交易、自由球员、交易记录。
4. 清理失效样式与文案，运行测试、类型检查和 H5 构建。

### 完成情况

- [x] 交易台直接展示全部系统报价，保留选筹码、刷新与成交详情；移除对三份报价意义不大的搜索、位置计数、薪资限制、排序和重置。
- [x] 市场删除重复的“询价回应”tab，剩余三个入口等宽排布；清理对应失效样式。
- [x] 组件回归测试、完整 50 文件 / 265 用例、类型检查、生产构建与 `git diff --check` 通过。
- [ ] 本地 H5 页面仍受浏览器策略限制，视觉检查待实机复核。

Current request (2026-09-26): 激励视频刷新报价、交易详情和选筹码面板优化。

### 实施计划

1. 复用选秀页的 `ColorboxAI.vatask` 接入约束：用户点击后完播才刷新，失败保留当前报价。
2. 刷新入口改为报价列表下方的单个显著按钮；交易详情移除重复的底部返回，仅保留顶部返回和底部成交。
3. 选筹码面板按球员主位置筛选并显示位置人数，保证分组人数与总人数一致。
4. 补交互/数据测试，运行完整测试、类型检查和 H5 构建。

### 完成情况

- [x] 报价区改为单个显著的刷新按钮；虎扑 App 激励视频确认完播后才执行询价刷新，取消或失败时原报价不变。本地浏览器无桥接时给出提示。
- [x] 交易详情按交易资产、规则校验和球队影响重新排版；移除重复的底部“放弃”，保留顶部返回和底部成交。
- [x] 选筹码面板显示按主位置统计的五个位置人数并可筛选，人数相加等于球队名单人数。
- [x] 完整测试在两并发下 51 文件 / 269 用例通过，TypeScript、生产构建、`git diff --check` 通过。
- [ ] 当前浏览器策略仍阻止本地 H5 视觉复查，App 原生激励视频须在虎扑 App 中实机验证。

Current request (2026-09-26): 交易刷新文案、切换筹码加载态及市场引擎入口核对。

### 完成情况

- [x] 刷新报价按钮去掉前置刷新图标。
- [x] 切换我方筹码时，系统询价回应显示加载占位并隐藏旧报价；保存完成后展示新方案。
- [x] 询价成功提示改为实际生成方案数，不再固定写 3。
- [x] 核对市场相关引擎与 UI：常规赛裁员可执行但没有常规赛入口；AI 球队成交日志保存在状态中但市场交易记录只显示玩家成交；自由球员报价、撤回和 RFA 决策已有休赛期页面，常规赛签约未获引擎阶段支持。
- [x] TypeScript、交易相关 3 个测试文件 / 13 用例、H5 生产构建通过。
- [ ] 浏览器策略阻止本地 file:// H5 视觉复查；激励视频仍需虎扑 App 实机验证。

Current request (2026-09-26): 合同页裁员、交易记录合并和常规赛自由球员签约。

### 实施与核对

- [x] 管理 → 合同薪资为每份球员合同增加独立裁员入口和确认页，显示剩余保障金额及死钱影响；引擎裁员后同步修复轮换。
- [x] 市场 → 交易记录同时展示玩家职业生涯交易与 AI 球队成交；新生成的 AI 记录带赛季，历史记录不再截断为最近 30 笔。
- [x] 核对数据链路：开档自由球员池为 54 人；保罗·乔治和杰拉米·格兰特开档分别在 BOS、MEM 并有标准合同。市场总人数按存档中当前 UFA/RFA 动态计算，合同到期、裁员和落选球员会增加人数。当前浏览器 file:// 策略阻止读取该标签页存档，无法确认其 162 人的逐项来源。
- [x] 按需求文档 25.13 的操作矩阵开放常规赛交易截止日前后 UFA 报价；RFA 仍限休赛期。常规赛报价沿用薪资预留、球员决策和比赛日结算，支持撤回与通知。
- [x] TypeScript、相关测试、完整 52 文件 / 273 用例、H5 生产构建与差异空白检查通过；新增轮换修复后再次通过 4 文件 / 15 用例与类型检查。
- [x] AI 交易记录不再截断；相关 5 文件 / 17 用例、最终 H5 构建与差异空白检查通过。
- [ ] 本地 file:// 页面视觉复查与玩家当前存档的 162 人名单逐项来源仍受浏览器策略限制。

Current request (2026-09-26): 交易记录标题与球员中文名。

- [x] 交易记录中的「AI 球队成交」改为「联盟交易」。
- [x] 我方与联盟成交摘要在展示时按球员资料转换中文姓名，已有存档记录同步生效。
- [x] 相关 2 个测试文件 / 9 用例、TypeScript、H5 构建与差异空白检查通过。

Current request (2026-09-26): 复查保罗·乔治、杰拉米·格兰特进入常规赛自由市场。

- [x] 按默认扩军种子复现：开档两人分别在 BOS、MEM 且持有标准合同；首季锁定阵容前仍在原队，旧 AI 缩减名单逻辑会把格兰特裁入自由市场，常规赛自由球员共 161 人。
- [x] AI 缩减超员名单改用包含剩余保障金额的留队评分，避免把高额保障合同误当成可节省的薪资；同条件回归覆盖保罗·乔治与杰拉米·格兰特。
- [x] 完整开赛流程复核两人均留在原队；全量 52 文件 / 277 用例、H5 构建与差异空白检查通过。
- [ ] 浏览器策略限制读取当前 file:// 存档；已被旧逻辑裁掉的球员不会因引擎修正自动回到球队。

Current request (2026-09-26): 优化 AI 裁员与开赛前自由球员签约。

- [x] 离线自由球员快照中的 RFA 绑定上一支 NBA 球队的匹配权；杜伦开档匹配权归活塞。
- [x] AI 开启自由市场时先整理到常规赛 15 人目标；仅在有明确高能力升级且拥有匹配权或足够薪资空间时再腾出报价名额。
- [x] 原球队可以通过同一 Offer / Decision Window 直接向己方 RFA 报价，保留独立的 `RFA_OWN_TEAM_OFFER` 类型；其他球队继续使用 RFA Offer Sheet 与匹配流程。
- [x] AI 裁员留队分数以当前能力为主，并计入裁员后仍需支付的保障薪资。默认开赛流程杜伦与活塞签约，库兹马、洛佩兹、比尔、巴恩斯、克拉克森、克莱·汤普森和邓恩保留球队合同，市场最高 OVR 为 77（原本就在市场的 UFA）。
- [x] 修正选秀回归测试的判定：允许已选中的末轮新秀被裁，但落选新秀仍不能进入真实自由球员市场。
- [x] 全量 52 文件 / 279 用例、跨赛季 Headless、H5 生产构建和差异空白检查通过；新增 Offer 类型后相关 4 文件 / 54 用例与构建再次通过。
- [ ] 当前 file:// 存档不可从浏览器工具读取；旧存档里已经发生的裁员与未签约不会自动重写。

Current request (2026-09-26): 市场自由球员报价弹框与扩军自由球员报价弹框内容不一致。

### 自由球员报价弹框统一 — complete

- [x] 两个入口共用同一报价弹框：合同年限、首年薪资、逐年涨幅、保障比例、末年选项、承诺角色、逐年薪资/球员期望和合同金额预览保持一致。
- [x] 常规赛入口保留比赛日结算说明，两个入口仍按各自赛季阶段调用原有引擎校验与提交指令。
- [x] 删除常规赛旧弹框样式；新增共享弹框回归测试。TypeScript、定向 6 项测试、生产构建、差异检查通过。
- [ ] 浏览器视觉实测未完成：本机 in-app browser 对 file URL 的访问被 Browser Use URL policy 拦截。

Current request (2026-09-26): 市场自由球员列表的 OVR 数值更醒目，OVR 标签更小。

### 自由球员 OVR 字级 — complete

- [x] 自由球员列表将 OVR 标签与数值分成上下两行：标签 7px、数值 18px，保留原有橙色数值并用细分隔线区分球员资料。
- [x] 生产构建、市场页面 5 项定向测试及差异检查通过。
- [ ] 本机浏览器对本地 file URL 的访问仍受 Browser Use URL policy 限制，本轮未完成视觉截图。

Current request (2026-09-26): 通知增加日期、扩军开季欢迎消息，并将存读档和虎扑玩家反馈收进设置。

- [x] 新通知在创建时记录游戏内日期；待处理消息显示当前游戏日期，旧存档无法还原日期时显示“日期未记录”。
- [x] 扩军球队首次锁定常规赛名单时写入一次新球队欢迎通知，简述赛季、管理、市场、联盟和生涯入口。
- [x] 旧存档若已进入首赛季且缺少该通知，读取时只补一次，日期保留为开季日。
- [x] 顶栏“存/读档”改为“设置”，设置内保留原存读档面板并增加玩家反馈表单。
- [x] 反馈活动标识取自当前游戏 `activity.json`；调用虎扑用户资料、内容安全检测和公共反馈服务；本地预览显示非虎扑环境提示。
- [x] 本地浏览器实测设置→存读档、设置→反馈、输入字数、非虎扑提示和通知日期；定向测试与构建通过。
- [x] 首轮全量 64 文件 / 314 用例通过；补入旧档迁移后再跑相关 4 文件 / 40 用例、类型检查、生产构建与差异检查通过。
- [ ] 虎扑 App 内真实反馈提交待宿主环境联调。

Current request (2026-09-26): 覆盖正式 ZIP 并提交推送当前代码。

- [x] 重新构建 H5，并用 `h5/` 的 74 个文件覆盖 `release/篮球经理_联盟扩军时代.zip`；逐项比对 ZIP 内容与源文件、CRC 校验通过，SHA-256 为 `ea7a4ad1f41e98d34e3e3b440e18ea56c3e87ac6bd99517b105e6aecdd5db0c9`。
- [x] 此前全量 65 个文件 / 322 项测试通过；本轮 TypeScript 生产构建再次通过。
