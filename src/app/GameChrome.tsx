...
interface GameChromeProps {
  ...
  seasonProfile?: { teamName: string; record: string; rank: string };
...
type IconName = "home" | "save" | "bell" | "calendar" | "users" | "arrows" | "trophy" | "flag";
...
export function GameChrome({ phase, busy = false, dataLabel = "本地球员数据已载入", onSave, onLoad, onLoadLatest, activeSlot = 1, saveSlots = [], onSlotChange, onHome, initialDrawerTab, seasonProfile, notifications = [], onMarkNotificationsRead, onHandlePendingNotification }: GameChromeProps) {
  ...
  return (
    <>
      ...
      {inboxOpen && createPortal(<TeamInboxDrawer notifications={notifications} attentionCount={attentionCount} unreadIds={unreadIds} busy={busy} onClose={() => setInboxOpen(false)} onMarkRead={onMarkNotificationsRead} onHandlePending={(item) => { setInboxOpen(false); onHandlePendingNotification?.(item); }} />, document.body)}
...
function TeamInboxDrawer({ notifications, attentionCount, unreadIds, busy, onClose, onMarkRead, onHandlePending }: {
  notifications: TeamInboxItem[];
  ...
  return <div className="team-inbox-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="team-inbox-panel" role="dialog" aria-modal="true" aria-label="球队通知">
      ...
      <div className="team-inbox-list">{notifications.length ? notifications.map((item) => <article className={`team-inbox-item${item.pending ? " pending" : item.read ? " read" : " unread"}`} key={item.id}>
        ...
        <h3>{item.title}</h3><p>{item.playerName && item.playerId ? `${playerNameZh(item.playerName, item.playerId)} · ` : ""}{humanizeUiText(item.message)}</p>
