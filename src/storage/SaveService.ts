...
function migrateLoadedState(input: GameState): GameState {
  ...
  players.forEach((player, ordinal) => {
    ...
    player.marketPreference ??= profile.marketPreference;
...
export class SaveService {
  ...
  async syncWithCloud(slotId: number, cloud: StorageAdapter): Promise<SaveSyncResult | null> {
    ...
    const cloudEnvelope = this.parseValidEnvelope(await cloud.get(slotKey(slotId)));
  ...
  private parseValidEnvelope(serialized: string | null): SaveEnvelope | null {
    if (!serialized) return null;
    try {
      const envelope = JSON.parse(serialized) as SaveEnvelope;
      if (!envelope.saveId) envelope.saveId = stableHash(envelope.careerSeed, "save", envelope.slotId);
  ...
  private async writeEnvelope(adapter: StorageAdapter, slotId: number, envelope: SaveEnvelope): Promise<void> {
    ...
    const verified = this.parseValidEnvelope(await adapter.get(tempKey(slotId)));
    if (!verified || verified.stateHash !== envelope.stateHash) throw new Error("SAVE_WRITE_VERIFICATION_FAILED");
  ...
  private async loadEnvelope(slotId: number): Promise<SaveEnvelope | null> {
    ...
    const primary = this.parseValidEnvelope(primarySerialized);
    const pending = this.parseValidEnvelope(pendingSerialized);
    const previous = this.parseValidEnvelope(previousSerialized);
    ...
    if (pending && (!primary || pending.revision > primary.revision)) {
      await this.adapter.set(slotKey(slotId), pendingSerialized as string);
      await this.adapter.remove(tempKey(slotId));
      return pending;
    }
    if (!primary && previous) {
      await this.adapter.set(slotKey(slotId), previousSerialized as string);
